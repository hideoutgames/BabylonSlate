...
- `ios:sync` preserves the custom Capacitor plugins. `ios:archive` uses a Release generic-device archive, temporary credentials, normal App Store Connect export with automatic version management disabled, bundle/entitlement/privacy checks and direct Fastlane upload. No Apple binaries, signing logs or credentials become Actions artifacts.
