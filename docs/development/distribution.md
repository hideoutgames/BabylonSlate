...
Enable both capabilities for `no.hideout.babylonslate`, then regenerate its App Store provisioning profile and replace `APPLE_PROVISIONING_PROFILE_BASE64` in the `testflight` environment. The profile is checked before archiving, and the exported IPA must retain both boolean entitlements before upload. Debug device signing likewise needs an updated development profile. No new secret is required.
