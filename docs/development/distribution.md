...
- **Engine Settings → About → Automatic Updates** defaults on. Turning it off immediately stops scheduled checks, cancels active downloads and disables installation on exit, including already downloaded updates. Windows session end also defers installation to a later normal exit.
...
## Automatic provisioning qualification: blocked

The intended replacement uses `ASC_PRIVATE_KEY_P8_BASE64` as its only stored Apple secret, with the IDs and compliance settings below remaining variables. It is **not enabled**. The existing manual signer and its three signing secrets remain in use until a supported replacement passes acceptance. No automatic-mode selector or additional credentials have been introduced.

On September 24, 2026, a [credential-free hosted inspection](https://github.com/hideoutgames/BabylonSlate/actions/runs/36041921549) confirmed Xcode **26.6 (17F113)**, iPhoneOS SDK **26.5**, and the installed `xcodebuild -help` on standard `macos-26`. The command interface establishes:

- API authentication takes `-authenticationKeyPath`, `-authenticationKeyID` and `-authenticationKeyIssuerID` together with `-allowProvisioningUpdates`. Automatic provisioning may create/update certificates, profiles and app IDs; enabling it is permission to mutate team resources.
- Export supports `method=app-store-connect`, `destination=export`, `signingStyle=automatic` and `teamID`. A candidate must retain `manageAppVersionAndBuildNumber=false`, `testFlightInternalTestingOnly=false` and `uploadSymbols=false`, then validate the exported IPA before the existing direct upload and independent finalization.
- This Xcode version permits automatic distribution signing of a **manually signed** archive, with limits on provisioning changes. The help does not establish a supported unsigned archive route. `signingCertificate` and `provisioningProfiles` are manual-export options and would be omitted from a qualified automatic path.

Apple's [cloud-signing walkthrough](https://developer.apple.com/videos/play/wwdc2021/10204/) starts from a developer-signed archive, then cloud-signs during distribution. The API key authenticates requests; it does not supply the development signing private key. The reviewed [build settings](https://developer.apple.com/documentation/xcode/build-settings-reference) and installed help do not establish how to repeat this archive step on fresh external runners using only the API key without certificate churn or another signing credential. Unsigned or ad-hoc archives have not been qualified as supported input to cloud signing. Do not transplant Xcode Cloud's internal build flags or add certificate creation/revocation on every run.

Permissions are a separate unresolved gate. For the retained upload/finalization path, **App Manager** is the minimum documented role covering both operations; Developer can upload but cannot perform all build-information/tester management used by [Fastlane/TestFlight](https://docs.fastlane.tools/actions/pilot/). An Admin key is not required by this implementation. For the proposed provisioning path, use a **team** API key: [Apple excludes individual keys from provisioning endpoints](https://developer.apple.com/documentation/appstoreconnectapi/creating-api-keys-for-app-store-connect-api). Team keys cover **every app on the team**, regardless of role; app-ID checks in this repository do not narrow the key's authority. Creating a team key requires a human Admin/Account Holder, distinct from the runtime key's selected role.
...
The App target also requests Push Notifications: Debug uses `aps-environment=development`, while Release and both TestFlight channels use `production`, following [Apple's APNs environment contract](https://developer.apple.com/documentation/bundleresources/entitlements/aps-environment). The provisioning profile and exported IPA must allow production APNs before upload. This enables signing for APNs; it does not request notification permission, register a device token or send notifications. A notification feature and provider integration are separate work; no provider credential is introduced by this capability.
...
In Apple's dashboards, keep the explicit App ID and app record for this team, the two private groups with public links and automatic build access disabled, required agreements, beta contact/review information, app/SDK privacy declarations, and any encryption documentation current. For the retained path, provision the Apple Distribution certificate/private key and matching App Store profile, and an App Manager API identity for upload/finalization. An eligible individual key can retain that user's app restrictions; verify its operations with the pinned tooling before distribution. For future automatic provisioning, an Admin/Account Holder creates a team key under **Users and Access → Integrations → App Store Connect API → Team Keys**, explicitly selects the accepted runtime role, and records its key/issuer IDs. Cloud-signing permission for that runtime key must be established before switching paths; granting a user's checkbox alone is not that evidence.

Before the first Apple upload, replace the Capacitor placeholder with an existing, human-supplied opaque 1024×1024 app icon, review privacy manifests for the app and embedded SDKs, and complete any necessary encryption documentation in App Store Connect. Do not declare an exemption just to suppress a warning. The app targets iPhone and iPad (device family 1,2; iPhone allows portrait plus both landscape orientations, iPad remains landscape-only) and retains native storage, orientation and audio lifecycle integration.
