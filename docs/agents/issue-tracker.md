...
- **Allowed:** omit `model` (inherit parent); **Composer 2.5** (`composer-2.5`); **Grok 4.5** or **Grok 4.6** at **any effort level**, including Fast (e.g. `cursor-grok-4.5-high`, `cursor-grok-4.5-high-fast`, `cursor-grok-4.6-high-fast`). These are equal options.
- **Forbidden:** Claude (Sonnet, Opus, Fable, Haiku), GPT, Gemini, Composer Fast (`composer-2.5-fast`), and any other family. Do not pass those slugs even if the Task tool lists them.
