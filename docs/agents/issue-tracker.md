...
| --- | --- | --- | --- | --- | --- |
| 2026-09-24 | zeron/preview-launch-bug-plan | Preview Build launch | Spec | Preparation errors, retry/cancellation, and migration continuation are covered by five passing Chromium cases. The iPad A16 PWA silent exit still needs its Output Log error or an affected-project reproduction. Windows WebKit lacks `navigator.storage`, so the five fixture-based WebKit checks failed during setup and do not qualify the device fix. | Device/project diagnosis pending; unmerged |
...
| 2026-09-23 | zeron/babylonslate-rendering-handoff | Final integration review | Spec | Linux export rejected a valid draw while its GPU fence remained pending; completion now has a separate bounded deadline. Imported shadow fixtures omitted export settings and allowed changing pixel dimensions. Focused regressions, software/native host checks and current-build Computer Use pass; fresh Linux CI remains required. | Local repair verified; CI pending |
| 2026-09-21 | zeron/babylonslate-rendering-handoff | September 2026 rendering handoff | Spec | Stock instance IDs/registrations interfere across consumers. Narrow extension authorized on 22 September; shared owner, bounded passes and native survivor pixels now pass on both APIs. See [qualification](../design/renderer-qualification.md#current-continuation-status-23-september-2026). | Resolved 2026-09-23 |
| 2026-09-21 | zeron/babylonslate-rendering-handoff | September 2026 rendering handoff | Spec | Outlines, Scalability and area-light implementation and Chrome/T3 routes are qualified for the recorded desktop APIs. Physical A16 remains explicitly deferred and unmeasured. Current-head CI/reviews/merge remain delivery gates. | Browser scope resolved 2026-09-23; delivery pending |
| 2026-09-23 | zeron/babylonslate-rendering-handoff | Rendering handoff review | Standards | Sequential review of the main-to-head diff found no remaining documented-standard violation; compact existing UI and package/resource boundaries retained. | Reviewed |
| 2026-09-23 | zeron/babylonslate-rendering-handoff | Rendering handoff review | Spec | Hands-on acceptance found Windows-invalid loose asset filenames and Play leaking its hardware scale into the editor. Portable names preserve manifest IDs; Play restores borrowed scale before views resume. Targeted tests and actual exports/Stop reruns pass. | Resolved |
| 2026-09-23 | zeron/babylonslate-rendering-handoff | Rendering handoff CI repair | Spec | Frozen Material Graph submeshes retained stale WebGPU light buffers after camera updates. Refreshing missing or rotated bindings preserves native pixel parity without shader rebuilds; binding/shadow regressions and the native clustered oracle pass. See [repair evidence](../design/renderer-qualification.md#integration-acceptance-after-main-663). | Resolved |
...
Spec: [engineplan.md](../engineplan.md) §14, Appendix A `p19-*`. Additive on Done P11. Do **not** uncheck `p11-*`. **P18** is Done. **P20** is Done (editor/runtime perf, not a P19 reopen).
