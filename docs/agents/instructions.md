...
- Pass focused regressions and `pnpm verify:local` before opening any PR, including drafts. The preflight may be diff-only for instruction/prose metadata; add the full workspace `pnpm typecheck` for an intentional public cross-package API change. If verification or client access is unavailable, disclose that limitation and retain the compatibility entry points.
...
Agents own delivery through merge: integrate main, run focused regression tests and the `pnpm verify:local` preflight, then open a draft, wait for a free Verify slot, mark ready, and monitor CI. Local selection uses owner typechecks and related unit/tooling contracts, builds actual docs-site changes, and keeps agent/prose metadata diff-only; root build inputs and unknown paths retain conservative static checks. The full workspace typecheck, exhaustive coverage, consumer tests, and browser suites run in CI. On failure, reproduce the relevant case, fix it, pass local preflight, push, and wait for the new CI run. Merge only the verified current head once all required checks and branch protections pass, and confirm the merged state. Missing tooling should be repaired where possible; an unresolved blocker must be reported, never treated as a successful test. See [the workflow](../../.agents/rules/agent-workflow.md) and [PR cadence](../../.agents/rules/github-actions-pr-cadence.md) for the authoritative gates.
