...
**Actor Class graphs** may use `anim.actor.*`. Get/Set Anim Graph Variable, Get Current State, and Jump To State take a required `AnimationGraphComponent` target so they operate on that instance (wire **Get Component Ref**). An unwired Target is a no-op (empty name/id; jump does nothing). `tickAnimGraphs` evaluates every live `AnimationGraphComponent` on the actor; pending jumps and eval state are keyed by component. `animState` stays one command per actor slot — last graph in component order wins playback. `anim.event.*` / `anim.rule.*` / `anim.state.*` stay gated to Animation Object / rule hosts. Jump is pending until the next `tickAnimGraphs` so Actor `onTick` (inside `world.tick()`) cannot race the evaluator.
...
- **Variables** (left) — compact `--chrome-row` (28px) name `Input` + `PinTypePicker` limited to Bool / Int / Float / String, and trash `IconActionButton`. **Add Variable** / **Add State** use `Button` `size="sm"` (not 44px touch). States are a compact selected list (`bg-primary/20` + ink bar); the entry state carries an **Entry** `Badge`. Keeps `data-testid="anim-graph-parameters"` for existing e2e. Selecting a state from the list or canvas updates Details **without** `fitView` / `focusedNodeId`; only compiler diagnostics / Frame zoom.
- **Graph** (primary) — `GraphEditor` with `animGraphNodeTypes` / `animGraphEdgeTypes`, `defaultEdgeOptions` type `animTransition`, `uniqueDirectedPairOnConnect`, `connectEndMode="zone-add-node"` (far empty-canvas release opens Add State; release on or near a compatible pin — including an occupied plate — snap-connects; near-source tap/release and a second pointer cancel without breaking transitions). One directed pair `(from, to)` stays one visual edge (reconnect updates handles). The canvas honours Engine Settings **Default Zoom** (`graphDefaultZoom`) like Class graphs. Unreal-style rounded state nodes (`min-width: 200px`, `min-height: 88px`) show the state name, a muted clip line (clip asset name, the Model clip name, **No Clip**, or **Missing Clip**; editor-only `clipLabel` node data, never persisted) with a loop glyph when the state loops, and the green ENTRY mark on the entry state (same node height as the others), with **four thin side handles** (~16px plates, geometric ticks; the body is the 44px move target). Each side stacks `*-in` and `*-out`; dropping on the source plate still becomes a transition after handle migration rewrites it to the input plate. Directed `MarkerType.ArrowClosed` paths use a more open bezier stub (`animTransitionPath`); **Both Ways** is two `AnimTransition` rows that render as one visual edge with arrows on both ends. CSS-triangle blend badges rotate with the edge tangent, flipped toward the target when a short hop makes the curve double back. Single-click a badge selects that blend rule exclusively (deselects states and other badges; Details). **Break Links** then deletes only that visual edge (and the hidden reverse Both Ways row). Details **Open Rule** (not the badge) opens a nested rule graph: breadcrumb (ghost **State Machine** button, chevron, current `Source To Target`), undeletable Enter/Exit sinks. **One Way** dims **Exit State** (unused reverse) and compile/eval treat it as always true (`__disabled` is canvas-only; `persistTransitionRuleGraph` strips it); **Both Ways** keeps both sinks live on each directed rule. Palette: `scriptPaletteNodes(..., { animationGraphHost: "rule" })` — `anim.state.*` queries, pure math/bool, Get Variable; no events, no Set Variable, no `debug.log`. A palette pick from a dangling wire auto-connects the **opposite** side (`right-out` → `left-in`).
- **Details** (right) — titled `PropertyGrid` sections for the selected state: **State** (Name, Entry State), **Clip** (**Clip Kind**, **Clip Asset**, **Clip Name**), **Playback** (Speed, Loop), then one **Transition To Target** section per outgoing blend. A badge-selected blend shows a single **Source To Target** section. Each blend section has **Blend Seconds** / **Priority** / **Direction** (`One Way` | `Both Ways`) and an action row: Open Rule, **Flip Direction** (enabled only for One Way; swaps `from`/`to` and pin plates, then selects the new source state so Details stay on that blend; works from a badge selection too), and Open Reverse Rule for Both Ways. Nothing selected shows **Select a State or Transition**. While a To State rule is open, Details shows the selected scripting node (Inspector); **State Machine** in the breadcrumb restores state/transition Details. Clip Kind **Animation**: Clip Asset is **Animation** catalog rows only (`allowedTypes: ["Animation"]`, picker title **Pick Animation**). Clip Name is hidden unless a leftover `clipAssetType === "Model"` row is still bound (then a `SearchDropdown` of Model `payload.clipNames`). Empty catalog shows disabled **No Clips**. Clip Kind **Sprite**: Clip Asset is **Sprite Animation** only; Clip Name is hidden. `kind: "animation"` `assetGuid` is the Animation Play seeks (`clipAssetGuid` on stamped groups). The actor still loads the **Model** GLB from `MeshComponent.assetGuid`. `validateAnimGraph` emits `anim.skeletonMismatch` when that Animation’s `skeletonGuid` does not match the owning Model’s `skeletonGuid`. Importers store Model `clipNames` and `skeletonGuid` on the header payload.
