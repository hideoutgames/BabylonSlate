...
- Engine bases: `BTTask`, `BTDecorator`, `BTService`, `BTComposite`. Built-ins: Wait, MoveTo (crowd when the runtime host is attached), SetBlackboardValue, Loop / Cooldown / TimeLimit, BlackboardIsSet, CompareBlackboardValue. RotateToFace and PlayAnimation are catalogued and **succeed** without a host until **P18**. PlaySound stays a catalog stub until **P16 Audio**.
