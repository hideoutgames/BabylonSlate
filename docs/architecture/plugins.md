...
Project Settings shows the selected icon immediately after the plugin name, and independent **Experimental** and **Beta** badges. Enabling a flagged plugin names its maturity in the confirmation. Dependency status includes `Missing Dependency`, `Dependency Cycle`, `Engine Version Changed`, `Dependency Version Changed`, and `Blocked Dependency`. Engine and dependency version differences appear as selectable inline warnings. They do not block enabling or open a compatibility popup; the separate maturity confirmation remains.
...
`discoverProjectPlugins` / `discoverEnginePlugins` → `shadowEnginePlugins` (project guid hides the engine original) → `resolvePluginGraph` (Kahn topo) → `mountEnabledPlugins`. Mounting and Project Settings diagnostics resolve the enabled plugin set, so a disabled prerequisite is unavailable. Diagnostics carry explicit severity: `plugin.unsatisfiable` and `plugin.engine_unsatisfiable` are warnings; `plugin.cycle`, `plugin.missing`, and transitive `plugin.dependency_blocked` are errors. Version differences, including an unknown recorded engine version, do not block mounts, Play/editor utilities, or export/Preview Build. Legacy `acceptedCompatibility` values can suppress warnings for the previously reviewed context, but are never required to enable a plugin. Missing/disabled dependencies and cycles still block the affected plugins and their dependents; independent plugins still mount. After mount, missing `header.dependencies` guids and override guids with no discovered plugin become `Unresolved` placeholders that keep the guid (`placeholder: true`). Remounting the plugin replaces the placeholder.
