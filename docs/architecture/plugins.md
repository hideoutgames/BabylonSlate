...
| Field | Role |
| --- | --- |
| `displayName`, `pluginGuid` (= header guid), `version` (author-controlled nonempty string, such as `0.1`, `2.7`, or `1.0.0`), `description`, `author`, `category`, `iconKey` | Identity. Optional Lucide key, not generated art |
...
| `engineVersion`, `pluginDependencies[]` | Automatically recorded engine version and dependency `{ guid, version }` identities. Engine version comes from `release/version.json`; dependency versions use exact author strings. |

Opened as DockView kind `plugin-settings` (Details panel). Identity includes a **read-only GUID** row (the PluginSettings asset guid — not editable, so `pluginOverrides` stay keyed). **Icon Key** is a searchable dropdown with Lucide previews; **Default** clears the override. Engine/dependency version fields are absent. New plugins record the running engine version; **Add Dependency** selects another installed plugin and records its current author version; self and duplicate choices are excluded. Dependencies can be removed, and missing plugins remain visible for repair. Engine plugin documents are read-only. Saving descriptive PluginSettings fields refreshes discovered metadata, diagnostics, and mounted roots immediately without updating recorded compatibility. Saving a changed author Version or an actual change to a plugin-owned asset document records the current engine and installed dependency versions. Reading, importing, re-exporting, no-op saves, and metadata-only edits preserve the recorded compatibility; authoring stamps describe the editing environment, not a guarantee of compatibility.

Project Settings shows the selected icon immediately after the plugin name, and independent **Experimental** and **Beta** badges. Enabling a flagged plugin names its maturity in the confirmation. Dependency status includes `Missing Dependency`, `Dependency Cycle`, `Engine Version Changed`, `Dependency Version Changed`, and `Blocked Dependency`. Enabling an outdated plugin opens **Enable Outdated Plugin** with the engine/dependency differences and **Disable** / **Try Enable Anyway**. Maturity warnings share this confirmation. Dismissal does not enable the plugin. An enabled plugin blocked by changed versions exposes **Review Compatibility**.
...
`discoverProjectPlugins` / `discoverEnginePlugins` → `shadowEnginePlugins` (project guid hides the engine original) → `resolvePluginGraph` (Kahn topo) → `mountEnabledPlugins`. Mounting and Project Settings diagnostics resolve the enabled plugin set, so a disabled prerequisite is unavailable. Diagnostics: `plugin.cycle`, `plugin.unsatisfiable`, `plugin.missing`, `plugin.engine_unsatisfiable`, and transitive `plugin.dependency_blocked`. Version mismatches block mounting until the user accepts that exact compatibility context. Project overrides persist an opaque `acceptedCompatibility` value bound to plugin identity/version, recorded/current engine, and recorded/installed dependency versions. Disabling clears acceptance; version changes invalidate it. This same decision is honored by registry remount, Play/editor utilities and export resolution, including preset enablement. Missing or disabled dependencies and cycles cannot be bypassed. Other invalid plugins and their dependents stay unmounted; independent plugins still mount. After mount, missing `header.dependencies` guids and override guids with no discovered plugin become `Unresolved` placeholders that keep the guid (`placeholder: true`). Remounting the plugin replaces the placeholder.
