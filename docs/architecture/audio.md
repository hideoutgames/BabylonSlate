...
Streaming music, mic capture, authored acoustic zones/materials, **triangle** runtime occlusion/ray tracing, waveform editing, DSP plugins, IR convolution, converting Texture/Model/Audio `asset-settings` to DockView, P17 idle-unmount, BT RotateToFace / PlayAnimation (P18). Voxel DDA muffling on the existing occupancy bake is additive (still no triangle rays).
