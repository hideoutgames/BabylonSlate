...
Native fixtures exercise the packaged Havok solver in NullEngine, separately from browser/device rendering; the unresolved gates are recorded below. Physics geometry preparation optimizations are a separate workstream; these transactions do not claim geometry-sized tick discovery has been removed.
