...
- While active, bone bodies replace the actor's ordinary collision. The actor follows the simulated root's translation; physics owns bone rotations. Existing **Add Impulse** affects the full assembly. Collision queries return the owning actor.
...
Local checks were deferred at the user's request on 2026-09-22. Branch `agent/engine-physics-lifecycle-b`, implementation checkpoint `7177dda9`, is integrated into draft [PR #663](https://github.com/hideoutgames/BabylonSlate/pull/663) and remains **unverified and unmerged**. Keep `BL_TEST_PROFILE=shared` and the machine-wide resource configuration unchanged when resuming. The physical A16 check was waived; local native/browser evidence is still required.
