...
| **Dialog** (`DialogTitle`, `DialogHeader`, `DialogContent`, …) | Centered modal. Title required. Close control is `touch-icon` (44px) on the existing `DialogPrimitive.Close` path. | Global search, Play prepare, Content Browser rename/move/New Asset, CatalogDialog shell, SearchDialog, debug console, Preview session report, navmesh bake; Class member Get/Set chooser after dropping a variable onto the graph. |
