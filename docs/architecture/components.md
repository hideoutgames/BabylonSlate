...
| Component | What it does | Used for |
| --- | --- | --- |
| **PanelFrame** | Docked panel shell (`--sidebar` body, optional title/toolbar). Omit the title when Dockview already shows the tab name. | Outliner, Details, Inspector, Graph, Class, Prefab, Output Log, Compiler Results; Animation Graph Parameters / Details docks; live Editor Utility tabs; Sprite / Tileset / Tilemap docks. |
...
| Component | What it does | Used for |
| --- | --- | --- |
| **GraphEditor** | Touch-first React Flow shell (`pinCompatibility` overrides the exact-kind connection rule; material graphs use it so a Float splats into a vector pin): Blueprint node chrome; Development Only and Editor Only hazard-tape footers (`data.__editorOnly`); hollow pin visuals until linked (`data-pin-connected="false"\|"true"`); tap- and drag-to-connect; cancelled pin-drag disconnect; Break Links; marquee; Add Node. Hosts may pass `nodeTypes`, `edgeTypes`, `nodesDraggable`, `nodeDragHandle`, `commitPositionsOnDragEnd`, `connectEndMode` (`default` keeps the 96px cancel + wire-break; `add-node` opens Add Node even on a short drag and never breaks), `replaceIncomingOnConnect`, `canConnect`, `emptyPaneDoubleTapAddsNode`, `toolbarExtra`, attachment selection, `hiddenToolbarActions` (Copy / Paste / Delete / Break Links / Format), `lockNodeDragAxis` (optional sibling-only reorder), `contextMenuItemsForNode` / `contextMenuItemsForAttachment` (pane + node `ContextMenuOverlay`), and `PaletteNode.nodeType`. Reconciles external `initialGraph` updates (undo/redo, Inspector) without emitting `onChange`; identical graphs are not re-emitted. | Graph document panel; UserInterface Logic (Class docks); Material / Material Function / Animation Graph / Behaviour Tree DockView hosts (catalog `__pins` hydrated so Add Node is not an empty box; Anim Graph persists `AnimState.position`; BT uses parent-child handles, optional `editorPositions`, and tree-mode menus). |
