...
| **TypeVisualIcon** | Colored Lucide glyph for an asset / class family (`resolveTypeVisual`). `size` defaults to 16 (chrome); pass 40 (`TYPE_VISUAL_ICON_TILE_SIZE`) on Content Browser tiles so SVG `width`/`height` match the CSS box. | Outliner, Details, Prefab, Content Browser, global search, Place Actors, Add Component, AssetPicker, ClassPicker, SceneComponentPicker, document tabs; gallery. |
