...
| **Badge** | Compact status chip. | Compilation errors, Content Browser type tags, global-search result kinds, live-wire “Tap to Add Node”. |
...
| Component | What it does | Used for |
| --- | --- | --- |
| **GraphEditor** | Touch-first React Flow shell (`pinCompatibility` overrides the exact-kind connection rule; material graphs use it so a Float splats into a vector pin): Blueprint node chrome; Development Only and Editor Only hazard-tape footers (`data.__editorOnly`); hollow pin visuals until linked (`data-pin-connected="false"\|"true"`); tap- and drag-to-connect; **exec** pins multi-connect, **data inputs** replace on a second wire; cancelled pin-drag disconnect; Break Links; marquee; Add Node. Hosts may pass `nodeTypes`, `edgeTypes`, `nodesDraggable`, `toolbarExtra`, attachment selection, `hiddenToolbarActions`, `lockNodeDragAxis` (sibling-only reorder), `contextMenuItemsForNode` / `contextMenuItemsForAttachment` (pane + node `ContextMenuOverlay`), `PaletteNode.nodeType`, and `commitPositionsOnDragEnd` (Material graphs commit XY once per drag via `onChange(graph, meta?)`; topology still commits immediately). Reconciles external `initialGraph` updates (undo/redo, Inspector) without emitting `onChange`; identical graphs are not re-emitted. | Graph document panel; UserInterface Logic (Class docks); Material / Material Function / Animation Graph / Behaviour Tree DockView hosts (catalog `__pins` hydrated so Add Node is not an empty box; Anim Graph persists `AnimState.position`; BT uses parent-child handles, computed layout, and tree-mode menus). |
| **NodePalette** | CatalogDialog of host-legal nodes with role-color chips. Honors the host `pinCompatibility` rule when filtering by a dragged pin. Footer **Context Sensitive** switch (`data-testid="node-palette-context-sensitive"`, default ON): pin-drag / pin filter applies only when on. Search and category reset every time the dialog opens. | Add Node from GraphEditor (second pointer during pin drag, and double-tap empty pane). |
