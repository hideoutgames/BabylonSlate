...
- Append after successful edits, Undo and Redo on an open document (`appendJournalLine` in derived data). A multi-command history action is one `edit.batch` record with ordered child payloads, so concurrent actions cannot interleave its deltas. Legacy single-command lines still replay. An unsupported child skips the entire batch rather than applying only part of it.
