...
- Fixed-size shadcn `Card` tiles (`grid-cols-[repeat(auto-fill,7rem)]`, `size="sm"`): square thumb (`aspect-square` + `object-cover`, type-colored Lucide fallback via `resolveTypeVisual` when no thumbnail), then `CardTitle` / `CardDescription` / badges. Titles strip a trailing `.{type}` suffix; type stays in the description. Cells do not stretch with `minmax(…, 1fr)`. Class tiles inherit the parent engine icon (Object / Actor / Widget) and share `--asset-class`.
- Root folder `assets` lists the whole tree; any other folder shows **direct children only** (`collectFolderGuids`).
- **Click / tap selects** (replace selection; `data-selected`). **Tap / click empty grid** (padding/gaps, not a tile) **clears selection**. **Double-click / double-tap opens** every catalog type via `openOrFocusDocument` (`documentKindForAssetType`). Hold ~250ms arms a move; hold still ≥500ms then **release** opens the tile/folder context menu (`useHoldDragMenu`). The workspace root does **not** bind `useContextMenu` pointer handlers — tiles and folder rows own the gesture so the overlay cannot cover a drag.
- Folder tree drop highlight (`dropPath` / `bg-accent`) follows HTML5 `dragover` and touch hold-drag via `elementFromPoint` on `[data-folder-path]` / `[data-asset-folder]`. The registry tree root (`assets`) is **not movable** (no hold-drag, not HTML5-draggable) but **accepts** asset and nested-folder drops so items can return to the root.
...
- Long-press multi-select + `ContextMenuOverlay`: Duplicate, Rename, Move, Copy, Show References, Retry encoding, Delete; folder right-click opens the delete confirm for the folder tree (assets root is protected). **Duplicate** / copy allocate `stem_N` in the destination folder (`stripTrailingCopyIndex` then `nextCopyName`; keep the unsuffixed stem when unused) and write that name into both the file path (preserving `.scene.babasset` / `.class.babasset` / `.graph.babasset`) and `header.name`, with a new guid. Rename confirm is disabled on a colliding path.
...
- Content Browser helpers (filter / new-asset / unique names / drag MIME / folder-tree flatten for Move / drop-target hit testing) unit-tested in the editor. Tile and folder-tree hold-drag vs menu-on-release covered in jsdom (`content-browser-asset-tile.test.tsx`, `content-browser-folder-tree.test.tsx`).
- E2E: Scene/Class **double-click** open, PNG+GLB import with **Importing** overlay then reload, killed-tab journal recovery (`e2e/p2-accept.spec.ts`); density IA, empty-grid deselect, unique-name/duplicate (`e2e/editor-density.spec.ts`).
