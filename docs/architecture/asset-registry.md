...
**Encode path:** the editor wires `createWorkerEncodeFn` with `editorEncodeWorkerUrl()` (`publicAssetUrl("basis/encode-worker.js")` so GitHub Pages `/BabylonSlate/` and Capacitor `/` both resolve) plus vendored Basis encoder wasm under `apps/editor/public/basis/` into `EncodeQueue`. Worker init errors clear the cached `ready` promise so the next job can spawn a new Worker. Decode prefers `createImageBitmap` and falls back to `Image.decode()` when that rejects (Safari / odd MIME). Unit tests keep `stubEncodeKtx2` as the default when no Worker is injected. CI runs a real Basis encode smoke (`a16-encode-smoke.test.ts` / `createNodeBasisEncodeFn`) against checked-in A16 wall envelopes in `@babylonslate/test-kit`. On project enter, `probeKtx2TranscoderAvailable(editorKtx2PublicBase())` marks compressed textures `fallback_uncompressed` when decoder files are missing. Editor `createEngine` passes the same KTX2 base.
...
- Thumbnails: `generateThumbnailBytes` at import → `writeThumbnail` in derived data; CB grid lazy-decodes visible Texture cells via `ThumbnailDecodeLru` / `loadAssetThumbnail`. **P16** revokes off-screen blob URLs and skips decode while the browser tab is CSS-hidden.
