...
- Fixed-size shadcn `Card` tiles (`grid-cols-[repeat(auto-fill,9rem)]`, `size="sm"`): square thumb (`aspect-square` + `object-cover`, compact `size-10` type-colored Lucide fallback via `resolveTypeVisual` when no thumbnail), then `CardTitle` / `CardDescription` / badges. Cards mix `--asset-*` into `--card` / `--muted` (`typeColorCardAccent` / `typeColorThumbAccent` from DataTypes) for darker type accents. Titles strip a trailing `.{type}` suffix; type stays in the description. Cells do not stretch with `minmax(…, 1fr)`. Class tiles inherit the parent engine icon (Object / Actor / Widget) and share `--asset-class`.
...
- Content Browser helpers (filter / new-asset / unique names / Move destination validity / folder+asset flatten / drop-target resolution / additive selection / type-card accents) unit-tested in the editor. Tile long-press menus and empty-grid pointer isolation covered in jsdom (`content-browser-asset-tile.test.tsx`, `content-browser-folder-tile.test.tsx`, `content-browser-move-dialog.test.tsx`). Registry `duplicateFolder` / `copyFolder` / in-place folder rename via `moveFolder`.
