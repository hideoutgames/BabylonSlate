...
- **Click / tap selects** (replace selection; `data-selected`). **Tap / click empty grid** (padding/gaps, not a tile) **clears selection**. **Double-click / double-tap opens** every catalog type via `openOrFocusDocument` (`documentKindForAssetType`). Stationary hold ≥500ms or right-click opens the tile/folder context menu (`useLongPressMenu`). Early pointer movement scrolls. Tiles and folder rows own the gesture; the workspace root does not bind `useContextMenu` pointer handlers.
...
- Content Browser helpers (filter / new-asset / unique names / Move destination validity / folder-tree flatten and search) unit-tested in the editor. Tile and folder-tree long-press menus (no drag-to-move) covered in jsdom (`content-browser-asset-tile.test.tsx`, `content-browser-folder-tree.test.tsx`, `content-browser-move-dialog.test.tsx`).
