...
- Every catalog type opens a tab. Scene / Class / UserInterface / Font / Sprite / AnimationGraph / Shader keep their existing editors. Texture, Material, Model, Audio, Animation, Enum, Structure, and ScriptInterface open an **`asset-settings`** tab in `AssetDocumentWorkspace` (PropertyGrid / member rows / `ParameterListEditor`) — not a new Shader or UI designer.
