...
- **Click / tap selects** (replace selection; `data-selected`). **Tap / click empty grid** (padding/gaps, not a tile) **clears selection**. **Double-click / double-tap opens** Scene and Graph via `openOrFocusDocument`. Hold ~250ms arms a move; hold still ≥500ms then **release** opens the tile/folder context menu (`useHoldDragMenu`). The workspace root does **not** bind `useContextMenu` pointer handlers — tiles and folder rows own the gesture so the overlay cannot cover a drag.
...
- **Graph** `.graph.babasset` is a **preview document** for Actor UI (Graph + Prefab + Components + Class). It is not a long-term engineplan catalog type; Import does not create Graph assets; class-owned graphs are a later follow-up.
...
- **New Asset** uses type + engine-base parent-class pickers → `registry.createAsset`. Create is disabled when `newAssetFileName` already exists in the selected folder (`new-asset-name-taken`). `createAsset` throws if the path exists (no silent overwrite).
...
- File ops: `moveAsset` / `renameAsset` / `duplicateAsset` / `copyAsset` / `moveFolder` keep guids stable on move/rename; duplicate/copy assign a new guid and a unique `stem_N` name. Open Scene/Graph tabs are retargeted via `DocumentService.repathDocument`; `refreshAssetRegistry` rewrites `project.json` scene/graph path lists.
...
- E2E: Scene/Graph **double-click** open, PNG+GLB import with **Importing** overlay then reload, killed-tab journal recovery (`e2e/p2-accept.spec.ts`); density IA, empty-grid deselect, unique-name/duplicate (`e2e/editor-density.spec.ts`).
