...
- **Click / tap selects** (replace selection; `data-selected`). **Tap / click empty grid** (padding/gaps, not a tile) **clears selection**. **Double-click / double-tap opens** Scene and Graph via `openOrFocusDocument`. Hold ~250ms arms drag-reorder; hold still to ~500ms opens the context menu.
...
- Content Browser helpers (filter / new-asset / unique names / drag MIME / folder-tree flatten for Move) unit-tested in the editor.
