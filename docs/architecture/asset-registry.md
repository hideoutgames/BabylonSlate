...
- Content Browser helpers (filter / new-asset / unique names / Move destination validity / selection menu intersection / folder+asset flatten / drop-target resolution / additive selection / thumb-only type accents) unit-tested in the editor. Tile long-press menus and empty-grid pointer isolation covered in jsdom (`content-browser-asset-tile.test.tsx`, `content-browser-folder-tile.test.tsx`, `content-browser-move-dialog.test.tsx`). Registry `duplicateFolder` / `copyFolder` / in-place folder rename via `moveFolder`. Encode queue timeout, worker error, remount requeue of `pending`/`encoding`, and Texture preview / maxDimension helpers.
