...
- **New Asset** creates authored types only: Scene, Class, UserInterface, EditorUtilityInterface, Sprite, AnimationGraph, Shader, Enum, Structure, ScriptInterface. Import-only types (Texture, Material, Model, Audio, Font, Animation) are created by Import, not the New Asset button.
