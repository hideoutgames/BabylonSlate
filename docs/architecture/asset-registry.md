...
- **Click / tap selects** (replace selection; `data-selected`). **Tap / click empty grid** (padding/gaps, not a tile) **clears selection**. **Double-click / double-tap opens** Scene and Graph via `openOrFocusDocument`. Stationary hold ≥500ms or right-click opens the tile/folder context menu (`useLongPressMenu`). Early pointer movement scrolls. Tiles and folder rows own the gesture; the workspace root does not bind `useContextMenu` pointer handlers.
