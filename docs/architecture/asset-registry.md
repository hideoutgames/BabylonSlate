...
- One toolbar row: Import, New Asset, shared **SearchInput** (clear when non-empty), **Filter** dropdown (multi-select type checkboxes; empty = all types), **Deselect All** and Delete when a tile is selected. No page heading or subtitle.
- Fixed-size shadcn `Card` tiles (`grid-cols-[repeat(auto-fill,9rem)]`, `size="sm"`): square thumb (`aspect-square` + `object-cover`, compact `size-10` type-colored Lucide fallback via `resolveTypeVisual` when no thumbnail), then `CardTitle` / `CardDescription` / badges. Cards mix `--asset-*` into `--card` / `--muted` (`assetTypeCardAccent` / `assetTypeThumbAccent`) for darker type accents. Titles strip a trailing `.{type}` suffix; type stays in the description. Cells do not stretch with `minmax(…, 1fr)`. Class tiles inherit the parent engine icon (Object / Actor / Widget) and share `--asset-class`.
- Folder tree and asset grid are native vertical scrollports (`min-h-0 overflow-y-auto overscroll-y-contain`). The grid itself is nested inside `content-browser-asset-grid`; overflow is not on `display: grid`.
- Root folder `assets` lists the whole tree; any other folder shows **direct children only** (`collectFolderGuids`).
- **Click / tap adds to selection** (`addSelectedAssetGuid`; `data-selected`). It does not replace. **Deselect All** (`content-browser-deselect-all`) or **tap / click empty grid** (padding/gaps, not a tile) **clears selection**. Reveal-from-search still focuses a single guid. **Double-click / double-tap opens** every catalog type via `openOrFocusDocument` (`documentKindForAssetType`). Stationary hold ≥500ms or right-click opens the tile/folder context menu (`useLongPressMenu`). Early pointer movement scrolls. Tiles and folder rows own the gesture; the workspace root does not bind `useContextMenu` pointer handlers.
...
- Long-press + `ContextMenuOverlay`: tiles get Duplicate, Rename, **Move…**, Copy, Show References, Retry encoding, Delete. Nested folders get **Move…** and Delete (assets root is protected). **Duplicate** / copy allocate `stem_N` in the destination folder (`stripTrailingCopyIndex` then `nextCopyName`; keep the unsuffixed stem when unused) and write that name into both the file path (preserving `.scene.babasset` / `.class.babasset` / `.graph.babasset`) and `header.name`, with a new guid. Rename confirm is disabled on a colliding path.
- **Move…** opens `ContentBrowserMoveDialog`: item preview, `SearchInput`, editor-kit `TreeView` of `flattenFolderTree(assetRegistry.folderTree("project"))` with folder icons. Illegal destinations (unchanged parent; folder into itself or a descendant) are muted and not selectable. Confirm calls `moveAsset` or `moveFolder`. After a folder move, open document tabs for contained assets are retargeted. Rename stays a text dialog.
- File ops: `moveAsset` / `renameAsset` / `duplicateAsset` / `copyAsset` / `moveFolder` keep guids stable on move/rename; duplicate/copy assign a new guid and a unique `stem_N` name. Open document tabs are retargeted via `DocumentService.repathDocument`; `refreshAssetRegistry` rewrites `project.json` scene/graph path lists. `listDocumentPaths` treats Class and legacy Graph as compile/script documents. New projects scaffold `assets/main.class.babasset` with `parentClass: "Actor"`.
...
- Content Browser helpers (filter / new-asset / unique names / Move destination validity / folder-tree flatten and search / additive selection / type-card accents) unit-tested in the editor. Tile and folder-tree long-press menus (no drag-to-move) covered in jsdom (`content-browser-asset-tile.test.tsx`, `content-browser-folder-tree.test.tsx`, `content-browser-move-dialog.test.tsx`).
- E2E: Scene/Class **double-click** open, PNG+GLB import with **Importing** overlay then reload, killed-tab journal recovery (`e2e/p2-accept.spec.ts`); density IA, additive multi-select, Deselect All, empty-grid deselect, vertical scrollports, unique-name/duplicate (`e2e/editor-density.spec.ts`).
