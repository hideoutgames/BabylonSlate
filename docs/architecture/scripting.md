...
- **New graphs** seed native events from `nativeEventStubs` via `createDefaultLogicGraphSerialized` (Actor: Begin Play + Tick; BT parents as in Class Events; FunctionLibrary / EditorFunctionLibrary: empty). Existing `logMessage` graphs hydrate to `debug.log` pins without auto-injecting events. Class Add Node uses `scriptPaletteNodes({ parentClass, parentOf, classId, graph, otherClassGraphs, activeFunctionId, functionLibraries })` so BT leaf events do not appear on Actor graphs, editor-only defs stay off runtime hosts, and Call Custom Event / Call Function / Get Variable / Set Variable rows are injected per class member (locals only when that function graph is open). Static FL Call Function rows come from open FL docs and `header.payload.functions`. UserInterface Logic defaults `parentClass` to BObject and passes its graph so UI Variables get Get/Set too. Hydrate regenerates `flow.event.call`, `functions.call`, `flow.function.input`, `flow.function.output`, `variables.get`, and `variables.set` pins so type, Target, and Class function Input/Output handles stay in sync.
