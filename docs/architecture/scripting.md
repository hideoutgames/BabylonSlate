...
Texture / Audio stay compact `asset-settings`. Model, Skeleton, and Animation open DockView documents (Preview + Details). Sprite opens a DockView document (Preview + Details). FunctionLibrary remains a Class parent, not a file type.
...
- **Widget events:** hosts post `uiWidgetEvent`; the worker invokes `onWidgetClick` / `onWidgetValue` / `onWidgetChecked` / `onWidgetText` / `onMouseEnter` / `onMouseExit` / `onMousePress` / `onMouseRelease` on the owning UI with `{ widget, widgetId, value }` (`ctx.args`). Prefixed ids dispatch to the nested instance with the local widget id. Live EUI compiles nested logic into the same ScriptHost and routes the same way (Event Editor On Begin Play, no Tick).
- **Nodes:** **Apply User Interface** `classRef(UserInterface)` → `objectRef(UserInterface)`; **Remove User Interface** / **Set Widget Visibility** take object refs; generic **Get Widget** is hidden — palette injects `Get <Name>` as `objectRef(ButtonWidget)` / `objectRef(MaterialWidget)` / …. **Set Input Mode** (`input.setInputMode`) is allowed on UserInterface Logic (hidden on EUO / EFL / EUI). Actor-only APIs no-op when `self` is a `UserInterface`. `getWidget` is scoped to that UI.
