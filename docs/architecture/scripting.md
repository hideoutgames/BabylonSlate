...
- Tap-to-connect and drag-to-connect both persist (`onConnect`). A pin drag into the empty-canvas zone (outside a 96px screen-space safe zone around the source pin and compatible opposite pins, and not over a node) shows a **Tap to Add Node** badge. A **second pointer** while the drag is held opens Add Node at the wire end and ends the rubber-band; lifting the drag finger does not close the catalog. With **Context Sensitive** on, that menu is pin-filtered and auto-wires; with it off, the menu is the host-legal catalog. Releasing without a snapped handle and without opening Add Node removes every edge on the dragged pin, unless the pointer is still over the source handle (tap-to-connect) or still in the Add Node zone (cancel). Palette uses the shared Dialog catalog shell (`@babylonslate/ui` Dialog + ScrollArea) with a role-color chip per item.
