...
**Audio** (`audio.play` / `audio.setChannelVolume` / `audio.setGlobalVolume`) lives on Class and Actor palettes. Play Sound takes an `assetRef("Audio")` pin and Volume default `1`, and emits `playSound` with `self` as emitter. Set Channel / Set Global clamp `0..1`, replace session mixer values, and update voices that are already playing (warned no-op without a selected mixer / unknown channel). Inspector maps `assetRef` pins to `AssetPicker` with `allowedTypes`. See [audio.md](audio.md).
...
| Asset | Editor | Feeds |
| --- | --- | --- |
| Enum | Row editor | `enumRef` pin types; Make / Equal / Switch; variable and pin Defaults |
...
| Engine registry | `ENGINE_ENUMS` / `ENGINE_STRUCTS` in `@babylonslate/scripting` (`engine:` ids, no Content Browser file) | Same Make/Break / picker paths as project assets. Engine **pin kinds** (`transform`, `rotator`, `color`, `vec2/3/4`) stay first-class types, not fake Structure assets. |
