...
| **Make / Break Structure** | Generic `struct.make` / `struct.break` are hidden. Palette injects `Make <Name>` / `Break <Name>` (`struct.make:<guid>`) per project Structure (and future engine user-style structs in `ENGINE_STRUCTS`). Pins come from a field snapshot so `pins(properties)` runs without TypeContext; hydrate regenerates from the live schema and keeps wires by **field name**. Make: one data in per field + struct out (`{ Field: … }`). Break: struct in + one data out per field (`(in).Field`). Empty struct is `{}`. Engine pin-kind math stays first-class: **Make/Break Rotator**, **Color**, **Transform**, plus **Break Vector2 / Vector4**. Transform graph pins are Location `vec3`, Rotation `rotator` (Euler degrees), Scale `vec3`; codegen writes `{ position, rotation: quat, scale }` to match `identityTransform()` / `transform.get`. |
| **Make Enum / Equal / Switch** | Generic `enum.make`, `enum.equals`, `enum.notEquals`, `enum.toString`, `enum.switch` stay in the catalog. Palette also injects `Make <Name>` / `Equal <Name>` / `Switch on <Name>` with `enumGuid` prefilled. Runtime enum values are **member name strings**. Switch exec outs are one per member plus **Default**; the compiler special-cases `if / else if / else` on `value === "Member"`. Connecting an `enumRef` pin copies that guid onto the node and rebuilds pins; disconnect keeps the last guid and re-enables Details. Type Select is disabled while a wired pin supplies a guid. |
