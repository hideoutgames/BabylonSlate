...
- **Undo**: `AddNodeCommand` / `RemoveNodeCommand` / `SetGraphMembersCommand` / `SetGraphComponentsCommand` in `@babylonslate/edit` so palette adds, Class panel members, and prefab components persist through `diffGraphCommands`.
