...
- **Hold empty pane ~250ms then move** marquees (`attachGraphPaneMarquee` overlay; one-finger pan until the hold arms). Overlay **Format** tidies the selection, or walks the exec/data then-chain to the right of a single selected node (`graph-format.ts`).
