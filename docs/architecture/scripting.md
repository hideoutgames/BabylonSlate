...
| **Call Custom Event** | `flow.event.call`: Target instance (`objectRef` of the event class) plus matching data **inputs** and Then only. Same-class unconnected Target → `ctx.self` when `implicitSelf`. Codegen `ctx.invokeCustomEvent`. Hidden as a generic catalog id; palette injects one row per event |
...
- **New graphs** seed native events from `nativeEventStubs` via `createDefaultLogicGraphSerialized` (Actor: Begin Play + Tick; BT parents as in Class Events). Existing `logMessage` graphs hydrate to `debug.log` pins without auto-injecting events. Class Add Node uses `scriptPaletteNodes({ parentClass, parentOf, classId, graph, otherClassGraphs })` so BT leaf events do not appear on Actor graphs and Call Custom Event rows are injected per class event.
