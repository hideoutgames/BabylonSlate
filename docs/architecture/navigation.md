...
Tile-cache obstacles, 2D bake input, scripting nodes, and crowd `MoveTo` are in (`p11-nav-blockers-2d`). Auto-bake-on-save stays **off** until **P18** (`p18-nav-leftovers`). Dynamic **cost** volumes are recorded but do not yet affect path cost (Recast tile cache obstacles are unwalkable-only; P18 picks a mechanism that can express cost). Bake bounds collect is P18. §18: `packages/runtime/src/p11-acceptance.test.ts` (including a dynamic box that **closes** an open route after MoveTo is running) plus `e2e/p11-ai.spec.ts`.
...
- Place Actors **NavMesh** spawns an Actor with `NavMeshComponent` (Recast numbers, solo/tiled enum, support-dynamic-obstacles, debug overlay). Auto-bake-on-save stays **off** and is not shown in Details until **P18** wires it. Not an Add Component row.
