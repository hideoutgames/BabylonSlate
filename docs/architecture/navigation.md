...
- Play reads the Scene chunk (`readPlayNavmeshBytes`) and posts `loadNavMesh` **before** `realizePlayWorld`. `NavAgentComponent` registers `addAgent` (batch after Begin Play, or lazily on the first Move To); dynamic **unwalkable** blockers register `addObstacle`; **cost** blockers (static and dynamic) call `applyCostVolume`; `tickCrowd` copies pose + `facingYawFromVelocity` into the snapshot and re-stamps moving dynamic cost volumes.
