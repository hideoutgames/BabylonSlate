...
| Renderer | When | How |
| --- | --- | --- |
| Bitmap | Always | Canvas `FontFace` glyphs packed onto an RGBA atlas (`emissiveTexture` + `opacityTexture`). Canvas that does not paint (Node / NullEngine, failed face) uses the bundled 5×7 bitmap. Missing Font uses the project default CSS stack. |
