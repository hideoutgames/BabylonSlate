...
- `applyAssignMaterial` records `materialAssetGuid` per slot (and optional `componentId`) and binds the compiled NodeMaterial from a scene-local `MaterialLibrary`. `createEngine` loads Material / Material Function documents, resolves textures through `ResourceCache`, and rebuilds the camera `postProcessStack` on `loadScene`, `possessCamera`, document updates, and Engine Settings `postProcessingEnabled`. That local gate defaults on and skips attachment without mutating the scene or exported games. Scene Depth / Scene Normal probe device support, allocate a linearized depth renderer or pre-pass renderer only while an attached pass needs them, and report anchored diagnostics when a required buffer is missing. The packaged player hydrates packed Material / Material Function JSON and the startup stack and forwards `assignMaterial`.
- `applyPossessCamera` outranks the Default Camera: `refreshPlayActiveCamera` prefers the possessed slot, then the `isDefault` slot, then the Play fallback camera. The scene `attemptPossessViewTarget` option and the graph node both arrive as the same `possessCamera` command ([engineplan §2.5](../engineplan.md)).
...
`adaptToDeviceRatio: false`; resolution via `setHardwareScalingLevel`. Engine Settings `hardwareScalingLevel` (default 1) is applied when a viewport or Play session starts and when the setting changes. Pause render loop, game worker, and encode queue on background.
