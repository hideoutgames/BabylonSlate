...
**Real-time reflections and volumetric lighting.** Project Settings ? Rendering ? Post Processing adds two optional blocks, persisted in `render.effects` and carried into Play/export. Older projects keep both disabled. Reflections use Babylon 9.20 screen-space ray tracing on PBR surfaces (default half resolution, 32 steps, 50-unit distance), a separable roughness blur and full-resolution Fresnel composition. Existing environment IBL remains the fallback when a ray misses; geometry outside the camera view, transparent surfaces and hidden surfaces cannot be reflected. This avoids rendering six additional scene views for a dynamic cube probe. Metallic/roughness and base color feed reflectivity for authored graph materials as well as native PBR materials.

Volumetrics integrate homogeneous fog along camera rays using directional, point and spot lights (default half resolution, 24 steps, up to four admitted lights). Density, intensity, distance, anisotropy, resolution, steps and light limit are editable. Directional/spot comparison shadows, cascaded sun maps and point cube shadows are borrowed from the managed shadow controller; this effect never creates another shadow map. Unshadowed or unadmitted shadow sources scatter without occlusion. Rect-area lights and hemispheric fill are not volumetric sources. Depth clips the ray and weights the four-tap upsample so foreground silhouettes limit fog bleeding. It is single scattering without temporal accumulation, local fog volumes or multiple scattering; coarse steps can band. No device-specific performance claim is implied by these defaults.

Both paths share the same GLSL/WGSL stages, run before bloom/Display Color, and rebuild for projection, output size, quality or light/shadow topology changes. Light transforms and colors bind live. Effects require WebGL2/WebGPU with depth sampling, four draw buffers and renderable half-float targets; unsupported devices preserve base rendering. Disabled effects allocate no additional targets. An exhausted shared resource budget skips the spatial stages, preserves base rendering and logs the reason. Geometry and intermediate targets participate in the shared Engine resource budget and shader compilation retirement. The FrameGraph adds a demand-driven geometry pass; classic rendering requests the matching native prepass buffers.
...
**D2 (not implemented).** SSAO, TAA, LUT and capture stages remain out of scope; the versioned stage contract reserves `colorPipeline.version` for their addition.
