...
Requested path changes enter the existing viewport loading/shader/first-presentation barrier. Engine session requests and Play/player serialized settings reach the same renderer policy; a session change re-keys shader/presentation admission like a project change and reports the new status through `renderPathStatus` bridge messages. Project Settings shows authored Render Path and GPU Backend preferences; it has no Active Scene status section. A viewport-generation lease rejects stale status callbacks and cleanup. `EngineHandle.renderPathStatus()` and `RenderDiagnostics.pipeline` expose plain metadata; `EngineHandle.setRenderPath` requests the session path (`null` resumes the project path). The path controller does not call `Scene.render` or replace FrameGraph ownership. Device performance, WebGPU clustering and unsupported material/layout parity remain separate gates; no Apple A16 qualification is implied.
