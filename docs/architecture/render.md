...
**Dockview GUI host (P12):** EditorUtilityInterface **live** tabs (`p12-editor-extensions`) are Dockview panels, not the designer. They need the same ADT-copy path inside a Dockview tab (resize with the panel, freeze when the tab is hidden, dispose on close). Do not `registerView` a utility canvas. Last P12 (`p12-ui-editors`) uses that same surface for UserInterface and EditorUtilityInterface **authoring** (Dockview Design canvas + working `presentAdtToCanvas`) so widgets render while editing.
