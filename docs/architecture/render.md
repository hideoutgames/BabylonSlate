...
- **Seconds and pause:** `updateSpeed = 1/60` at creation; `setPaused` writes 0 and restores it. Systems that become ready while paused start on resume, so prewarm runs at speed; one stopped before it started drains at once. Live edits never write `updateSpeed`, prewarm or `targetStopDuration`.
...
- **Emission driver** (`particle-emission-driver.ts`): drives Spawn Rate and Lifetime curves over the emitter cycle and schedules bursts through `manualEmitCount`, one frame late. A GPU emitter with Pre Warm advances it only after its first draw, because GPU prewarm inside the first ready render would emit a queued burst.
- **Live edits:** `updateLibrary(library)` returns `{ tier }`. `live` edits write values and rebake GPU gradient textures in place, so particles survive; `respawn` (shape class, gradient key counts, blend into or out of Multiply) calls GPU `reset()`, so particles vanish and emission continues; `rebuild` recreates the bundle. `libraryChangeTier(library)` returns that tier without applying it (it knows skipped slots, which re-prepare on any edit), so previews debounce by it. `setLibrary` keeps Play semantics (later preparations only).
...
- **Playback state:** `playbackState(actorGuid, componentId)` returns `preparing`, `ready-stopped`, `playing`, `draining` or `failed` (`null` without an entry). A drained bundle is released as `ready-stopped`, so previews keep Restart instead of showing a failure.
