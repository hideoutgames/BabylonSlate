...
`@babylonslate/render/bake-provider-prototype` is an isolated authoring experiment using MIT-licensed [three-gpu-pathtracer 0.0.24](https://github.com/gkjohnson/three-gpu-pathtracer/tree/v0.0.24), Three 0.181.1 and three-mesh-bvh 0.9.5. It is a separate import from the viewport renderer and has no persisted scene binding, Bake Lighting UI, or exported-player dependency.
...
**Known v1 gap:** when the bake's contributions `includeEnvironment`, receiver PBR clones set `environmentIntensity = 0` so baked environment diffuse is not double-counted. Babylon exposes no diffuse-only IBL gate on this path, so this also drops environment **specular** from those receivers — accepted for v1; restoring a specular-only environment term is future work.
