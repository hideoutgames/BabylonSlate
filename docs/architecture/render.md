...
- Surface Resolution (8-128) controls target world-space detail. Geometry gains subdivisions as volumes grow, within bounded vertex budgets (193 by 193 for finite surfaces; rivers up to 97 by 385). Large surfaces concentrate detail near the camera while preserving fixed boundaries. Distant under-sampled displacement fades smoothly; per-pixel wave normals retain the surface detail. Physics queries retain the full analytic waves at all distances.
- Realistic shading combines smooth depth absorption, directional swell, layered ripple normals, Fresnel response and filtered highlights. Stylized shading uses color bands, softer highlights and defined foam shapes. Both use flowing, irregular bank/crest foam with screen-space detail filtering. Built-in water reflects the visible skybox when no explicit environment cube is assigned; a scene-owned, leased texture view leaves the skybox sampling and custom materials unchanged. Color depth is estimated from bank distance and component Depth, not scene-depth refraction or object-intersection foam.
