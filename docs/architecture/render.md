...
containers. Changed resources still refresh. Equivalent material documents and effective post-process stacks
...
Lighting uses realtime lights and environment lighting. Baked-lighting authoring, generated UV atlases, assets and runtime/export consumption have been removed. Scenes no longer store bake settings or references; Mesh and Light Details have no bake participation or mobility controls.
...
KTX2 decoding thread and upload format are independent. Packed players can decode on the main thread with self-hosted WASM while retaining ASTC/BC7 on capable hardware. WebGPU rejects an ASTC 4×4 / BC7 texture whose base width or height is not a multiple of 4 (the vendored decoder pads data but not `pixelWidth`), which invalidates every frame; Particle-usage Textures therefore encode block-aligned ([asset registry](asset-registry.md)). Missing compressed-format capabilities and known software renderers still select RGBA. Preview Build retains its existing PNG/pixel payload policy.
...
- Authors produce custom faces with Content Browser **Skybox Creator** (editor-only helper, not a Skybox document). Preview's toolbar holds the primary Create button (disabled without a source) and a status line ("N of 6 Faces Created"); without a source it shows a **No Texture** empty state with **Pick Texture**. Net faces are labelled Up / Left / Front / Right / Back / Down with Babylon axis chips (+X … −Z); unused net cells are dimmed. Details lists **Generated Faces** (n / 6) rows only once a face exists, and Cubemap shows **No Cubemap** until then. Preview letterboxes a 4×3 net; CSS grid cells **stretch to fill** it so each compass face is square (¼ × ⅓ of the net). Decode the source Texture once (sniff PNG/JPEG MIME for the overlay blob; skip KTX2/`pixels` stubs and use `source`). Overlay placement undos merge as `skybox-creator-source`. That overlay is what Create crops; **Create Skybox Textures** writes six uncompressed `usage: "skybox"` Textures, commits face guids on the helper **before** registry refresh (so Save All cannot miss them), and surfaces registry errors as-is. **Cubemap** re-slices from the cached RGBA when placement changes. Assign those guids on `SkyboxComponent`. Equirect-to-cubemap projection is out of scope.
...
- **Per-emitter backend:** a Basic emitter gets `GPUParticleSystem` (`emitRateControl: true`) when the owning engine supports compute or transform feedback, else CPU `ParticleSystem` with `min(capacity, 512)`. The owned GPU system claims its whole slot ring after construction and `reset()`, behind a runtime layout guard; at capacity the GPU recycles the oldest particle and the CPU drops new ones.
- **Particle Graph slots** are always CPU `ParticleSystem`s at the authored capacity, built from Node Particle blocks (`particle-graph-realize.ts`, `particle-graph-blocks.ts`):
  - The service lowers the document with `@babylonslate/particle-graph`, caching per document object, then builds it synchronously inside `prepare` into one `NodeParticleSystemSet` per slot. Blocks are constructed directly, never through `Parse` or `buildAsync`, and settings are written on a `SystemBlock` subclass before the build.
  - Validator errors skip the slot with `particle.graph_invalid`; Babylon build failures skip it with node-anchored `particle.compile.*` and leave no system or texture.
  - Retire stops the system, then disposes the set, which releases the blocks, the system and its readiness texture. `ParticleSystemSet.emitterNode` is never used.
  - Details: [particles → Particle Graph](particles.md#particle-graph).
...
- **Live edits:** `updateLibrary(library)` returns `{ tier }`. `live` edits write values and rebake GPU gradient textures in place, so particles survive; `respawn` (shape class, gradient key counts, blend into or out of Multiply) calls GPU `reset()`, so particles vanish and emission continues; `rebuild` recreates the bundle. For a Particle Graph, a compile-key change rebuilds only that slot, a Material change (`live`) rebinds the running system, and node moves or renames do nothing. `libraryChangeTier(library)` returns that tier without applying it (it knows skipped slots, which re-prepare on any edit), so previews debounce by it. `setLibrary` keeps Play semantics (later preparations only).
- **Stop and drain:** per-emitter stop halts the driver before muting. CPU slots (the Basic fallback and every graph) finish at zero live particles; GPU slots wait the lifetime bound, because the claimed ring always reports full capacity. `resetParticleSession` / `dispose()` on Play close, `changescene` and despawn.
...
- **Stats:** `statsScope: "global"` (default; Play and player) publishes `systems`, `playing`, `gpu`, `gpuSystems`, `graphSystems` to test-mode `window.__babylonslateParticleStats` (`particleStats`); previews pass `"local"`. `previewStats()` returns `{ active, capacity, backend, approximate }` for the Preview badges. Play open/close must return `systems` to 0.
...
**Native path.** `SceneEffectsOwner` mirrors the same plan and order with owned camera passes — extract-highlights + separable blur + `BloomMergePostProcess`, `ImageProcessingPostProcess`, `FxaaPostProcess` — retired through the shared compile-guarded `PostProcessRetirement`. Native effects detach before an authored-stack replacement so the new chain preserves pass order and never borrows retired prepass buffers. Fog-only and reflection configurations use distinct prepass cache identities. The native and graph consumers are mutually exclusive through `useGraph()`, exactly like the authored stack owner, and the coordinator whitelist counts `passes` as live native attachments.
...
- Ocean is a finite rectangular volume. Width, Length and the actor/component scale resize its bounds; it stays in place when the camera moves. Queries outside those bounds return Found false. Existing Ocean components now use these finite bounds; use **Global Water Volume** for the previous unbounded behavior.
- Global Water Volume (`GlobalWaterVolumeComponent`) extends to the camera far plane and is unbounded for queries. Set its elevation, Depth, Water asset, waves and current in Details; Width and Length do not apply. Its dense inner grid is anchored to world-sized cells; smoothly graded outer cells cover the horizon.
...
- River uses component-local control points with elevation, a width and a directed current. **Curvature** (0-1) blends straight reaches into a centripetal Catmull-Rom spline through every point (default 1); elevation and width interpolate linearly between points so the water never overshoots uphill. Each point's **Width Scale** multiplies Width, so a river can narrow or widen along its course. Rendering, queries and buoyancy use the same sampled centreline (`waterRiverCentreline`). Flow Speed reverses with a negative value.
- Selecting a single unlocked water actor shows its outline and shape handles on the gizmo layer. Ocean, Lake and Puddle have red Width and blue Length handles (resized symmetrically about the origin) and a green Depth handle below the centre; Global Water Volume has only Depth. Rivers show white point handles (drag across the water plane; elevation is kept), orange width handles beside each point, and faint midpoint handles that insert a point when dragged. Double-click a point to remove it (a river keeps two). The water reshapes live while dragging and one undoable change is committed on release. Point elevations and exact values remain editable in Details.
- Transforms resize and position the water volume, including signed scales. Waves, ripples, foam width and current speed use world metres: moving or stretching a volume does not stretch or drag their pattern. Wave Direction is a world X/Z heading; waves displace vertically. Vertical surfaces cannot be sampled as water height.
...
- Realistic shading treats the surface as clear, absorbing water: transmittance follows Beer-Lambert absorption along the view path, so shallows near a bank stay see-through and deep water turns dark and reflective. Opacity caps deep-water opacity and Depth Color Distance sets the absorption length. The body colour is emitted from a scene light estimate (sun and hemispheric lights plus environment) rather than diffusely lit, keeping ripples glassy. Six sharp-crested wind-chop octaves (Ripple Scale and Ripple Strength; bounded so the WebGL shader still compiles correctly with four scene lights) ride on the analytic swell, calm near the shore, and advect with the current; the sun adds PBR glints and back-lit crest scattering. Realistic foam is a soft, bubbly cell web with fine grain, thresholded by a foam density that shores, objects and steep crests supply: dense near a waterline, it breaks into lace and then scattered patches as it thins, and churned air lightens the water beneath it.
- Stylized shading uses saturated depth bands (Color Bands), a crisp wobbling white shoreline outline with a travelling second ring, soft drifting light streaks, and twinkling star-shaped Sparkles that favour the sun's reflection. Its body is more opaque, fading only at the very edge.
- Both styles are world-space: patterns never slide with the camera. Derivative-based filtering only fades octaves, streaks and sparkles before they would alias. One GLSL-shaped source is translated to WGSL, so WebGL2 and WebGPU match. Built-in water reflects the visible skybox when no explicit environment cube is assigned; a scene-owned, leased texture view leaves the skybox sampling and custom materials unchanged. Shader world positions add the scene's floating-origin offset (`slateWaterOrigin`), because the app engines use large-world rendering and Babylon then supplies eye-relative positions; without it every pattern would travel with the camera.
- Waves: **Choppiness** (0-1) sharpens swell crests and flattens troughs, **Wave Spread** (0-1) fans the swell components around Wave Direction for a confused sea, and **Crest Foam** whitens the steepest crests. Rendering, queries and buoyancy share the same crest profile (`waterWaveProfile`). To hide tiling, the wind-chop octaves use non-harmonic scales and headings, a slow noise field rotates the chop domain, and a slow world-space gust field varies chop strength and foam across open water.
- Contact foam: each built-in surface owns a CPU-built **water field** (`WaterField`), an RGBA8 texture over the terrain and objects that meet it. It stores the signed distance to the terrain shoreline, the water depth over terrain, and the distance to meshes crossing the waterline (triangle cross-sections followed by a Euclidean distance transform, at most 512 by 512 cells and 0.2 m per cell). Realistic water gathers dense, broken foam at the waterline of rocks, piers and hulls that thins into lace; Stylized draws a crisp white outline with a travelling second ring. **Contact Foam Width** (metres) sets their width. Terrain is refreshed when a landscape or the surface moves; objects are re-sliced at most every 100 ms when they move. Where a landscape overlaps the surface, the field supplies true terrain depth for transmittance and shore foam; elsewhere depth is estimated from bank distance, a shelving floor and component Depth. It is not scene-depth refraction, and editor-only helpers, lines, skyboxes, water and removal volumes are ignored.
- Cut-outs: landscapes remove every water surface wherever terrain rises above the water, so a Global Water Volume stops at an island's shoreline instead of drawing through it. **Water Removal Volume** (`WaterRemovalVolumeComponent`) removes water inside a Box, Sphere, Cylinder or Capsule (Width, Height, Length in metres; Width is the diameter for round shapes, Length applies to boxes, and the actor transform positions, rotates and scales it). It cuts every water component, including a boat hull's interior or a well. The editor shows it as a blue wireframe; it is invisible in Play. Up to four removal volumes nearest each surface are cut in the shader; queries and buoyancy honour every enabled volume and landscape, returning Found false inside a cut. A Custom Material draws the full surface; queries remain cut.
...
| Graph | Node | Purpose |
| --- | --- | --- |
| NodeGraph | Sample Water Surface | Queries a world position and optional Water Actor. Returns Found, world surface height, signed immersion depth, surface normal, water velocity, bank distance and the live Water Actor. Outside water returns Found false. Global Water Volume bank distance is infinite. |
