...
`e2e/baked-parity.spec.ts` proves consumption on both backends: a synthetic analytic physical-E atlas (uniform quadrants over generated UV2) reproduces realtime point-light diffuse shading within 3/255 through the plugin on PBR and CEL receivers. `e2e/baked-player.spec.ts` proves the packed path: a Preview Build of a project carrying a synthetic bake hydrates through the player's payload-map `bakeAssetReader` and shades the receiver warm.
