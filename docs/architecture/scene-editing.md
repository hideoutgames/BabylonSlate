...
`setMeshAssets` fingerprints sprite/tilemap/tileset guids, texture/model byte sizes, and `pixelsPerUnit`. Equivalent payloads (new `Map` instances after a transform commit) are a no-op — they must not dispose the live mesh or the gizmo/outline stay bound to a disposed ghost, so the next drag only jumps on submit. When the fingerprint actually changes (first textured load, swapped sprite), meshes rebuild and `createEngine` re-applies the last `setSelectedActors` ids so the gizmo attaches to the new mesh.
