...
- **Details** (`PropertyGrid`): Unreal-style **label | value** rows at `--chrome-row` (32px) with a wider label column (`minmax(0,8rem)`). Labels are Title Cased (`meshKind` → `Mesh Kind`, `2D camera width` → `2D Camera Width`). Category headers use `--secondary`. Vector axes use `--axis-x/y/z` on one nowrap row (letters live on `NumericDragField`). Checkboxes are compact (`size-4`) inside the row. Reset is a compact icon button (`↺`), not the word Reset. **Add Component** uses the same catalog chrome, grouped Rendering / Camera / Physics. Numeric rows (`NumericDragField`) keep an empty typed draft; emptying does not commit `0` — blur restores the last committed number.
