...
**Focus** (toolbar toggle) closes dock tabs that are not on the Engine Settings Focus keep-list. Class default is Graph only; Scene default is Viewport. Keep-listed tabs stay only if they were already open. Saved `layout.json` is restored with `fromJSON`; a throw from an older catalog falls back to the kind’s default DockView layout instead of crashing the editor.
