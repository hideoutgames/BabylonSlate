...
Play instantiates the **open scene tab** (`playSceneFromOpenDocuments` / `resolvePlayScene`). Graphs and Content Browser may stay open; **only one scene document** is open at a time (opening another closes the previous after the Unsaved dialog). **If no scene tab is open, Play is disabled** — it does not fall back to `startupSceneGuid`, `assets/main.scene.babasset`, or demo actors. `startupSceneGuid` on `project.json` is the packaged / export boot scene (asset guid, so rename is safe). Play looks through the first `CameraComponent` in the loaded scene; no camera keeps the default Play camera. See [physics.md](physics.md) and [render.md](render.md).
