...
Existing compact Texture / Audio `asset-settings` tabs and pinned Content Browser are exceptions until converted — do not add new types to that path. Model, Skeleton, and Animation open Sprite-style DockView documents (**Preview** + **Details**). Skybox Creator is a DockView helper (not a Skybox document).
