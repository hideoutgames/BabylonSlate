...
- Set a particle texture's **Usage** to **Particle** (Texture Details). Particle encodes KTX2 at base dimensions rounded up to a multiple of 4; WebGPU rejects ASTC/BC7 textures that are not block-aligned and invalidates the frame's command buffer, so a 1×1 Albedo texture made the preview draw nothing. Particle usage binds (and exports) only a block-aligned KTX2, else source pixels. See [asset registry](asset-registry.md).
...
- Pause sets `updateSpeed = 0` and restores each system's value. The emission driver and the GPU drain clock run on simulation time, so they pause too. A Particle Graph system also sets Babylon's `paused` flag, because its Update blocks run every frame whatever the speed (an Angle + 0.1 input, a re-rolled Random or Camera Position would keep changing); resume clears the flag before starting systems, since a paused system skips prewarm. Systems that become ready while paused (a Restart, a rebuild or a new assignment) start on resume, because prewarm at `updateSpeed` 0 simulates nothing and never runs again.
