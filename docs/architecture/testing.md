...
| --- | --- | --- |
| `node` | node | `packages/core`, `packages/assets`, `packages/edit`, `packages/object-model`, `packages/physics` (Havok via `NullEngine`), `packages/bridge`, `packages/runtime`, `packages/debugger`, `packages/input`, `packages/test-kit`, `apps/docs` (sidebar coverage + repo-link rewriter) — no DOM |
