...
`tilemapChunkVertexData({ kind: "static" | "animated" })` splits the draw and returns animation descriptors alongside geometry. The `:anim` mesh starts on frame zero and is subsequently sought by the scene clock. `parallax` is stored on child `metadata` and applied in Play against the active camera (`tilemapParallaxOffset` formula) after every camera is posed, the active camera is refreshed and the pixel snap has run. Chunks whose parallax is not 1 register per Scene when built and unregister on dispose; Play calls `updateSceneTilemapParallax(worldScene, activeCamera.position)` once per applied snapshot and skips it without an active camera.
...
| Kind | Docks |
| --- | --- |
| Tileset | **Preview** (clickable pixel-aligned atlas) + **Details** (Atlas, Grid, Selected Tile) |
| Tilemap | **Paint** (primary) + **Palette** (left, ~280px) + **Details** (Tilesets, Map, Layers, Selected Layer) |
...
- Toolbar (`ToolbarStrip`, compact controls that grow to 44px on coarse pointers): **Move** (default, Lucide `HandIcon`) | **Select**, a divider, the **Collision** group None / Full / Chain, and **Paint Collision**; the selected tile or selection count sits at the trailing edge. In Move, one-finger drag pans; a tap (movement < 8px) still selects. In Select, drag a rectangle to select several cells; collision, flags, animation, and chain-point edits apply to every selected tile. Selection commits on release; a second finger or canceled pointer discards the pending selection. Two-finger pinch/pan and wheel zoom stay in both tools. `tileset-preview-cell-{id}`. Full cells show a hatch; Chain draws the stored polyline.
- With no Texture, **No Texture** covers the atlas with a **Pick Texture** action; a missing Texture offers the same action.
...
- Details groups **Atlas** (Texture, read-only atlas size), **Grid** (Tile Width / Height, Margin, Spacing), and **Selected Tile**: a swatch cropped from the atlas, the tile ID with its column and row (or the selection count), a collision badge, then the tile rows.
- Tile Width and Tile Height keep local drafts until **Confirm Tile Size** (directly below them) applies both together, avoiding atlas reconstruction on every keystroke. Undo/redo refreshes the drafts from the restored dimensions.

### Tilemap

- Details sections use `PropertySectionTitle` bands with counts: **Tilesets**, **Map**, **Layers**, and **Selected Layer** (the layer rows follow the list they edit).
- **Tilesets** is a `NamedListEditor` (`Add Tileset` opens `AssetPicker`). Rows show `PickerIdentity`. Empty copy: “Add a Tileset to start painting.” Removing a Tileset opens a confirmation explaining that its painted cells on all layers will be erased; Cancel preserves them, and confirmation is one undoable document edit. The Tileset asset itself stays in the project. Several tilesets share one GID space on the map — not one tileset per layer.
- **Palette** loads each listed tileset with `loadAssetDocument` (closed tabs included) plus Texture `pixels`. Thumbs are cropped with `tilesetTileRect` and nearest-neighbor, grouped under sticky tileset headers with a tile count; they are at least 36px on desktop and 44px on coarse pointers, and the selected tile carries the amber selection ring. Tap sets the paint GID. The compact search bar keeps Clear inside the field; the tile list scrolls in the remaining panel height. The Paint toolbar (`ToolbarStrip`) groups the tools, the **Paint Layer** picker, and the brush: a 28px selected-tile thumb (44px on coarse pointers; `data-gid` / `data-tile`) with its Tileset and tile ID, not a text Palette dropdown. With no Tilesets, Paint shows **No Tilesets** and Palette **No Tiles**, each with **Add Tileset**.
- Palette and Paint share a Tileset GUID/local-tile selection, so range relocation and undo cannot silently change the brush. Open Tileset edits refresh their loaded payloads even when GUIDs stay the same. The working map reconciles capacity for display; only an authored edit persists it. Unavailable selected cells cannot overwrite painted tiles after atlas shrink or removal. A stroke captures its brush and working map, and canceling for a pinch restores the authored pre-stroke payload.
