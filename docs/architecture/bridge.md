...
| Commands | worker → main (ordered) | `spawn`, `despawn`, `assignMesh` (`meshAssetGuid` + `meshKind`, optional `light` / `camera` payloads), `possessCamera` (`slotId`), `setShadowQuality` (`level`: `off`/`512`/`1024`/`2048`), `assignMaterial`, `log`, `diagnostic`, `stats`, `uiSetVisible`, `uiApply` (`instanceId`, `assetGuid`), `uiRemove` (`instanceId`), `animState` (`slotId`, `stateId`, `normalisedTime`, `blendWeights`, optional `clipName` / `clipKind`), `playSound` (`assetGuid`, `volume`, `frameId`), `setRenderResolution` (`width`, `height`; session-only Play framebuffer) |
