...
Play and exported player hosts enable the cooperative runtime path (`cooperativeSceneLoading`) through their shared load factory. It creates the complete Actor set before realization and preserves the owned SceneLayer remap, spawn, design-pose, anchor, and assignment passes. Chunks yield after at most 32 Actor operations or 8 ms; one Actor/component setup remains indivisible. The immediate runtime and public `createSceneLayer` APIs preserve synchronous behavior. Cancellation tracks actual Actor and SceneLayer instances so a stale same-guid load cannot remove its replacement. Failed preparation retains its rejected readiness latch.
