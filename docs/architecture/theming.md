...
- Delete confirms use a stable width (wider when references exist) and grow in height up to the large-dialog bounds. A compact icon-and-text header sits above one column: a warning callout, then bordered **Deleting**, **Replace Class**, and **Referenced By** lists with count badges. Rows show type icon, name, and path on one line, wrapping on narrow screens; the Referenced By list is subtly tinted and omitted when empty. All sections share a contained touch-scroll region. Headers and Cancel/Delete remain visible.
...
Numeric drag fields retain compact 28px inputs and 12px text on desktop and tablet; phone layouts use separate 44px input and scrub targets. Numeric fields evaluate arithmetic on Enter or blur (for example, `30/2` becomes `15`). Sprite transparency checks use theme surfaces. Preview Play/Resume controls share the filled Play glyph, and Tilemap tools expose hover/focus labels. Home uses a trash glyph for permanent project deletion and a close glyph for removing a recent entry.
...
Launcher cards use a subtle 2px lift (and a slight thumbnail glyph or picture zoom) on fine-pointer hover and remain stationary on touch devices.
...
- Galleries scroll vertically; a contact that stops scrolling momentum does not activate a card, and the gallery stays marked as scrolling until a held touch lifts. Off-screen cards skip rendering through `content-visibility`.
- Touch-capable hybrid devices receive the same touch targets as coarse pointers. Mouse hover remains available when using the mouse; touch does not leave card hover motion behind.
...
Favicon is a theme-aware SVG (`prefers-color-scheme`) plus `favicon.ico` and `apple-touch-icon.png` in each app's public assets. The launcher uses the existing icon and text brand **Slate**, styled as an editor workspace on the shared Graphite tokens (dark and light follow the shared engine appearance setting). The window has a 28px titlebar with the brand and Projects / Templates tabs, a panel-header toolbar (New Project, Open, theme, Engine Settings), a panel with an always-visible search/sort/view toolbar, and a 22px status bar showing the project count, storage location, and build label (or the current operation). The former `slate:launcher-scheme` preference is no longer used. Portaled composer/profile content uses the same tokens; `.homepage-theme` keeps only badge colors and launcher-only shadow/viewport values. Profile opens one avatar dropdown for every host: an identity header, Account Settings (Clerk's user profile, when signed in to a web account), Manage Subscription, Application Settings, Engine Settings, Changelog, and Sign In or Sign Out. Web accounts drive it through Clerk's `useUser` / `useClerk` (`openSignIn`, `openUserProfile`, `signOut`); desktop and mobile accounts use the native Clerk session. Application Settings is a small catalog dialog whose Automatic Updates switch shares the Engine Settings preference. It controls the Windows release updater and is disabled on web and mobile. The old launcher-only placeholder preference is no longer used. Changelog opens the bundled release history; first-launch news remembers each dismissed version in Engine Settings. The CSS is imported as text and mounted only with Home, so launcher entrance and card stagger motion never reach the editor; all of it stops under reduced motion.

Projects and Templates occupy separate viewport-sized views, each a vertically scrolling auto-fill grid. Project view density persists independently as large cards, small cards, or a list; column counts follow the available width. Cards show the project name and a relative activity caption (for example, Opened 5 days ago). Card thumbnails are neutral graphite tiles: an uploaded picture always fills the thumbnail edge to edge and is never replaced, otherwise a muted placeholder glyph sits over a faint scene grid. The project color appears only as a small dot beside the name. Every project shows a primary **Local** storage badge (static for now), followed by a secondary **Source Control** badge when that project's settings enable source control. Card badges sit at the thumbnail's top left and list rows place them before the Actions button; small cards and phone-width rows show Source Control as its icon only, with the label kept for assistive technology. List rows and the phone creator use a compact neutral chip with the picture or placeholder, and the creator previews the card itself. Template cards reserve an image well for future catalog images, show a short kind caption, and share the thumbnail proportions of project cards. Importing templates disables the relevant controls without adding a status-text row. Search, Most Used / Name ordering, and ZIP import appear in both template pickers. The creator's Start With step pairs a sidebar (search, All Templates / Starters / Installed filters with counts, and Add Template) with the results grid, matching the two-pane identity step; phones stack the search and filters above the grid. The creator Options disclosure uses only its label and plus/minus indicator, with no border or fill. The create flow transitions from template selection to name/appearance; edit opens the identity step directly. New projects suggest a free three-word name (descriptor, noun, and place or thing, for example Copper Rabbit Club); automation mode keeps TestProject. Uploaded pictures use bounded local raster thumbnails, and vivid preset colors have named, focusable circular choices alongside the picture upload action. Coarse pointers keep compact 32px controls, including the project Actions button, and extend each hit area invisibly to the shared 44px touch target, with gaps that keep neighboring hit areas apart. They also avoid automatic keyboard opening; phone widths hide optional toolbar labels and the status bar. Project context menus support right-click, long-press, and Shift+F10; after Escape restores focus, Enter can open the project immediately without being consumed by the touch-release guard.

The empty Project Browser on every viewport, including phone and short-landscape layouts, frames the sculpture in a small editor-style viewport with Perspective / 3D chips above Create Project and Browse Templates actions, and loads `homepage-sculpture.tsx` and its Blender-created `public/launcher/slate-object.glb` on demand. The model cuts the existing Slate logo silhouette through only the front slab in Blender, preserving the source image's 289:371 aspect ratio; it has no printed logo texture. The two rear slabs are simple beveled meshes; the optimized GLB is about 290 KB with 4,706 source mesh vertices. This isolated Three.js renderer has a capped pixel ratio and 30 fps loop, pauses on document visibility changes and behind loading, launcher dialogs, profile menus, or Templates, renders statically with reduced motion, and disposes its GPU resources on unmount. It also draws the drifting grid floor as screen-space anti-aliased shader lines that fade with distance and cell density, so the floor does not shimmer or form moiré. Only the Metal 3D cards are shown; the 2D card placeholder is removed. All layouts wait for the model readiness callback before releasing the loading cover, and show no decorative placeholder while the model loads or if rendering is unavailable. Switching Projects and Templates retains the canvas while pausing hidden rendering. A matching HTML/React splash is a small editor window: a titlebar with the brand and build label, a dark viewport with lightweight animated slab shapes over a grid floor, and an info footer, on an opaque Graphite backdrop that follows the appearance setting. It shows the engine or project name, release build version (Development build for local previews), and real loading status. It has a two-second minimum and waits for the launcher assets; project launch keeps the cover through editor route setup and fades it away. The cover is portaled above the composer and unmounts its stylesheet afterward. The model was created for the explicitly requested Blender design iteration; it does not change the general artwork policy.
