...
Favicon is a theme-aware SVG (`prefers-color-scheme`) plus `favicon.ico` and `apple-touch-icon.png` in each app's public assets. The launcher uses the existing icon and text brand **Slate**, with cool graphite and light silver palettes scoped under `.homepage-theme`. Its theme toggle persists `slate:launcher-scheme` independently of editor appearance and defaults to dark. Portaled composer/profile content shares the launcher palette. The CSS is imported as text and mounted only with Home; editor tokens and density remain independent.

Projects and Templates occupy separate viewport-sized views with native horizontal pagination. Project view density persists independently as large cards, eight-card (4 by 2) desktop pages, or a list. Project pictures stay on the beveled badge; Lucide glyphs render without forced parent 3D layers. Static inverted dot vignettes and subtle badge-colored glows sit behind the badges. Template cards reserve an image well for future catalog images. Search, Most Used / Name ordering, and ZIP import appear in both template pickers. The create flow transitions from template selection to name/appearance; edit opens the identity step directly. Uploaded pictures use bounded local raster thumbnails, and vivid preset colors have named, focusable circular choices alongside the picture upload action. Coarse pointers receive 48px targets and avoid automatic keyboard opening.

The desktop empty state loads `homepage-sculpture.tsx` and its Blender-created `public/launcher/slate-object.glb` on demand. The model cuts the existing Slate logo silhouette through each slab in Blender; it has no printed logo texture. This isolated Three.js renderer has a capped pixel ratio and 30 fps loop, pauses on document visibility changes and behind launcher dialogs or Templates, renders statically with reduced motion, and disposes its GPU resources on unmount. Phone and short-landscape layouts use the existing mark without loading the 3D module. Switching Projects and Templates retains the canvas while pausing hidden rendering. A matching HTML/React loading cover with slowly aligning slab outlines around the existing logo has a two-second minimum and waits for the launcher assets; project launch keeps the cover through editor route setup and fades it away. The cover is portaled above the composer and unmounts its stylesheet afterward. The model was created for the explicitly requested Blender design iteration; it does not change the general artwork policy.
