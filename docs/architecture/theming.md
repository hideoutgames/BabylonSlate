...
Default Blueprint shells use Tailwind `w-max min-w-80` and grow with `whitespace-nowrap` titles and pin names (`shrink-0`, no `min-w-0` pin columns) plus a `gap-6` gutter between in/out labels. Pin rows are `w-max` so left cluster + gutter + outputs size the shell. Compact BT nodes use `min-w-56`. Get Variable uses a `w-max` pill (`rounded-full`, `min-h-14`, `px-8`, `gap-4`, no `min-w-80`) outlined in the value pin color. Title bars are `text-base`. Pin rows stay `--touch-target` (44px). Text pin defaults (string, numeric, type name) use `w-fit min-w-fit` and truncate at `--graph-pin-default-max-width`, so they contribute `min(text, 12rem)` instead of collapsing under `overflow: hidden`. `graph-editor.css` repeats that as `[data-pin-default-field]` (`width`/`min-width: fit-content`) and `[data-pin-row] { width: max-content }`. XYFlow wrappers in `.graph-editor-canvas` use `width: max-content` so a previous measured px width does not clip that chrome.
