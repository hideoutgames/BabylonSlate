...
- Opening a utility adds a Dockview panel (`eui-<guid>`, component `editor-utility`). Last placement is stored in `layout.json` like any other tab. Focus **keeps already-open** `eui-*` tabs even when Engine Settings has a non-empty keep list (they are not silently closed). Changing `dockKind` while a live tab is open closes it on the old dock rather than orphaning it.
