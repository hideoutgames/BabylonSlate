...
- `e2e/p14-preview-build.spec.ts`: default overlay Play; toggle on/off; missing startup scene alert; and a booted proof (`data-startup-scene` matches the project guid, `data-booted="true"`, ticks advance, no `data-error`, canvas laid out) so a black overlay cannot pass again.
