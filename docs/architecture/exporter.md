...
- Closure BFS (GameInstance, EUO/EUI strip, plugin disable, sprite `textureGuid` payloads, reachable UserInterface + nested UI / image / Font family), zip `index.html` at root, packed boot + per-scene packs, Havok XOR Rapier, file-count warn/fail, range + whole-fetch dual servers, `pixelsPerUnit` / Font `name` on `game.json`, UserInterface logic as `UserInterface:<guid>` in `scripts.js`, Scene navmesh sidecars, Texture `UiImage` sidecars (`uiimage:<guid>`).
- Player `packedContentFromGame` hydrates sprite/tilemap/navmesh, Material / Material Function, and UserInterface payloads from the pack, plus the startup scene post-process stack. `packedBootControls` emits `loadUserInterfaces` before `loadScripts` and drops UI class ids from `spawn`. GUI Image URLs come from UiImage sidecar bytes, not KTX2 labeled as PNG.
