...
- Player `packedContentFromGame` hydrates sprite / Sprite Animation / tilemap / navmesh / audioReverb, Material / Material Function, and UserInterface payloads from the pack, plus the startup scene post-process stack. `packedBootControls` emits `loadUserInterfaces` before `loadScripts` and drops UI class ids from `spawn`. `loadSprites` carries Sprite Animation documents referenced by packed Animation Graphs. GUI Image URLs come from UiImage sidecar bytes, not KTX2 labeled as PNG.
