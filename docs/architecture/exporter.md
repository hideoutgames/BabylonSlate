...
- Closure BFS (GameInstance, EUO strip, plugin disable, sprite `textureGuid` payloads, `audioMixerGuid` → channels / Audio → channel/attenuation), zip `index.html` at root, packed boot + per-scene packs, Havok XOR Rapier, file-count warn/fail, range + whole-fetch dual servers, `pixelsPerUnit` / Font `name` on `game.json`, UserInterface logic in `scripts.js`, Scene navmesh and audioReverb sidecars, packed Audio BSAU.
- Player `packedContentFromGame` hydrates sprite/tilemap/navmesh/audioReverb and Material / Material Function payloads from the pack, plus the startup scene post-process stack.
