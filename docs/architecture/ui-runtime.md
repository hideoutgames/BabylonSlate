...
Dedicated document workspace (not a Dockview Windows menu): **Design** tab (Babylon GUI canvas, widget hierarchy `TreeView`, Details `PropertyGrid`, device-preset selector: **Desired**, built-ins **4:3** / **16:9** / **Widescreen**, then Engine Settings custom labels) + **Logic** tab (`GraphEditor` from `graph-ui` with the same `scriptPaletteNodes` + pin hydration as Class graphs, plus shared `ClassMembersView` against `payload.logic` members). Play compiles that logic with Class graphs. Built-in canvases are read-only. If the selected custom id is deleted, the designer falls back to `desktop-16-9`. Undo via `@babylonslate/edit`. Compose from [components.md](components.md) (`PanelFrame`, `Tabs`, `TreeView`, `NestedMenu`, `PropertyGrid`, `AssetPicker`, `CatalogDialog`, `NamePromptDialog`, `GraphEditor`).

**Parked (`ui-designer-dockview`, not P12):** move this designer onto Dockview and fix editing-stage Babylon GUI present (widgets still fail or show **Babylon GUI Preview Unavailable** in the authoring canvas). EditorUtilityInterface **authoring** reuses `UiDesigner`, so it inherits this until that slice. P12 still hosts **live** Editor Utility widgets in Dockview tabs via `createUiSurface` ([engineplan.md](../engineplan.md) §7.4).
