...
**Keep and build on:**
- The two-tier tab model: a **two-row editor chrome** in [apps/editor/src/components/editor-chrome-bar.tsx](apps/editor/src/components/editor-chrome-bar.tsx) — a title/tab bar (truncated project name with a trailing `.babproject` stripped for display, plus global document tabs with a pinned Content Browser and **no tab-bar Add**) and a **global toolbar** below it (Undo, Redo, Save All with a dirty dot when unsaved, Play, Focus, Search, Settings) — plus a per-document Dockview workspace in [apps/editor/src/shell/dockview-shell.tsx](apps/editor/src/shell/dockview-shell.tsx). This is exactly the Unreal-style shell the engine needs.
...
- **My Blueprint** and the outliner show a warning icon on functions or events whose graphs fail validation.
...
**Scene document:**
- *Viewport*: Babylon view with an overlaid Move/Rotate/Scale `ToggleGroup`, snap toggle, and 2D/3D mode; touch-sized screen-space gizmo handles, selection outline, grid. Play lives on the **global chrome toolbar** only (not duplicated on the viewport).
- *Scene Outliner*: actor hierarchy with search, drag-to-reparent, visibility and lock icon toggles, long-press context menu. Dockview tab is the title — no second PanelFrame heading. **+** opens a **Place Actors** `CatalogDialog` (Shapes, Lights, Camera, Empty, Project assets). There is no Scene Assets / Mini Asset Browser dock; saved layouts close `mini-asset-browser` on restore.
- *Details*: scene settings when nothing is selected (environment and skybox, fog, gravity, fixed timestep, post-process stack, default camera, GameInstance override); otherwise the selection, with transform as one nowrap XYZ row each for Position / Rotation (Euler degrees in the UI, quaternion in `transform.rotation`) / Scale, a components list supporting add, remove and reorder via the same catalog chrome, per-component properties, exposed script variables, and per-property reset-to-default.
...
- *Graph*: event graph and per-function graphs, with a centered **Add Node** catalog modal (categories + search; search is **not** autofocused).
- *Prefab* (Actor): **full-size center-group tab**, sibling of Graph (`direction: "within"`). Same touch gizmos / fly-look camera as the Scene viewport. Not a fourth chrome `DocumentKind`. Persistence is preview-only. **Focus** hides Prefab and Compiler Results so Graph + Inspector + the left stack remain.
- *My Blueprint*: compact collapsible member tree (Graphs, Functions, Variables, Events, Interfaces) stacked **below** Components (`direction: "below"`). Events come from `membersForGraph`; other sections stay empty until class documents store metadata. Inherited members are shown but marked when that data exists.
- *Components* (Actor): component tree in the left dock **above** My Blueprint. Add Component uses the Place Actors catalog chrome (Rendering / Camera / Physics). Add/remove is session-local until prefab persistence lands.
...
**Focus** (global toolbar toggle, disabled on Content Browser): snapshots the current Dockview layout in memory (not `LAYOUT_FILE`), then maximizes the Viewport on a Scene (or closes Outliner / Details / Output if maximize is unavailable). On a Graph it keeps Graph + Inspector + Components / My Blueprint. Exit restores the snapshot. Capture-for-save uses the **pre-focus** layout so Focus does not rewrite the project layout. Switching documents exits focus for that workspace independently.

**Content Browser** (pinned, non-closable): folder tree with **New Folder** on the tree (git-visible marker file; selected row uses catalog ink bar + secondary fill); one toolbar row (Import, New Asset, Search, type filter `ToggleGroup`, Delete when selected — no page heading); fixed-size shadcn Card tiles (`repeat(auto-fill, 10rem)`, square thumb); click/tap **selects**, double-click / double-tap **opens** Scene/Graph (`openOrFocusDocument`; no tab-bar Add control); long-press / right-click context menu; Move uses a `Dialog` + `TreeView` of `folderTree("project")`; rename, duplicate, copy, and delete; Show References from the dependency graph; drag source for viewport, graph and details drops.
...
The parameter list editor is the same component as the ExecuteJavaScript pin-list editor. Both are "a typed, named, reorderable row list feeding generated pins", and building it once in `editor-kit` is the sort of reuse this project is meant to be organised around; the function-signature editor in My Blueprint and the ScriptInterface signature editor are the third and fourth users of it.
...
- **P5 Visual scripting**: IR, type system including both wildcard forms, **continuous graph validation** with Compiler Results and inline markers blocking Preview on errors, validator, compiler with source anchors, core node catalog, the four special nodes from section 6.1, touch graph UI, My Blueprint panel, interfaces, function libraries, enums, structs. *Accepts when* an actor scripted in the editor compiles and runs in the worker, a type mismatch is flagged before Preview and tap-to-navigate focuses the pin, an ExecuteJavaScript node round-trips values through the graph, and compiler golden tests cover all of it.
...
- [ ] **p8-bdebugcommand** — P8: BDebugCommand object base class with command name, description, category and a typed parameter list driving generated OnCommandRun pins; registry discovery through the parent chain; user commands shipped in every export; the parameter-list editor extracted as the shared row-list component also used by ExecuteJavaScript, My Blueprint function signatures and ScriptInterface
