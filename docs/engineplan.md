...
- *Prefab* (Actor): **full-size center-group tab**, sibling of Graph (`direction: "within"`). Same unlit gizmos / fly-look camera / joystick as the Scene viewport. Not a fourth chrome `DocumentKind`. Persistence is preview-only. **Focus** hides Prefab and Compiler Results so Graph + Inspector + the left stack remain.
...
**Focus** (global toolbar toggle, disabled on Content Browser): snapshots the current Dockview layout in memory (not `LAYOUT_FILE`), then maximizes the Viewport on a Scene (or closes Outliner / Details / Output if maximize is unavailable). On a Graph it keeps Graph + Inspector + Components / Class. Exit restores the snapshot. Capture-for-save uses the **pre-focus** layout so Focus does not rewrite the project layout. Switching documents exits focus for that workspace independently. Focus ON is filled (`bg-accent`); OFF is outline.
