...
**Next engine slice:** P11 remaining: `p11-nav-editor-host`, `p11-nav-blockers-2d`. Behaviour trees author and tick in the worker; catalogs stay gated for `NavAgentComponent`. Lighting and cameras are specified in section 2.5 and land as named slice `p-lighting-camera` (render / core / runtime / editor); that slice may run beside P11. The pre-P11 foundation-hardening wave closed Play/scripting contracts that Appendix A `[x]` had overstated (ScriptHost input, tick-clock Delay, spawn/addComponent, GameInstance singleton, editor Play from the open scene tab only, closed-tab class prefabs, real `changescene` library load, ResourceCache textures on sprites/tilemaps, GLB `assetGuid`, authored lights/cameras, HUD TouchButton/DPad, `playSound` command). Do not start P11 editor/runtime wiring from leftover **chrome** polish (pin flash, multi-select gizmo). Parked: `p1-device-spikes` (hardware-only).
...
| Play content from closed tabs | AnimationGraph / Sprite / Tilemap payloads, texture pixels, and GLB source bytes load from scene component guids; UserInterface library is every HUD asset; **Editor Play uses the open scene tab only** (disabled with no scene tab). Packaged/export boot uses `startupSceneGuid` | Done |
...
| Lights / cameras | Foundation: authored Babylon lights/cameras (editor orbit stays; Play uses the first authored `FreeCamera`). Selected camera: dashed frustum + 1 Hz RTT preview. Selected light: dashed range/cone/arrow (`range` / `lightKind` / spot `outerAngle`). Remaining §2.5: direction `(0,-1,0)`, white Play lights, detached `UniversalCamera`, `ShadowGenerator` / IBL, Default Camera `SceneComponentPicker`, Possess Camera | Foundation done; implement remainder as `p-lighting-camera` |
| Unbuilt components in catalogs | `NavAgentComponent` stays gated. `BehaviourTreeComponent` is addable. `WidgetComponent` is hidden until `CreateForMesh` exists. `AudioComponent` is not in Search/Add; graphs use `audio.play` → `playSound` command | P11 / later |
...
| ADT HUD / `.babtrace` tab / §9.4 HUD completeness / full audio mixer | Viewport-layer HUD is nested Babylon GUI (`StackPanel` / `Grid` / `ScrollViewer` / SafeArea) via `BabylonUiApplyHost` (designer + Play). `.babtrace` tab and full `AudioContext` mixer remain parked — `playSound` emits a command and Play logs it | ADT HUD done; rest parked |
...
| UserInterface design canvas | Babylon GUI design-space ADT + screen-space gizmos; CatalogDialog Add Widget; Details edit alignment / left / top / size; 3×3 presets as macros; hierarchy reparent/delete | Done (`cursor/babylon-native-ui-138e`) |
...
- **Packages (P0 names).** `core` (was shared), `vfs` (was storage), `render` (was engine), `graph-ui` (was graph), plus `assets`, `edit`, `object-model`, `scripting`, `scripting-nodes`, `input`, `runtime`, `physics`, `bridge`, `debugger`, `ui-runtime`, `anim-graph`, `shader-graph`, `behaviour-tree`, `navigation`, `ui`, `editor-kit`, `test-kit`. Layout: [architecture/overview.md](architecture/overview.md). Planned but absent: `exporter`, `source-control`, `apps/player`, `apps/desktop`, `engine-plugins/`.
- **VFS and containers (P1).** Binary `ProjectStorage` via `createStorage()`; OPFS web adapter; `.babasset` / `.babproject` codecs; app settings outside projects. Port: [packages/core/src/storage-port.ts](packages/core/src/storage-port.ts). Test hatch: [packages/vfs/src/test-mode.ts](packages/vfs/src/test-mode.ts).
...
- **Content systems (P9).** `@babylonslate/ui-runtime` Babylon GUI layout fields and `previewRect` tables; Font payload + `FontRegistry` on Play and designer ADTs; UserInterface designer (design/logic tabs, Babylon GUI canvas) and Play ADT HUD; TouchJoystick → P6 `touchAxis` / default `Move`; Sprite packer + quad; `@babylonslate/anim-graph` worker evaluator + `animState`; `@babylonslate/shader-graph` IR with throttled preview. See [architecture/ui-runtime.md](architecture/ui-runtime.md).
...
Package-level detail lives under [docs/architecture/](architecture/). Do not follow old paths (`packages/shared`, `packages/storage`, `packages/engine`, `packages/graph`) — those names were removed in P0.
...
- **60fps at native refresh** while playing or interacting, with dynamic resolution scaling as the relief valve before frame drops. Play/Preview is capped at the project `playFrameCap` (default 60). The stats HUD thresholds in section 9.4 are derived from this budget rather than picked arbitrarily. A **visible editor viewport** (scene or Prefab Preview) renders at the Engine Settings frame cap (default 60). Hidden tabs, open modals, Play, and a backgrounded app freeze that loop — that freeze, not dirty-skip of a still visible scene, is the thermal saving in section 2.4.
...
flowchart TB
  ...
  subgraph gameWorker [Game worker]
    ...
    Physics["Physics backend (Babylon Physics V2 or Rapier2D)"]
...
- **Play-in-editor** is a fullscreen React overlay with an X in the top-right, hosting its own game worker and its own Babylon `Scene`. The editor render loop pauses while playing, which matters for iPad battery and thermals. Play itself holds a continuous-render lease so the overlay blit is always preceded by `scene.render()`; the editor stays dirty-driven and is resized + invalidated when Play ends (`syncEditorPlayState`).
- **One `Engine`, many `Scene`s — never an `Engine` per Play session.** Each Babylon `Engine` owns a WebGL context, browsers cap the number of live contexts at somewhere around eight to sixteen, and a user will press Play hundreds of times in a session. Any leak, or even slow reclamation, turns into "the editor stopped rendering" after twenty plays, which is a miserable bug to diagnose. So the editor creates one `Engine` for the app lifetime and Play adds a second `Scene` on it, with `engine.registerView(canvas, undefined, true)` binding the overlay's canvas (`clearBeforeCopy` so the 2D blit cannot composite additively) and `unRegisterView` releasing it on close. Stopping Play disposes exactly one `Scene` and `stopRenderLoop`s that view's callback, which is a bounded and testable operation. Play does not seed the default Cube; snapshot proxies (with `assignMesh` `meshKind`) are the actor meshes. Authored `SerializedScene` actors load via `p7-play-scene-load`.
...
- **Game UI is Babylon GUI’s layout model; editor and debug overlays are DOM.** UserInterface documents store alignment, px/%, padding, and transform center — the same fields `Control` consumes. ADT `idealWidth` / `idealHeight` / `useSmallestIdeal` is the design-resolution scale rule. Safe area is a padded `Container` under the Canvas, not a parallel solver parent. `AdvancedDynamicTexture` re-renders into a texture when anything inside it changes, so a per-frame stats HUD or a keyed Print overlay built in Babylon GUI would re-upload a texture every frame to draw text the DOM renders for free and more sharply. Game-facing widgets still use Babylon GUI because they must composite in-scene and support world-space 2D prefabs via `CreateForMesh` later.
...
- **`NullEngine` is the test seam for `render`, and the production host for the worker-local physics Scene.** It needs no canvas and runs in Node and in workers, which is what makes the Babylon-touching parts of `render` unit-testable at all. `@babylonslate/physics` uses the same `NullEngine` + `Scene` pair in Play so `HavokPlugin` can run beside scripts without a drawable surface.
...
**Visible editor viewports render at the Engine Settings frame cap; freeze when they are not on screen.** A scene or Prefab Preview the user can see runs continuously at `viewportFrameCap` (default 60) so materials, gizmos and camera framing stay live. The thermal win is freeze-when-not-visible: an inactive Dockview tab, a zero-size canvas, an open modal or sheet overlay, Play, or a backgrounded app skip `scene.render()`. Play/Preview itself uses the project `playFrameCap` (default 60), not the editor viewport cap — applied when the session starts from Project Settings. The Play overlay HUD shows FPS / script / physics timings and does not expose a live cap field. Invalidation and refcounted continuous-render leases still exist — they are how the DEV Always Render checkbox, default on, can be turned off to recover a dirty-driven idle loop, and they still honor the same frame cap during camera and gizmo interaction. The stats HUD shows rendered frames per second next to invalidations per second so a stuck Always Render or a missed invalidation after opting out is visible.
...
Authored lights and cameras already exist. The foundation wave creates Babylon `PointLight` / `DirectionalLight` / `SpotLight` and a camera from `LightComponent` / `CameraComponent` in both the editor and Play (`scene-illumination.ts`), dims the default hemispheric `"light"` when any authored light is present, and keeps the editor orbit camera in control. That is a stub, not the lighting system. Direction is still world-down `(0, -1, 0)`, Play lights are white, game cameras are `ArcRotateCamera`, the active camera is whichever CameraComponent appears first in the actor list, and `shadowquality` does not attach a `ShadowGenerator`. This section is the contract. Implementation is the named slice `p-lighting-camera` in Appendix A. It may run beside P11 because it owns `render` / `core` / `runtime` / `editor`, not `behaviour-tree` / `navigation`.
...
`LightComponent`:
- `lightKind`: `point` | `directional` | `spot` (enum, not a text field)
- `color`: RGB 0..1
- `intensity`: number
...
**Sync is incremental.** Lights and cameras are named `authoredLight:<actorId>` and `authoredCamera:<actorId>`. Update the existing Babylon object in place when properties or transform change; dispose only the actor that left the document. Dispose-and-recreate of the whole set on every Details edit is the current stub and thrashes GPU state.

**Play must match the editor.** The worker already emits `assignMesh` with `meshKind: "light:${kind}"` or `"camera"`. That path must apply color, intensity, range, cone angles, enabled, and the full transform (position plus direction from rotation). Extending the `assignMesh` payload or adding `assignLight` is an implementation choice; the contract is parity. White Play lights are a bug against this section.

**At most one shadow map**, as section 2.4 already budgets. The first `castShadows` light in actor order owns a single `ShadowGenerator`. `shadowquality` (already a core console command) selects `off` / `512` / `1024` (default) / `2048` (warn). Further `castShadows` flags are ignored with a one-line diagnostic rather than a second map. Receivers are the meshes that exist; v1 does not add a per-mesh cast/receive authoring pass.

**IBL is optional and off by default.** `SceneSettings.environmentTextureGuid` (nullable) loads a cube through `ResourceCache` with `isCube`. Null or missing means no environment texture. Do not auto-generate IBL from a procedural sky.

**Fog is linear and authored.** Keep `fogEnabled`. Add `fogColor` `[r, g, b]`, linear `fogStart` and `fogEnd`, mapped to Babylon linear fog. Exponential fog is out of v1.

**Clear color is the scene, not the chrome.** `environmentColor` is the Babylon `clearColor` for Play and for the 3D editor scene so authoring is WYSIWYG. Appearance → Theme does not own this. Today's always-dark `EDITOR_CANVAS_COLOR_SCHEME` on the editor canvas is chrome; `p-lighting-camera` replaces the 3D scene clear with `environmentColor`. The 2D editor viewport may keep the dark chrome clear so the pixel/tile grid stays readable; Play still uses `environmentColor` in both modes.

**Cameras are not `ArcRotateCamera`.** The editor orbit camera in `editor-camera.ts` stays ArcRotate. Game cameras are `UniversalCamera` (or `FreeCamera`) with all inputs detached. Pose is the actor world transform each frame: position plus rotation, looking along local `+Z`. Using ArcRotate for a game camera makes FOV and orthographic size fight a radius/target model that gameplay scripts do not own.

**Active camera is a named CameraComponent, not first in the list.** Scene settings expose **Default Camera**: a `SceneComponentPicker` filtered in source to `CameraComponent` (`allowedClassIds={["CameraComponent"]}`, optional None). The picker lists components in the open scene (button shows actor name + Camera). Persist `{ actorId, componentId } | null` on `SceneSettings` (`mainCameraActorId` / `mainCameraComponentId`, or one `mainCamera` object — implementation choice; both ids are required because component ids are not globally unique). Play sets that component’s camera as `activeCamera`. Missing, None, or a stale id (actor/component gone or not a CameraComponent) keeps the default Play camera — do not silently steal the first CameraComponent (today’s stub). The editor never steals (`stealActiveCamera: false`) unless the viewport **Game Camera** preview toggle is on, in which case the named game camera is shown without replacing the stored editor orbit pose.

**2D game cameras are orthographic and look +Z.** Section 13's convention is unchanged: camera at negative Z looking toward +Z, XY plane, +Y up. A CameraComponent placed in a 2D scene defaults `projectionMode` to `orthographic`. `cameraBounds2D` remains the framed-rect overlay in the editor; it is not a second camera. Pixel-perfect snaps the **game** camera to the pixel grid, not the editor pan/zoom camera.

**Editor visualization.** Camera-facing billboard icons for light, camera, and audio actors stay pickable proxies. They are not a `BillboardComponent`. Mesh, sprite, and tilemap still win over the billboard when present. This slice adds directional-light line and spot-cone gizmos on the editor utility layer.

**Details when nothing is selected** matches the section 7.4 promise:
- `environmentColor`
- optional `environmentTextureGuid` (sky / IBL cube)
- fog enabled, color, linear start/end
...
**`SceneComponentPicker` is kit, not a one-off Details row.** Same pattern as `AssetPicker` / `ClassPicker`: SearchDialog, optional None, button shows the current name. Call sites pass `allowedClassIds` in source (e.g. `["CameraComponent"]`) — there is no user-facing type dropdown. The list is the open scene’s components of those class ids (actor name + component title). First use is Default Camera; later scene settings reuse it (a light, an audio listener) without a new catalog primitive. Lives in `@babylonslate/editor-kit`; gallery row when it lands.
...
**Scripting is a small catalog in the same slice.** No camera or light nodes exist today. Add **Possess Camera** (Class graphs): exec in/out, a CameraComponent / actor-camera pin; it changes the **global** Play `activeCamera` for the session (same object as Default Camera, not a local viewport). Also get/set field of view, get/set orthographic size, enable/disable light, set light color, set light intensity. Graphs emit commands or mutate component variables; they do not import Babylon. Possess is a camera switch, not a follow rig.

**Out of v1:** area lights, multiple shadow casters, volumetric lights, required skybox mesh (the guid is enough), Cinemachine-style follow or virtual cameras, extra viewports, render-target cameras, camera stacking, post-process as lighting. **Possess Camera is in v1** (global active-camera switch only).
...
The Content Browser badges `pending`, `encoding`, `fallback_uncompressed`, and `encode_failed` assets and refreshes those badges when encode callbacks bump the registry generation. Project Settings reports how many assets are uncompressed and offers **Retry encoding** for failed, fallback, pending, and interrupted encoding jobs. On mount, interrupted `encoding` is treated as `pending` and re-queued with `pending` and `fallback_uncompressed`. When the transcoder loads successfully in a later session, assets sitting in `fallback_uncompressed` with no KTX2 chunk are **re-queued once** automatically. A hung encode worker fails the job on timeout so later textures still start.
...
**A resolution clamp is the other half of the win and is much cheaper.** Project Settings carries a maximum texture dimension, defaulting to 2048 for the iPad baseline, applied inside the import worker before decode with the original preserved and a registry warning naming any asset that exceeded it. Each Texture may also set `payload.maxDimension` (Source / 4096 / 2048 / 1024 / 512 in the asset Settings tab); effective size is `min(source longest edge, asset max, project max)`. Halving a dimension quarters the memory, so a clamp frequently beats compression outright on a project full of 4096-square source art. Mipmaps stay on for 3D — the extra third of memory buys sampling coherence that matters more — and stay off for pixel art, as section 13 already requires. The Texture Settings tab shows a source preview; Usage (including Pixel Art) is the filter/compression policy — no extra mip/nearest toggles.
...
- **UserInterface** with 2D design and logic tabs (UMG-like). Composable: it may contain other UserInterface assets but not itself, with a cycle check at edit time. Added to the global viewport layer from a class graph (**Apply User Interface** / **Remove User Interface** by instance ref), or placed as a 2D prefab in the world. Not auto-applied on Play.
...
- The **Class** panel and the outliner show a warning icon on functions or events whose graphs fail validation.
...
- **Project browser**: recents list showing display names with a trailing `.babproject` stripped. Left-click opens. A context menu offers **Open**, **Rename** (writes `metadata.name` and the recents label; the folder name is unchanged), and **Remove from list** (drops the recent without deleting project files). **Duplicate** is omitted until VFS can copy a folder cheaply.
...
- **Create Project modal**: **Create Project** opens a dialog. **Name** is required (default `MyGame`; `/?test=1` / `VITE_TEST_MODE` prefills `TestProject`; `.babproject` is appended on submit, not typed in the field). **Template** is selectable **Empty** (default 3D cube), a built-in **2D** card (pixel-perfect Rapier, no cube), plus one card per discovered template (web still has no zip-template folder). **Location**: web is read-only “This browser (OPFS)”; native defaults to **App Documents** with no picker. Optional **Choose folder…** picks then scaffolds. Creating from a template copies the `.babproject` into the new location and rewrites only its name and identity.
...
- **Engine Settings** opens from the Homepage as its own catalog modal, and from the editor via a Settings dropdown that lists **Project Settings** and **Engine Settings** as separate items. The two are never nested in one sidebar. See 7.2. Catalog search is **not** autofocused on open (iPad keyboard would cover the modal), including **Add Node**. Global Search may keep search focused because it is search-first with no category list.
- **Save and Close**: the editor **global toolbar** has icon-only Save All, Undo, Redo, **Compile** (graph documents: hammer icon plus the word Compile, like Play), Play, a labeled **Windows** dropdown immediately left of **Focus**, then Focus, Search, and Settings (`--chrome-row` 28px, `aria-label` plus tooltip). Save All is disabled when the project is clean, writes immediately, and shows a destructive dirty dot when documents are unsaved. Compile is shown on a graph tab, runs `compileGraphDocuments`, writes Compiler Results, and activates that panel; it is disabled when open graph structure/data/edges match the last compile (node positions are ignored). When the active document is a graph and blocking diagnostics exist, a **Compilation Error** status sits immediately after Compile; tap it to open Compiler Results. **Close Project** lives in Project Settings (Close category), not the toolbar, so it stays reachable without colliding with the iPad sash. Close flushes pending writes, releases the folder handle and returns to the Homepage. A dirty-document check runs first, listing exactly which documents are unsaved rather than asking a vague question.
- **Autosave and crash recovery**: WKWebView kills backgrounded tabs without warning, so this is not optional on iPad. Dirty documents schedule a write after `ProjectSettings.autoSaveIntervalMs` (default **120000**); a second edit does not reset an already-running timer. **Save All** stays immediate. When a save runs and `compileOnSave` is on (default **true**), open graphs compile. The undo layer in 7.3 already produces a stream of reversible commands, so the same stream is appended to a per-document journal in the project's derived-data directory (app-private, keyed by project guid, outside the project folder as described in 3.2). Reopening a project with a non-empty journal offers recovery, and a clean Close Project truncates it. Recovery reuses the command layer instead of being a second, separately-buggy serialisation path.

### 7.2 Engine Settings versus Project Settings

Two distinct stores, and the split is enforced by where they live:

- **Engine Settings** are global to the install and stored *outside* any project, through an app-settings port in `vfs` with platform adapters (Capacitor Preferences on iPad, localStorage or OPFS on web, Electron userData on desktop). Contents: templates folder, default project location, recent project list and bookmarks, appearance (theme, coarse-pointer target scale), **undo history length (default 50)**, editor viewport frame cap (which bounds visible scene and Prefab Preview rendering — freeze when the canvas is hidden, obstructed, or a modal is open, per 2.4) and editor hardware scaling level, thumbnail generation on or off, debugger defaults for new projects, **Focus keep-lists** (per document kind: which already-open dock tabs stay when Focus is on), **graph default zoom (default 0.5)** (opening fit-view cap for node graphs; range 0.1–1.5), and **UserInterface designer canvas presets** (`uiDesignerPresets`: custom label / size / safe-area rows; built-ins stay read-only).
- **Project Settings** are per project and live in `project.json`: **startup scene guid** (packaged / export player boot only — editor Play never reads it), GameInstance class, registered EditorUtilityObjects, input mappings, rendering (including the **Play/Preview frame cap, default 60**, **game render size**: `render.customResolution` plus width/height/black bars — new projects default **1920×1080 stretch**; existing projects with the field missing keep Follow System fill. When custom resolution is on it supersedes Follow System for Play and future export; Follow System + stored aspect defaulting to **16:9** remain for the unset path. This is **game** render size, not Preview-only letterbox. Editor viewports still fill the panel. Section 3.5 texture policy — maximum import dimension defaulting to 2048, default compression mode, and the per-usage policy defaults that keep pixel art uncompressed), physics, plugin enable overrides, source control repository and branch, export presets, **`compileOnSave` (default true)** and **`autoSaveIntervalMs` (default 120000)**. The source control *token* is the exception and lives in the platform secret store, never in the file. The General category exposes Compile on save and Auto-save interval (seconds). The Rendering category exposes Play frame cap, Custom Resolution (width / height / Black Bars), Follow System, and Aspect Ratio. The Export category exposes Startup Scene (`AssetPicker`, Scene assets only).
...
**UI:** each store is its own fullscreen `CatalogDialog` (`scope="project" | "engine"`). Project Settings lists only project categories (General, Input, 2D, Rendering, Textures, Export, Close). Engine Settings lists only engine categories (Appearance, Undo, Viewport, Graph, User Interface, Thumbnails, Focus, Templates). Engine is not a row inside Project Settings.
...
**Scene document:**
- *Viewport*: Babylon view with an overlaid Move/Rotate/Scale `ToggleGroup`, **Drag Select**, a **Viewport Settings** menu (Snap, Show Grid, Joystick), and 2D/3D mode. Joystick persists `scene.settings.editorJoystickEnabled`; Show Grid persists `settings.grid.showGrid` (default true). Transform gizmos are stock Babylon classes restyled in place: unlit/emissive axis colors matching chrome `--axis-x/y/z`, thinner shafts and rotation tori, larger translate cones / scale boxes, fat invisible colliders for touch, planar translate squares, hover brighten, and Babylon’s small unlit octahedron for uniform scale (not a custom cube). Scene and Prefab canvases always use dark `clearColor` (`EDITOR_CANVAS_COLOR_SCHEME`) regardless of Appearance → Theme. Play lives on the **global chrome toolbar** only (not duplicated on the viewport).
- *Scene Outliner*: actor hierarchy with **Search** and **+** on the same row (not a PanelFrame toolbar), compact `--chrome-row` (28px) rows, immediate drag-to-parent (pointer move past 8px; drop on a row attaches as child), visibility and lock as compact ghost icons, trailing **⋯** menu for Duplicate / Delete (no row long-press). **Double-tap** a row frames that actor in the viewport. Dockview tab is the title — no second PanelFrame heading. **+** opens a **Place Actors** `CatalogDialog` (Shapes, Lights, Camera, Empty, Project assets). There is no Scene Assets / Mini Asset Browser dock; saved layouts close `mini-asset-browser` on restore.
- *Details*: scene settings when nothing is selected (environment colour and optional environment texture / IBL, fog colour plus linear start/end, gravity, fixed timestep, post-process stack, **Default Camera** via `SceneComponentPicker` filtered to `CameraComponent`, GameInstance override — section 2.5); otherwise the selection, with transform as one nowrap XYZ row each for Position / Rotation (Euler degrees in the UI, quaternion in `transform.rotation`) / Scale, a components list supporting add, remove and reorder via the same catalog chrome, per-component properties (humanized camelCase labels, icon reset; light kind and camera projection as enums), exposed script variables, and per-property reset-to-default.
- Optional docked tabs: Output Log (opened from **Windows**; in the Scene default layout). **EditorUtilityInterface** widgets are not auto-docked here — they are opened from **Windows → Editor Utilities**.

**Class document (Object, Actor, ActorComponent, EditorUtilityObject, BDebugCommand):**
- *Graph*: event graph and per-function graphs. **Double-tap empty pane** opens the unfiltered Add Node catalog (search is **not** autofocused). There is no persistent floating Add node button. **Tap-to-connect** and **drag-to-connect** both persist edges (`onConnect` / `isValidConnection`). Dropping a wire on empty canvas opens Add Node **filtered to compatible opposite pins** only when the pointer is outside a **96px screen-space** safe zone around the source pin and compatible opposite pins (and not over a node body); a drag-wire **Add Node** hint appears in that same empty-canvas zone. Releasing a dragged pin without snapping a handle and without opening Add Node **breaks all wires on that pin** (a tap that stays on the source handle does not). Tap empty pane clears selection; hold empty pane ~250ms then move marquees (custom overlay; XYFlow `selectionOnDrag` is not used). Overlay toolbar: Copy, Paste, Delete, **Break Links** (drop every incident wire on the selection; nodes stay), and **Format** (layout selected nodes, or if exactly one node is selected walk the exec/data chain to the right, pull data-input trees to the left of those nodes, and tidy that chain).
...
- *Class*: **My Blueprint** member tree (Functions, Variables, Events, Interfaces — **no Graphs section**) stacked **below** Components (`direction: "below"`). Initial height is about **50%** of the left stack. Trailing **+** on each section (`NamePromptDialog`); delete via row context menu only. Type-colored rows. Writes `SerializedGraph.members` (variable `typeId` / default, function `pins[]` + `functionGraphs[id]` with protected Input/Output, interface ScriptInterface `assetGuid`). Native events always stay listed (click spawns if missing). Selecting a function switches the Graph dock to that slice. Custom events insert `flow.event.custom`; variables do not spawn Get nodes. Selection drives Inspector Details (function Inputs/Outputs including exec).
- *Components* (Actor): nested component tree (`parentId`) in the left dock **above** Class. Immediate drag-to-parent (drop on a row attaches as child; Prefab Root unparents). Add Component uses the Place Actors catalog chrome (Rendering / Camera / Physics). TreeView `onReparent` writes `SerializedGraph.components` through the command layer.
- *Details*: properties of the selected graph node (canvas selection; unconnected applicable data pins get literal defaults), selected Class member (variable type, function pins, interface asset), or component.
- *Compiler Results*: diagnostics with tap-to-navigate to the offending node.

**Windows** (labeled dropdown immediately left of Focus, disabled on Content Browser): lists every built-in dock tab for the active Scene, Class, Enum, Structure, ScriptInterface, or Sprite document with a check mark when that panel is open. Toggling closes or reopens the panel; the menu stays open so check marks update in place. Reopening uses the last dock placement stored in `layout.json` (`ProjectLayouts.panelPlacements`) when the reference panel still exists, otherwise the catalog default. Captured placement is **addPanel-relative** (where this panel sits relative to the reference, not which way the neighbor is from here). Capture first asks whether the catalog default neighbor still has this panel as an adjacent group (so a full-height Outliner does not steal Output Log in portrait). Otherwise it prefers the catalog default neighbor among adjacent groups, then Viewport/Graph. Mirrored or `within`-dumped placements from older saves fall back to the catalog default. The last remaining panel cannot be closed. A nested **Editor Utilities** submenu is the primary way to open EditorUtilityInterface tabs (empty until P12 registers widgets). Closing a dock tab with the Dockview × control still records placement so Windows can restore it. Focus does not write those placements (it already snapshots layout in memory).

**Focus** (global toolbar toggle, disabled on Content Browser): snapshots the current Dockview layout in memory (not `LAYOUT_FILE`), then closes every open dock tab that is not on the Engine Settings **Focus** keep-list for that document kind. Defaults: Scene keeps Viewport; Class keeps Graph. A keep-listed tab stays **only if it is already open** — Focus never opens a tab just because it is on the list. An empty keep-list falls back to the primary surface (Viewport / Graph). **Add tab to keep** lists every dock tab that can exist on that document, including **applicable EditorUtilityInterface widgets** for that kind (Scene utilities on Scene, class/editor utilities on Class) when those widgets can dock there. Exit restores the snapshot. Capture-for-save uses the **pre-focus** layout so Focus does not rewrite the project layout. Switching documents exits focus for that workspace independently. Focus ON is filled (`bg-accent`); OFF is outline.

**Content Browser** (pinned, non-closable): catalog **TreeView** of folders and assets with **New Folder** above the tree (git-visible marker file; selected row uses catalog ink bar + secondary fill; hold-to-drag reparent after ~250ms, no tree context menu). One toolbar row (Import, New Asset, shared **SearchInput** with a clear control, **Filter** dropdown with multi-select type checkboxes — empty selection = all types — **Deselect All** and Delete when selected — no page heading). Every folder including root `assets` shows **direct children only**: folder tiles first, then assets. Fixed-size shadcn Card tiles (`repeat(auto-fill, 9rem)`, square thumb with a compact type glyph; thumbs use a 2px type-colored `typeColorThumbAccent` border on `--card` with `rounded-t-xl` (folder cards stay uncolored `--card`), text panel stays `--card`). Titles strip a trailing `.{Scene|Graph|…}` (type stays in `CardDescription`). Click/tap **adds to selection** (does not replace), **Deselect All** or tap/click **empty grid** (not a tile) **clears selection**, double-click / double-tap **opens** an asset or **enters** a folder (`openOrFocusDocument`; no tab-bar Add control). The folder tree and asset grid are native vertical scrollports (`min-h-0 overflow-y-auto`; overflow is not on the CSS grid). Logic graphs live on **Class** `.class.babasset` (legacy Graph files still load). **New Asset** creates authored types only (Scene, Class, UserInterface, Sprite, AnimationGraph, Shader, Enum, Structure, ScriptInterface); Texture/Material/Model/Audio/Font/Animation come from Import. Import-only types open compact **Settings** tabs. Enum / Structure / ScriptInterface open DockView documents (Members / Methods / Preview / Details). **New Asset / rename / new folder** refuse an existing name (no overwrite). **Duplicate** allocates `stem_N` in the destination folder (strip one trailing `_digits`, then the next free index; keep the unsuffixed stem when it is unused). Toolbar **Import** on web/electron clicks the in-DOM file input (same path as Playwright); iOS/Android use `pickImportFiles`. A blocking **Importing** overlay shows `n / m` during the write pass only; KTX2 encode stays a background queue with CB badges. Hold still ≥500ms or right-click opens one tile context menu from the selection intersection (Duplicate/Move/Copy/Delete apply to every selected asset and folder; Rename when exactly one item; Show References when exactly one asset and no folders — Retry Encoding is Project Settings only). Empty-grid hold/right-click: New Folder, New Asset, Import. Early pointer movement scrolls. **Move…** / **Copy to Folder…** open `ContentBrowserMoveDialog` (`Dialog` + `SearchInput` + `TreeView` of `folderTree("project")`, illegal destinations muted, **Move 3 items** when the selection is larger than one); confirm loops `moveAsset` / `copyAsset` / `moveFolder` / `copyFolder`. The New Asset name starts empty and Create stays disabled until the user types one. Rename, duplicate, copy, and delete; Show References from the dependency graph.

**Other windows:** Output Log; Play overlay (fullscreen, X top-right) with the debug console dialog, stats HUD layered over it, and a **Preview session report** dialog on close when runtime errors occurred (section 9.7). When project `render.customResolution` is on, Play calls `engine.setSize(width, height)` so the framebuffer is exactly WxH, then either stretches to the overlay (`blackBars: false`) or letterboxes with black unused space (`blackBars: true`). The overlay ResizeObserver must not call `engine.resize()` while that size is locked. `ctx.setRenderResolution` / `setRenderResolution` command overrides WxH for the session only. When custom resolution is off, the Play canvas fills the overlay when Follow System is on, or letterboxes to the project aspect (default 16:9) with black bars when it is off. Editor viewports are unchanged. Debug Trace playback tab; **Sprite Editor** DockView (Preview with cropped frame + pivot overlay, Details with named Texture picker); UI Designer (canvas, widget hierarchy, details, logic graph tab); AnimationGraph editor; Shader graph with live preview; **Enum / Structure / ScriptInterface DockView editors** (Members/Methods, read-only interface Preview, Details — not compact Settings tabs); Texture/Material/Model/Audio/Animation as compact Settings tabs (Texture includes a source preview and Max Dimension); Project / Engine **Settings** as **separate** fullscreen catalog modals (categories + search, search not autofocused; Close Project lives in Project Settings); a Locks panel when source control is enabled; plus a dev-only Component Gallery route for on-device visual checks of `editor-kit`.

### 7.5 Global Search

A **Search** icon button sits immediately left of Settings on the global toolbar and opens a **centered dialog** (not a `SearchDialog` pick list) for project-wide discovery. The query is a case-insensitive substring over indexed labels and keywords. An empty query shows a type-to-search empty state rather than dumping the whole project. Desktop `Ctrl/Cmd+K` toggles the same dialog.

The index is a **separate layer** from the P2 asset registry:

- The registry stays **header-only** (section 2.4). Global search may load Scene and Class **document** JSON chunks, never texture/mesh/audio payloads.
...
- Choosing a hit **opens the asset that contains it**: Scene/Class tabs via the existing open-or-focus path; actors select in the outliner; graph nodes reuse Compiler Results focus (`setFocusDiagnostic`); Texture/Material/Model/Audio/Animation open Settings tabs; Enum/Structure/ScriptInterface open their DockView documents.
...
- **Canonical shadcn theme:** Minimal Neutral chrome (ink `--primary`, near-black/white surfaces) in [`packages/ui/src/styles/globals.css`](../../packages/ui/src/styles/globals.css), with chromatic pin/node/success/axis tokens documented in [architecture/theming.md](architecture/theming.md). Geist remains the UI font. Engine Settings Appearance Light / Dark / System toggles `html.dark` at runtime for **chrome**. Scene, Prefab, and graph **canvases stay dark** regardless of that toggle.
- **UI composition:** all editor chrome and panels compose from `@babylonslate/ui` + `@babylonslate/editor-kit` — no raw styled `<input>`, `<select>`, or `<button>` in `apps/editor/src`. Settings forms use `Field` + shadcn inputs; overlays use `AlertDialog` / `Dialog` / `DropdownMenu`. Pick lists use `SearchDialog` (compact modal) or `SearchDropdown` (anchored menu). Do not use full-width bottom `Sheet`s. `Button` sizes `touch` / `touch-icon` map to `--touch-target` (44px). **Outline** = visible action; **ghost** is for tabs, menu items, and icon-only close. Pressed tools use `Toggle` / `ToggleGroup` with `aria-pressed` and **accent fill + primary border** (not a near-invisible secondary wash). A dev-only **Component Gallery** at `/?test=1&gallery=1` renders every primitive for on-device checks; new panels must pass gallery + touch-target audits before shipping.
...
- **Docking targets stay easy to grab without tall chrome.** Dockview 8 grows some hit areas on coarse pointers automatically. Tab strips are **18px on fine pointers** and **26px on coarse**; a dedicated grip zone on the leading edge of each tab stays at least 44px wide; sashes stay visually thin with a widened hit area via an invisible pseudo-element; drop overlays keep an enlarged edge band. All of it comes from CSS variables in one Dockview theme file, and the touch-target audit covers tabs, grips, sashes, and drop zones as first-class interactive elements rather than treating them as chrome.
...
- **Gesture contract** documented once in `docs/design/gestures.md` and tested: one-finger tap picks; one-finger drag looks the 3D camera and **pans in 2D** (hold ~250ms then move marquees in 2D; **Drag Select** marquees immediately in 2D and 3D then unpresses); pinch zooms; three fingers pan; long-press opens context menus with right-click as the secondary trigger; graph connections support tapping one pin then another **and** dragging a pin onto a compatible handle (both persist). Content Browser **tree** hold-to-drag moves assets/folders (no tree menu); **grid** long-press opens tile menus or the empty-space create menu (**Move…** / **Copy to Folder…** use a destination dialog). Outliner, Components, and UserInterface hierarchy **immediate** drag-to-parent (8px move); Outliner and UserInterface hierarchy **⋯** menus (no row long-press). Tap empty Content Browser grid clears selection.
- **Touch targets**: `--chrome-row` (28px) sizes editor chrome, panel headers, property rows, catalog items, and tree rows; `--touch-target` (44px) remains the floor for **graph pin rows**. Playwright audits assert chrome controls ≥ 28px, dock tabs ≥ 18/26, tree rows ≥ 28, and graph pins ≥ 44. That turns density into a failing build rather than a review comment.
- **Layout**: safe-area insets, no hover-only affordances, anchored dropdowns and centered dialogs instead of full-width bottom sheets, numeric fields that scrub by dragging, and virtual-keyboard avoidance for text entry. Catalog dialogs (`CatalogDialog`, including Add Node, Place Actors, Add Component, and Settings) do **not** autofocus search on open — `initialFocus` is the scrollable body so wheel/touch scroll works. Global Search may focus search because it is search-first with no category list.
...
- **rendered frames per second next to invalidations per second**, plus the current hardware scaling level, which is how the visibility-gated editor loop and the dynamic resolution valve in 2.4 are made debuggable — a frozen obstructed viewport and a runaway Always Render look completely different here and identical in a plain FPS counter;
...
- **Styling and editability**: every widget exposes style properties (colours, radius from the token scale, padding, fonts, images, and states for hover, pressed and disabled), so the stock set can be restyled to fit a game without any custom authoring. Where full custom art matters most, TouchJoystick, TouchButton and Button additionally accept a **UserInterface asset as a visual override**, applied as a nested GUI subtree under the host widget.

### 11.3 Anchoring and layout

- **The document is Babylon GUI.** Each widget stores `horizontalAlignment` / `verticalAlignment`, `width` / `height` with `px` or `percent`, `left` / `top` offsets (always added, Y-down), layout `padding` (`Control.padding*`), and `transformCenter` (Babylon pivot in 0–1). Stretch between two arbitrary anchors is `left` percent + `width` percent, or 100% size plus layout padding.
- **3×3 + stretch presets are designer macros**, not a second solver. They write the fields above (Top Left → left/top align + px size; Stretch → 100% × 100% + padding) and do not store a parallel RectTransform.
- **Safe area is a padded `Container`** under the Canvas. Default-add parents into it. A Details flag (`ignoreSafeArea`) reparents to the full-bleed canvas.
- **A design resolution plus a scale rule** (fit width, fit height, or shortest side) maps only to ADT `idealWidth` / `idealHeight` / `useSmallestIdeal`. The designer shows a device-preset selector driven by the same rule the runtime uses.
- **Boxes, Grid, and ScrollBox own their children.** Apply parents with `parent.addControl` / `grid.addControl(child, row, col)`. Never flatten every control onto the ADT root.
- The designer edits **the same widget model the runtime consumes**. `ui-runtime` stays Babylon-free: it validates the GUI fields and builds a nested spec tree; `packages/render` copies those fields onto controls. A `previewRect` helper mirrors Babylon’s published alignment/%/padding formulas so jsdom hit-tests work without WebGL.

**P9 residual (not a new phase):** Play and the designer apply this Babylon-native document. Legacy RectTransform payloads migrate to v2 (`migrateUserInterfacePayload`). Accept still means one HUD across iPad landscape/portrait/desktop with safe area — via ADT ideal scale + SafeArea padding, not min/max anchors.
...
**Custom fonts silently fall back to Arial unless they are explicitly loaded**, and this is the single most common way Babylon text goes wrong. The browser does not fetch a font until something uses it, and Canvas2D draws whatever is available at that instant. So `packages/render` owns a font registry that registers each Font asset with `new FontFace(family, bytes)` plus `document.fonts.add`, awaits `document.fonts.load("Npx 'Family'")` before the first UI draw, and calls `markAsDirty()` on the HUD and designer ADT if a font resolves late. Play and the UserInterface designer both `registerAll` project Font `source` bytes on that path. A font that fails to load is a visible warning in the editor, never a silent substitution.
...
**Game cameras follow the same look.** Section 2.5 requires a detached `UniversalCamera` whose pose comes from the CameraComponent actor, looking along local +Z, with `projectionMode: orthographic` as the 2D default. The editor 2D camera is a separate pan/zoom controller and is not the game camera. `cameraBounds2D` draws the framed rect the game camera will use.
...
- **Navigation** becomes pan and zoom only: one finger pans the camera (tap still picks; hold ~250ms then move marquees), three fingers pan, pinch to zoom, no look/orbit. Orbiting an orthographic 2D scene is how you get lost.
...
- **Playwright projects**: `desktop-chrome` runs the full suite; `ipad-landscape` and `ipad-portrait` (`hasTouch`, device scale factor 2) grep `@ipad` so only touch, coarse-pointer, and orientation tests pay the iPad tax. Tag those cases with `IPAD_TEST_TAG`. Visual-regression screenshots remain a later add.
...
- **P4 Bridge, workers and Play**: both transports; game worker; snapshot protocol; renderer applying snapshots; fullscreen Play overlay with X; raw input capture into the tick-stamped ring buffer including gamepad polling; the log ring buffer, Output Log panel and an FPS readout; worker error reporter and the **Preview session report** that maps runtime stacks back to graph nodes via compiler source anchors. The **memory and thermal machinery from section 2.4 lands here rather than as a later optimisation pass**: visibility-gated editor rendering at the Engine Settings frame cap, hardware-scaling control, the LRU resource cache with self-computed byte accounting, and the shared-texture guarantee across editor and Play scenes — all four are cheap to build into a renderer being written from scratch and invasive to retrofit into one that assumes an uncapped loop and unbounded caches. *Accepts when* the same scenario is identical in-process, over SAB, and over transferables, Play holds 60fps on the baseline A16 iPad, **a hidden or modal-blocked editor viewport renders no frames while a visible one honors the Engine Settings frame cap, and a Play open-and-close cycle leaves `getLoadedTexturesCache()` no larger than it started**, and a deliberate `throw` in a scripted node opens the session report on close with a tap that focuses the offending node.
- **P5 Visual scripting**: IR, type system including both wildcard forms, **continuous graph validation** with Compiler Results and inline markers blocking Preview on errors, validator, compiler with source anchors, core node catalog, the four special nodes from section 6.1, touch graph UI, Class panel, interfaces, function libraries, enums, structs. *Accepts when* an actor scripted in the editor compiles and runs in the worker, a type mismatch is flagged before Preview and tap-to-navigate focuses the pin, an ExecuteJavaScript node round-trips values through the graph, and compiler golden tests cover all of it.
...
- **P9 Content systems**: UserInterface designer and runtime with Babylon GUI alignment / px/% / padding, the widget library including TouchJoystick, TouchButton and TouchDPad wired to the input mappings from P6, the font registry with fallback chains and the project default font, the Sprite asset and editor with atlas frames, pivots and named clips plus `SpriteComponent` as a quad mesh per 13.2, AnimationGraph driving both 3D animation and sprite clips, Shader graph. *Accepts when* a UI authored once lays out correctly across iPad landscape, iPad portrait and a desktop aspect ratio with the safe area respected (ADT ideal scale + SafeArea padding), an on-screen joystick drives the same axis a gamepad stick does with no change to the game's script, and a custom font renders on the very first frame on a cold load with a fallback face visibly covering characters it lacks.
...
- **Lighting and cameras (`p-lighting-camera`)**: full contract in section 2.5 — rotation-driven lights, Play color/intensity/range/cone, one `ShadowGenerator` from `shadowquality`, detached `UniversalCamera` game cameras selected by a **Default Camera** CameraComponent pick (`SceneComponentPicker`), **Possess Camera** node for the global Play camera, fog/IBL/clear color, editor dir/spot gizmos, small camera/light node catalog. Named slice, not a new P-number; may run beside P11. *Accepts when* a directional light aims from actor rotation, Play matches editor color and intensity, one shadow map follows `shadowquality`, Play uses the named Default Camera as a detached UniversalCamera, Possess Camera switches that global camera from a Class graph, and 2D ortho still looks +Z.
...
- **P12 Editor extensions**: EditorUtilityObject and EditorUtilityInterface, Project Settings registration for objects, **Windows → Editor Utilities** as the way to open interface tabs with dockable persistence, stripped from exports.
...
- **iPad memory and thermals**: the full set of mitigations lives in section 2.4 — visibility-gated editor rendering, hardware scaling, the LRU byte-ceilinged resource cache, engine-level texture sharing between the editor and Play scenes, pausing everything invisible — plus compressed textures and the resolution clamp from 3.5, WebGL2 by default with WebGPU opt-in, and texture budget warnings in the asset registry.
- **A visible editor that never freezes** is the thermal failure mode now that Always Render is the default for on-screen viewports. Mitigations are in 2.4: IntersectionObserver plus overlay detection skip `scene.render()` when the canvas is hidden or a modal is open, Play and `visibilitychange` still pause the loop, and the DEV Always Render checkbox can be turned off to recover dirty-driven idle for bisecting missed invalidation. The HUD graphs rendered-fps next to invalidations per second so a stuck Always Render is as visible as a missing invalidation after opt-out.
...
- [x] **p0-shell-ux** — P0: Shell UX baseline - radius token scale with rounded panels, dock groups, sheets and tabs; Dockview theme with 18px fine / 26px coarse tab strips, 44px grips, widened sash hit areas and enlarged drop overlays; `--chrome-row` 28px chrome; global user-select none plus -webkit-touch-callout none with a SelectableText component as the only opt-in; Playwright audits for radius-free hardcoding, selection defaults and dock targets
...
- [x] **p1-homepage** — Homepage route with project browser, Documents/OPFS default + external opt-in, Create Empty plus template cards discovered from the Engine Settings templates folder (Empty-only on web), Engine Settings, Save All / Close with dirty check; journal recovery stub (full replay in P2)
...
- [x] **p2-content-browser** — P2: Content Browser rework: TreeView of folders+assets with New Folder, hold-to-drag move, folder tiles before assets (direct children including root), empty-grid create menu, fixed-size Card tiles, click-select / double-open, type filter chips, search, Import via document picker, New Asset with parent-class picker, TreeView move/copy picker from context-menu **Move…** / **Copy to Folder…**, DataTypes pin/asset colors
...
- [x] **p4-render-on-demand** — P4: visibility-gated editor loop (section 2.4) - visible scene and Prefab Preview canvases render at Engine Settings `viewportFrameCap`; freeze when the canvas is not intersecting, a modal/sheet overlay is open, Play is up, or the app is backgrounded; dirty-driven invalidation and refcounted continuous-render leases remain the Always Render-off path; DEV Always Render toggle defaults on; rendered-fps versus invalidations-per-second in the HUD; Engine constructed with adaptToDeviceRatio false and resolution owned via setHardwareScalingLevel; dynamic resolution valve with hysteresis and cooldown wired to the renderquality tier; MSAA off on the iPad baseline; skipPointerMovePicking true in every scene since touch has no hover; visibilitychange and Capacitor app-state pausing render loop, game worker tick and encode queue
...
- [x] **p5-graph-validation** — P5: graph validation surfaced in-editor - debounced edit-time pass, save and pre-Preview project sweep, Compiler Results panel with tap-to-navigate, inline node and pin error markers, Content Browser compile-error overlay, Play button error badge, Compilation Error status next to Compile on graph documents; errors raise a blocking dialog with tap-to-navigate and a Play Anyway button rather than refusing; export-preset-specific rules deferred to the export gate; same pure validator runs headless in CI over golden fixture projects
...
- [x] **p5-node-catalog** — P5: scripting-nodes catalog by category (flow, math, vector, string, array and map, actor, component, transform, physics, input, audio, UI, scene, render, debug, interface, variables, casting, timers), one test file per category
...
- [x] **p6-editor-kit** — P6: editor-kit reusable components (property grid, virtualized touch tree view, asset picker, search sheet, numeric drag field, long-press menu, toolbar strip, panel frame, catalog dialog) plus a dev-only Component Gallery route for on-device checks. *Accepts when* every docked panel wraps in `PanelFrame`, scene Outliner/Details/Place Actors use editor-kit composites (not one-off CSS), `/?test=1&gallery=1` renders all `@babylonslate/ui` primitives + editor-kit composites, and Playwright touch-target audit covers every panel — not only chrome tabs.
- [x] **p6-scene-editing** — P6: Scene document panels: Viewport with touch gizmos and two-finger camera, Scene Outliner with Place Actors catalog, Details for scene settings and selection (inline XYZ / Euler rotation); Actor prefab tab and component editing; every edit routed through the P2 command layer; viewport camera controller and gizmo system built around a 2D/3D mode from the first commit
...
- [x] **p9-ui-system** — P9: ui-runtime UserInterface widget model, Babylon GUI runtime, UMG-style designer with design and logic tabs, viewport-layer and 2D-prefab placement, self-containment cycle check
- [x] **p9-ui-anchoring** — P9: Anchoring and layout - Babylon GUI alignment, px/% size, left/top, layout padding, transform center; 3×3 presets as macros; SafeArea container; design resolution with a fit-width, fit-height or shortest-side scale rule mapped to ADT ideal; device-preset previews in the designer. Residual: document/runtime/host are the same Babylon model (not Unity RectTransform compiled at apply).
- [x] **p9-fonts** — P9: Font asset type and font registry - original woff2, woff, ttf or otf bytes as the chunk the GUI path needs, optional facetype JSON and msdf atlas chunks imported pre-generated for extruded and MSDF text, registration via new FontFace from bytes plus document.fonts.load awaited before the first draw with markAsDirty on late resolution, per-asset fallback chains compiled to quoted CSS font stacks terminating in a generic family, a project-level default font and global fallback chain in Project Settings, and a sample-text preview that flags characters falling through to the fallback; text measurement stays on the main thread beside the renderer
- [x] **p9-widget-library** — P9: Widget library - containers (Canvas, HorizontalBox, VerticalBox, Grid, ScrollBox, Overlay, SizeBox, Border), controls (Button, Text, TextInput, Slider, CheckBox, Image, ProgressBar, Spacer), and touch controls (TouchJoystick with fixed or floating origin, dead zone and auto-hide, TouchButton, TouchDPad) registered as input sources so an on-screen joystick and a gamepad stick drive the same axis; full style properties on every widget plus an optional UserInterface asset as a visual override for joystick and button
- [x] **p9-sprite** — P9: Sprite asset and editor with atlas frames, pivot, per-frame durations and named clips, plus SpriteComponent as a quad mesh with the atlas frame baked into its UV buffer rather than a Babylon Sprite; one mesh per sprite by default (thin instances / merged static batching are measured escape hatches, not v1); alpha-test by default with blending opt-in, nearest sampling and no mipmaps for pixel art; deterministic rectangle packer as a pure function for packing loose frames at import, with padding and edge extrusion
- [x] **p9-anim-graph** — P9: AnimationGraph state machine and deterministic evaluator in the game worker driving both glTF Animation clips and sprite named clips; snapshot/command fields for state, normalised time and blend weights; render seeks AnimationGroup / sprite UVs and never lets Babylon auto-advance gameplay animation
- [x] **p9-shader-graph** — P9: Shader graph IR (curated NodeMaterial-block catalog plus CustomBlock) compiled in render; throttled live preview; forceCompilationAsync at load; post-process materials off by default and flagged as iPad cost

### P10

- [x] **p10-tilemap** — P10: Tileset and Tilemap assets - tileset with tile size, margin, spacing and per-tile collision, animation and flags; tilemap with ordered layers of chunked tile data, per-layer sorting and parallax, and collision opt-in; rendering as chunked VertexData generated by a pure golden-tested function giving one draw call per chunk per atlas with only affected chunks rebuilt on edit; merged chain colliders generated per tilemap chunk on the P7 Rapier backend rather than a box per tile; autotile and terrain rules deliberately deferred
- [x] **p10-tilemap-painting** — P10: touch-first tilemap painting - brush, eraser, rectangle fill, bucket, stamp and tile picker, tile palette as a searchable dropdown, one finger painting while two fingers pan, and one undo entry per stroke rather than per tile using the merge key from packages/edit; plus a 2D project template on the Create Project cards

### Lighting and cameras

Named slice, not a P-number. Spec: section 2.5. May run beside P11 (different packages).

- [ ] **p-lighting-camera** — Lighting and cameras: `LightComponent` kinds point/directional/spot with typed color, intensity, enabled, range, spot inner/outer angles, and `castShadows`; direction from actor rotation along Babylon forward `(0,0,1)`; incremental `authoredLight:<actorId>` / `authoredCamera:<actorId>` sync; Play applies the same properties (not white lights); one `ShadowGenerator` on the first `castShadows` light sized from `shadowquality` (`off` / `512` / `1024` / `2048` warn); optional `environmentTextureGuid` IBL via ResourceCache cube; linear fog color/start/end; `environmentColor` as Play and 3D-editor clear; game cameras as detached `UniversalCamera` with explicit `projectionMode`, FOV, orthographic size, near/far — never ArcRotate; **Default Camera** is a CameraComponent pick via kit `SceneComponentPicker` (`allowedClassIds={["CameraComponent"]}`) stored as actor+component ids (missing keeps the default Play camera — not first in the list); editor `stealActiveCamera: false` plus optional Game Camera preview; 2D ortho look +Z per section 13; dir/spot gizmos; Details enums and scene-settings rows; **Possess Camera** node (Class graphs, exec in/out) switches the global Play `activeCamera`; get/set FOV, ortho size, light enabled/color/intensity. *Accepts when* a directional light aims from actor rotation, Play matches editor color and intensity, one shadow map follows `shadowquality`, Play uses the named Default Camera as a detached UniversalCamera, Possess Camera changes that global camera, and 2D ortho still looks +Z

### P11

- [x] **p11-behaviour-tree** — P11: packages/behaviour-tree - tree model with composites (Selector, Sequence, Parallel), tasks, attached decorators and services; Blackboard asset with keys typed by the existing pin type system; deterministic evaluator with an explicit execution stack so trees can be stepped and inspected; abort modes (Self, Lower Priority, Both) covered by a table-driven matrix over tree shapes rather than incidental tests
- [x] **p11-bt-authoring** — P11: BTTask, BTDecorator, BTService and BTComposite as BObject subclasses users inherit and implement with visual scripting (OnActivate, OnTick, OnAbort for tasks; OnEvaluate for decorators; interval ticking for services), the built-in node set (Wait, MoveTo, RotateToFace, PlayAnimation, SetBlackboardValue, Loop, Cooldown, TimeLimit, blackboard conditions), and BehaviourTreeComponent attachable to any Actor with a tree and blackboard reference
- [x] **p11-bt-editor** — P11: behaviour tree editor as a React Flow graph in graph-ui - parent-child edges with sortIndex for sibling priority, custom nodeTypes for composites and tasks with decorators and services as attached selectable rows rather than separate nodes, d3-hierarchy top-down auto-layout with computed positions and no free drag, sibling reorder snapping to an index, shared Add Node catalog modal and long-press context menu; live debug overlay during Play with running-branch highlight and blackboard watch, recorded into the P8 trace; runtime throws navigate via the P4 session report btNodeId
- [x] **p11-navigation** — P11: packages/navigation - generateSoloNavMesh + exportNavMesh / importNavMesh round-trip with no Babylon; NavigationBackend (findPath, closestPoint, randomPointInRadius, obstacle ids, addAgent / setAgentTarget / stepCrowd); 2D XY↔Recast XZ remap with property tests; facing yaw from Recast XZ velocity with a min-length guard; Scene `navmesh` extra-chunk helpers. Bake modal, NavMesh Place Actor, `@recast-navigation/babylon` debug draw, and runtime crowd load stay `p11-nav-editor-host` / `p11-nav-blockers-2d`
- [ ] **p11-nav-editor-host** — P11: NavMesh Place Actor + Recast settings Details, blocking bake modal (paint one frame, collect geometry on main thread, generate in a bake worker, write Scene `navmesh` chunk), `@recast-navigation/babylon` debug draw, runtime import of the chunk + crowd `addAgent` / `stepCrowd`, ungate `NavAgentComponent`
...
- [ ] **p12-editor-extensions** — P12: EditorUtilityObject with editor lifecycle events registered in Project Settings, and EditorUtilityInterface widgets registered into **Windows → Editor Utilities** (not auto-inserted into the Scene default layout) as dockable tabs with persisted position, both stripped from game exports
...
- [ ] **p14-export** — P14: exporter package producing an itch.io-ready zip (index.html at root, packed assets, compiled scripts, coi-serviceworker) with an E2E test that unzips, serves, boots and ticks the exported game; bundle-debugger and strip-Print export settings; boot and tree-shake from `project.json` `startupSceneGuid` (asset guid), not `BabprojectManifest.startupScene` path and not `assets/main.scene.babasset`; apply `render.customResolution` (WxH + black bars) as the packaged framebuffer
...
- [ ] **p14-platforms** — P14: apps/player standalone runtime host (boots `startupSceneGuid`, applies project render size), apps/desktop Electron shell for Windows and macOS with a Node VFS adapter, Capacitor iOS polish and on-device verification
- [ ] **p14-perf-smoke** — P14: CI perf smoke against the A16 iPad budget from section 1.2 - fixed scene through the deterministic harness asserting the combined script-plus-physics tick stays under 8ms with the two halves asserted separately, accounted texture and geometry bytes asserted against a fixed scene so byte-accounting drift fails a build, an obstructed-editor assertion that zero frames render when the viewport is hidden or a modal is open, plus draw-call ceilings surfaced as registry warnings, so perf regressions fail a build instead of being noticed phases later
