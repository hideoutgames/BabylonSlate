...
**Content Browser** (pinned, non-closable): folder tree with **New Folder** on the tree (git-visible marker file; selected row uses catalog ink bar + secondary fill); one toolbar row (Import, New Asset, shared **SearchInput** with a clear control, **Filter** dropdown with multi-select type checkboxes — empty selection = all types — **Deselect All** and Delete when selected — no page heading). Root folder `assets` lists the whole tree; any other folder shows **direct children only**. Fixed-size shadcn Card tiles (`repeat(auto-fill, 9rem)`, square thumb with a compact type glyph; cards mix `--asset-*` into `--card` / `--muted` for darker type accents). Titles strip a trailing `.{Scene|Graph|…}` (type stays in `CardDescription`). Click/tap **adds to selection** (does not replace), **Deselect All** or tap/click **empty grid** (not a tile) **clears selection**, double-click / double-tap **opens** the matching document tab (`openOrFocusDocument`; no tab-bar Add control). The folder tree and asset grid are native vertical scrollports (`min-h-0 overflow-y-auto`; overflow is not on the CSS grid). Logic graphs live on **Class** `.class.babasset` (legacy Graph files still load). **New Asset** creates authored types only (Scene, Class, UserInterface, Sprite, AnimationGraph, Shader, Enum, Structure, ScriptInterface); Texture/Material/Model/Audio/Font/Animation come from Import. Import-only and type assets open compact **Settings** tabs. **New Asset / rename / new folder** refuse an existing name (no overwrite). **Duplicate** allocates `stem_N` in the destination folder (strip one trailing `_digits`, then the next free index; keep the unsuffixed stem when it is unused). Toolbar **Import** on web/electron clicks the in-DOM file input (same path as Playwright); iOS/Android use `pickImportFiles`. A blocking **Importing** overlay shows `n / m` during the write pass only; KTX2 encode stays a background queue with CB badges. Hold still ≥500ms or right-click opens the context menu on tiles and nested folders (the `assets` root has no menu). Early pointer movement scrolls. **Move…** opens `ContentBrowserMoveDialog` (`Dialog` + `SearchInput` + `TreeView` of `folderTree("project")`, illegal destinations muted); confirm calls `moveAsset` / `moveFolder`. Rename, duplicate, copy, and delete; Show References from the dependency graph.
...
- [x] **p2-content-browser** — P2: Content Browser rework: folder tree with New Folder, fixed-size Card tiles, click-select / double-open for Scene/Graph, type filter chips, search, Import via document picker, New Asset with parent-class picker, TreeView move picker from context-menu **Move…** (assets and folders), long-press multi-select
