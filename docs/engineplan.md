...
- *Prefab* (Actor): **full-size center-group tab**, sibling of Graph (`direction: "within"`). Same touch gizmos / two-finger camera as the Scene viewport. Not a fourth chrome `DocumentKind`. Persistence is preview-only. **Focus** hides Prefab and Compiler Results so Graph + Inspector + the left stack remain.
