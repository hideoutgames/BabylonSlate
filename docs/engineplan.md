...
**Windows** (labeled dropdown immediately left of Focus, disabled on Content Browser): lists every built-in dock tab for the active Scene or Graph document with a check mark when that panel is open. Toggling closes or reopens the panel; the menu stays open so check marks update in place. Reopening uses the last dock placement stored in `layout.json` (`ProjectLayouts.panelPlacements`) when the reference panel still exists, otherwise the catalog default. The last remaining panel cannot be closed. A nested **Editor Utilities** submenu is the primary way to open EditorUtilityInterface tabs (empty until P12 registers widgets). Closing a dock tab with the Dockview × control still records placement so Windows can restore it. Focus does not write those placements (it already snapshots layout in memory).
