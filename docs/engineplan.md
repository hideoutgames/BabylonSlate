...
**Keep and build on:**
- The two-tier tab model: a **two-row editor chrome** in [apps/editor/src/components/editor-chrome-bar.tsx](apps/editor/src/components/editor-chrome-bar.tsx) — a title/tab bar (truncated project name with a trailing `.babproject` stripped for display, plus global document tabs with a pinned Content Browser and **no tab-bar Add**) and a **global toolbar** below it (Undo, Redo, Save, Save All, Play, Focus, Search, Settings) — plus a per-document Dockview workspace in [apps/editor/src/shell/dockview-shell.tsx](apps/editor/src/shell/dockview-shell.tsx). This is exactly the Unreal-style shell the engine needs.
...
- **Canonical shadcn theme:** Unreal-inspired layered charcoal + blue selection accent in [`packages/ui/src/styles/globals.css`](../../packages/ui/src/styles/globals.css), with pin/node/graph tokens documented in [architecture/theming.md](architecture/theming.md). Geist remains the UI font; `--primary` is the selection accent, not ink. Dark `--background` stays above complete black (`oklch` lightness ≥ 0.20).
