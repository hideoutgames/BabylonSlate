...
- **Packages (P0 names).** `core` (was shared), `vfs` (was storage), `render` (was engine), `graph-ui` (was graph), plus `assets`, `edit`, `object-model`, `scripting`, `scripting-nodes`, `input`, `runtime`, `physics`, `bridge`, `debugger`, `ui-runtime`, `anim-graph`, `shader-graph`, `ui`, `editor-kit`, `test-kit`. Layout: [architecture/overview.md](architecture/overview.md). Planned but absent: `behaviour-tree`, `navigation`, `exporter`, `source-control`, `apps/player`, `apps/desktop`, `engine-plugins/`.
...
- *Class*: **My Blueprint** member tree (Functions, Variables, Events, Interfaces — **no Graphs section**) stacked **below** Components (`direction: "below"`). Initial height is about **50%** of the left stack. Toolbar Add (kind menu + `NamePromptDialog`) and Remove; type-colored rows. Writes `SerializedGraph.members` (variable `typeId` / default, function `pins[]`, interface ScriptInterface `assetGuid`). Events insert `flow.event.custom`; variables do not spawn Get nodes. Selection drives Inspector Details. Class-owned per-function graph documents remain a later follow-up.
