...
- [ ] **p9-sprite** — P9: Sprite asset and editor with atlas frames, pivot, per-frame durations and named clips, plus SpriteComponent as a quad mesh with the atlas frame baked into its UV buffer rather than a Babylon Sprite; one mesh per sprite by default (thin instances / merged static batching are measured escape hatches, not v1); alpha-test by default with blending opt-in, nearest sampling and no mipmaps for pixel art; deterministic rectangle packer as a pure function for packing loose frames at import, with padding and edge extrusion
- [ ] **p9-anim-graph** — P9: AnimationGraph state machine and deterministic evaluator in the game worker driving both glTF Animation clips and sprite named clips; snapshot/command fields for state, normalised time and blend weights; render seeks AnimationGroup / sprite UVs and never lets Babylon auto-advance gameplay animation
- [ ] **p9-shader-graph** — P9: Shader graph IR (curated NodeMaterial-block catalog plus CustomBlock) compiled in render; throttled live preview; forceCompilationAsync at load; post-process materials off by default and flagged as iPad cost
