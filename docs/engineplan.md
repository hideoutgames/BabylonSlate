...
- **Engine Settings** opens from the Homepage as its own catalog modal, and from the editor via a Settings dropdown that lists **Project Settings** and **Engine Settings** as separate items. The two are never nested in one sidebar. See 7.2. Catalog search is **not** autofocused on open (iPad keyboard would cover the modal); Add Node keeps search focused.
- **Save and Close**: the editor **global toolbar** has icon-only Save, Save All, Undo, and Redo (44px `--touch-target`, `aria-label` plus tooltip). **Close Project** lives in Project Settings (Close category), not the toolbar, so it stays reachable without colliding with the iPad sash. Close flushes pending writes, releases the folder handle and returns to the Homepage. A dirty-document check runs first, listing exactly which documents are unsaved rather than asking a vague question.
...
- *Scene Outliner*: actor hierarchy with search, drag-to-reparent, visibility and lock icon toggles, long-press context menu. Dockview tab is the title — no second PanelFrame heading.
- *Mini Asset Browser*: compact palette with type filter `ToggleGroup`; drag an asset into the viewport to spawn an actor or assign a material.
...
- *Graph*: event graph and per-function graphs, with a centered **Add Node** catalog modal (categories + search; search **is** autofocused).
- *Prefab* (Actor): **full-size center-group tab**, sibling of Graph (`direction: "within"`). Same touch gizmos / fly-look camera as the Scene viewport. Not a fourth chrome `DocumentKind`. Persistence is preview-only.
- *My Class*: variables (type, default, category, expose-on-spawn, instance-editable), functions with local variables and input/output pins, event dispatchers, implemented interfaces. Inherited members are shown but marked.
- *Components* (Actor): component tree in the left dock (sibling of My Class). Add/remove is session-local until prefab persistence lands.
...
- **Layout**: safe-area insets, no hover-only affordances, bottom sheets instead of nested menus, numeric fields that scrub by dragging, and virtual-keyboard avoidance for text entry. Settings catalog search does **not** autofocus on open (iPad keyboard would cover the modal); Add Node and Global Search may focus search because they *are* search UIs.
