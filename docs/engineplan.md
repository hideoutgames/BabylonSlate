...
**Scene document:**
- *Viewport*: Babylon view with an overlaid Move/Rotate/Scale `ToggleGroup`, snap toggle, 2D/3D mode, and an **editor joystick** toggle (persists `scene.settings.editorJoystickEnabled`). Transform gizmos are stock Babylon classes restyled in place: unlit/emissive axis colors matching chrome `--axis-x/y/z`, thinner shafts and rotation tori, planar translate squares, hover brighten. Scene and Prefab canvases always use dark `clearColor` (`EDITOR_CANVAS_COLOR_SCHEME`) regardless of Appearance → Theme. Play lives on the **global chrome toolbar** only (not duplicated on the viewport).
