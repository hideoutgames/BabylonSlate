...
**Next engine slice:** P10 tilemaps (codecs + chain colliders landed; painting and the 2D template are next). Parked: `p1-device-spikes` (hardware-only). Do not start P11+ packages from leftover editor polish.
...
| Tileset / Tilemap documents | Codecs, UV math, golden chunk `VertexData`, Content Browser types, settings tabs, Rapier/software chain colliders, Play load from scene refs | P10 (painting next) |
...
- **Create Project modal**: **Create Project** opens a dialog. **Name** is required (default `MyGame`; `/?test=1` / `VITE_TEST_MODE` prefills `TestProject`; `.babproject` is appended on submit, not typed in the field). **Template** is selectable **Empty** (default) plus one card per discovered template (web still Empty-only). **Location**: web is read-only “This browser (OPFS)”; native defaults to **App Documents** with no picker. Optional **Choose folder…** picks then scaffolds. Creating from a template copies the `.babproject` into the new location and rewrites only its name and identity.
