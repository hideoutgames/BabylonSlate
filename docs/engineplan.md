...
**Keep and build on:**
- The two-tier tab model: global document tabs in [apps/editor/src/components/editor-chrome-bar.tsx](apps/editor/src/components/editor-chrome-bar.tsx) with a pinned Content Browser, and a per-document Dockview workspace in [apps/editor/src/shell/dockview-shell.tsx](apps/editor/src/shell/dockview-shell.tsx). This is exactly the Unreal-style shell the engine needs.
...
- `render` must not import React. `runtime` must not import Babylon or the DOM, which the physics wasm in 2.1 and the navigation wasm both satisfy.
...
- **We forgo Babylon's physics integration layer entirely.** `HavokPlugin`, `PhysicsAggregate` and `scene.enablePhysics` all assume physics lives beside the `Scene` on the main thread, and there is no `Scene` in the game worker. Driving the raw Havok wasm API means owning shape creation, raycasts, triggers and character control ourselves. That is the right call for this architecture, but it is real scope in P7 rather than a configuration choice. Bodies never cross a thread boundary: they are created and stepped in the same worker that runs the scripts mutating them, and only their resulting transforms reach the main thread, in the shared snapshot buffer along with everything else.
...
- **Save and Close**: the editor gets Save, Save All, and Close Project, which flushes pending writes, releases the folder handle and returns to the Homepage. A dirty-document check runs first, listing exactly which documents are unsaved rather than asking a vague question.
...
- Undo and redo are **visible buttons in the chrome bar**, acting on the active document and disabled when empty. Keyboard shortcuts still bind for desktop, but a tablet user has no shortcut reflex, so the buttons are the primary affordance.
...
**Scene document:**
- *Viewport*: Babylon view, move/rotate/scale gizmos with touch-sized screen-space handles, selection outline, grid and snap toggles, camera speed, view mode (lit, unlit, wireframe), Play button.
...
- *Graph*: event graph and per-function graphs, with a node-palette bottom sheet.
...
**Content Browser** (pinned, non-closable): folder tree, asset grid with thumbnails, type filter chips, search, sort; Import via document picker and New Asset via type picker then parent-class picker; long-press multi-select; move, rename and delete; Show References from the dependency graph; drag source for viewport, graph and details drops.

**Other windows:** Output Log; Play overlay (fullscreen, X top-right) with the debug console bottom sheet, stats HUD layered over it, and a **Preview session report** bottom sheet on close when runtime errors occurred (section 9.7); Debug Trace playback tab; Sprite Editor (frame timeline, pivot, texture picker); UI Designer (canvas, widget hierarchy, details, logic graph tab); AnimationGraph editor; Shader graph with live preview; Enum and Structure row editors; ScriptInterface signature editor; Project Settings sheet (General, Maps and Modes, Editor, Rendering, Physics, Input, Plugins, Source Control, Export with both Export Project and Export Game); Engine Settings sheet, reachable from both the Homepage and the editor; a Locks panel when source control is enabled; plus a dev-only Component Gallery route for on-device visual checks of `editor-kit`.
...
- **The node palette** is the same bottom sheet, filtered to child types legal for the current selection. Tap a composite, pick Sequence, and the child appears connected in the next slot — no wire dragging required, which matters on a tablet.
...
- [ ] **p0-foundation** — P0: Upgrade deps (dockview-react 8, Babylon 9, Capacitor 8, React Flow 12.11, Vitest 4, Playwright 1.62), rename packages (shared to core, storage to vfs, engine to render, graph to graph-ui), delete dead template files and the project-context shim
- [ ] **p0-testing-infra** — P0: Vitest 4 workspace with node, jsdom and babylon projects, v8 coverage thresholds in CI, golden-file helpers, fast-check, import-boundary ESLint rules, touch-target Playwright audit, iPad Playwright projects
- [ ] **p0-docs** — P0: Write docs/architecture/*, docs/CODING_STANDARDS.md, docs/agents/issue-tracker.md (required by the code-review skill and currently missing), docs/design/gestures.md, docs/design/perf-budget.md covering the A16 iPad baseline and the section 2.4 rules agents must follow (no per-frame allocation in the render sync path, no Texture construction outside the resource cache, every scene mutation marks the viewport dirty, no square corners, no hardcoded radii), and the agent operating model (one slice one PR one owner, API before implementation, design notes for shared surfaces); update .cursor/rules for the new package boundaries
- [ ] **p0-shell-ux** — P0: Shell UX baseline - radius token scale with rounded panels, dock groups, sheets and tabs; Dockview theme with 52px coarse-pointer tab strips, 44px grips, widened sash hit areas and enlarged drop overlays; global user-select none plus -webkit-touch-callout none with a SelectableText component as the only opt-in; Playwright audits for radius-free hardcoding, selection defaults and enlarged dock targets
- [ ] **p0-context-menu** — P0: useContextMenu hook in editor-kit - 500ms stationary long-press matching Dockview timing, contextmenu event as the secondary mouse path, native menu suppressed app-wide, cancels on scroll or drag arming; unit tests plus touch and mouse e2e coverage

### P1

- [ ] **p1-vfs** — P1: Binary-capable VFS port; OPFS web adapter replacing the localStorage adapter; node and memory adapters; the default iPad tier writing to the app own Documents directory with no picker or bookmark involved. For the opt-in external-folder tier, verify the Capacitor scoped-storage adapter on Capacitor 8 and spike sustained coordinated I/O into a real Working Copy repository through its file provider - NSFileCoordinator reads and writes, security scope held for the process lifetime, bookmark staleness surfaced as a Reconnect project folder flow - and expect to write our own Swift plugin, since this determines whether the section 12 collaboration story is possible at all
- [ ] **p1-babasset** — P1: .babasset binary codec with sorted-key stable headers, chunk table with sha256, a first-class header-only read that allocates no chunk payload (the property the registry invariant in 2.4 depends on), thin and bundled modes, and chunk locators so large chunks externalise to an immutable content-addressed blob store at assets/.blobs/<sha256> inside a project while staying inline whenever the asset leaves it; derived data (compiled scripts, thumbnails, import cache, journal) kept in app-private storage outside the project folder; golden and property-based round-trip tests
- [ ] **p1-babproject** — P1: .babproject codec with directory and zip backends behind one API, parameterised by manifest kind so .babplugin reuses it unchanged in P13, plus Export Project in Project Settings on all platforms
- [ ] **p1-schema-migration** — P1: Asset schema versioning - per-type ordered migration chains applied on load, refusal with a clear message for future versions, committed golden fixtures per historical version, and a migrate-on-save prompt that never silently rewrites untouched files
- [ ] **p1-app-settings** — P1: App settings store outside any project (Capacitor Preferences on iPad, OPFS or localStorage on web, Electron userData on desktop) holding Engine Settings: templates folder, default project location, recents with persisted iOS security-scoped bookmarks, appearance, undo history length defaulting to 50, viewport frame cap, thumbnail toggle, debugger defaults
- [ ] **p1-homepage** — P1: Homepage route with a project browser and two storage tiers - default to the app own Documents directory made visible in Files via UIFileSharingEnabled and LSSupportsOpeningDocumentsInPlace so the common iPad case needs no picker, bookmark or file provider, with any external folder (iCloud Drive, Working Copy repo) as an explicit opt-in, and OPFS on web; Create Project modal offering Empty plus one card per .babproject found in the nominated templates folder, Engine Settings sheet, and Save, Save All and Close Project with a dirty-document check; journal-based crash recovery on reopen

### P2

- [ ] **p2-registry** — P2: Asset registry (guid index, dependency graph, Show References) indexing headers and chunk tables only and never payloads, with payload chunks loaded on demand through the resource cache and a test asserting a several-hundred-asset synthetic project opens with near-zero accounted payload bytes (section 2.4); thumbnail generation at import into derived data as small compressed images, decoded lazily for visible virtualised grid cells with their own LRU budget separate from the scene resource cache; import pipeline with per-format importers as pure functions covering images, glb/gltf/obj/stl, audio and fonts (woff2, woff, ttf, otf, with facetype JSON and msdf atlases attaching to an existing Font asset rather than creating one); registry is content-root-aware from the first commit with a second synthetic root exercised in tests, so plugin roots are not a retrofit
...
- [ ] **p2-content-browser** — P2: Content Browser rework: folder tree, asset grid, type filter chips, search, Import via document picker, New Asset with parent-class picker, long-press multi-select, drag sources
- [ ] **p2-edit-undo** — P2: edit package - per-document undo stacks (never global), reversible command objects with apply, invert and a merge key so gesture drags coalesce, history capped by both the Engine Settings entry limit and a byte budget since fifty subtree-snapshot fallbacks is a real number on a 6GB device (section 2.4), snapshot fallbacks recording captured bytes so expensive command types surface in profiling, delta-based commands with subtree snapshots only as a fallback, chrome-bar undo and redo buttons as the primary touch affordance, and apply-then-invert property tests per command type
- [ ] **p2-destructive-guard** — P2: Destructive-action confirmation for asset, folder and plugin deletion naming exactly what is removed and listing inbound references from the dependency graph, since asset-file create and delete are the only operations deliberately outside undo (in-document edits including scene actor add and remove are undoable); plus the journal append in app-private derived data and the recovery path reusing the same command stream
...
- [ ] **p4-bridge** — P4: bridge package with SAB seq-lock and transferable ping-pong transports, snapshot buffer layout, reliable control and command channels, typed RPC; coi-serviceworker wired into the Pages build
- [ ] **p4-runtime-worker** — P4: runtime package as game worker, also runnable in-process, with scene instantiation, fixed-step loop with catch-up cap, and input ring buffer; the tick scheduler reserves a physics phase slot from the start since P7 hosts physics in this same worker rather than a separate one, and reports script and physics time separately because they share one budget
- [ ] **p4-input-capture** — P4: Raw input capture into a tick-stamped ring buffer - pointer and touch on the game canvas with touch-action none, keyboard, mouse, and Gamepad API polling once per frame since axes have no events; synthetic streams replayable through the deterministic harness
- [ ] **p4-render-sync** — P4: render package rework: replace the demo rotation and box-only loader with snapshot-driven scene sync, resource cache, interpolation and picking; scratch Vector3/Quaternion/Matrix reuse with no per-actor per-frame allocation in the sync path; bulk apply and bulk despawn wrapped in blockMaterialDirtyMechanism and blockfreeActiveMeshesAndRenderingGroups
