...
- [x] **p11-navigation** — P11: packages/navigation - generateSoloNavMesh + exportNavMesh / importNavMesh round-trip with no Babylon; NavigationBackend (findPath, closestPoint, randomPointInRadius, obstacle ids, stepCrowd); 2D XY↔Recast XZ remap with property tests; facing yaw from Recast XZ velocity with a min-length guard; Scene `navmesh` extra-chunk helpers. Bake modal, NavMesh Place Actor, `@recast-navigation/babylon` debug draw, and runtime crowd load stay the nav editor host / `p11-nav-blockers-2d`
