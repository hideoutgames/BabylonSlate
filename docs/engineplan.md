...
**P2 remaining (honest follow-ups, not blockers for the structural slice):** real Basis/UASTC wasm import Worker (stub encoder ships today); vendor `public/ktx2/*` decoder binaries; A16 wall-clock encode benchmark CI gate; thumbnail generate-at-import + virtualised CB decode; Capacitor document picker on iPad (web uses `<input type="file">`); GLB parse beyond stub dependents; scene edits on the command layer (graphs only today); plugin-deletion confirm (plugin packing is P13); killed-tab recovery Playwright; export transcoder-omitted smoke.
