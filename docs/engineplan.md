...
| Lights / cameras | Foundation wave: `LightComponent` / `CameraComponent` create authored Babylon lights/cameras (editor orbit stays; Play uses the first authored camera). Stub gaps: direction `(0,-1,0)`, white Play lights, `ArcRotateCamera` game cameras, no `ShadowGenerator` / IBL. Full contract is section 2.5 | Foundation wave done; implement as `p-lighting-camera` |
...
- **Project Settings** are per project and live in `project.json`: startup scene, GameInstance class, registered EditorUtilityObjects, input mappings, rendering (including the **Play/Preview frame cap, default 60**, **Play Preview Follow System (default on)** with a stored aspect defaulting to **16:9** that letterboxes the overlay with black bars when Follow System is off — Preview-only, not the exported player — and the section 3.5 texture policy — maximum import dimension defaulting to 2048, default compression mode, and the per-usage policy defaults that keep pixel art uncompressed), physics, plugin enable overrides, source control repository and branch, export presets, **`compileOnSave` (default true)** and **`autoSaveIntervalMs` (default 120000)**. The source control *token* is the exception and lives in the platform secret store, never in the file. The General category exposes Compile on save and Auto-save interval (seconds). The Rendering category exposes Play frame cap, Follow System, and Aspect Ratio.
