...
- [x] **p1-vfs** — Binary-capable VFS port; OPFS web adapter; node and memory adapters; durable iPad Documents tier via `@capacitor/filesystem` (no picker). Opt-in external tier with bookmark reopen via `openKnownFolder` and Reconnect-on-staleness. Working Copy / NSFileCoordinator custom Swift plugin remains a **device spike** (docs + interface only until on-device proof).
- [x] **p1-babasset** — .babasset binary codec with sorted-key headers, chunk table + sha256, header-only read, thin and bundled modes, blob locators to `assets/.blobs/<sha256>`; derived data outside the project folder; golden and property-based round-trip tests
- [x] **p1-babproject** — .babproject codec with directory and zip backends behind one API, parameterised by manifest kind, Export Project in Project Settings, committed directory↔zip golden
- [x] **p1-schema-migration** — Per-type ordered migration chains on load, refuse future versions, committed historical goldens, migrate-on-save prompt (never silent rewrite)
- [x] **p1-app-settings** — App settings store outside projects (Preferences / localStorage; Electron userData stub until P14) with Engine Settings fields including recents
- [x] **p1-homepage** — Homepage route with project browser, Documents/OPFS default + external opt-in, Create Empty (+ template cards when templates folder set; Empty-only on web), Engine Settings, Save / Save All / Close with dirty check; journal recovery stub (full replay in P2)
