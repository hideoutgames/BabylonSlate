...
**Active camera is named, not first.** `SceneSettings.mainCameraActorId` points at an actor that has a `CameraComponent`. Play sets that camera as `activeCamera`. If the id is missing, null, or the actor has no CameraComponent, keep the default Play camera — do not silently steal the first CameraComponent in the list (today's stub). The editor never steals (`stealActiveCamera: false`) unless the viewport **Game Camera** preview toggle is on, in which case the named game camera is shown without replacing the stored editor orbit pose.
...
- `mainCameraActorId` (actor picker filtered to CameraComponent)
...
**Scripting is a small catalog in the same slice.** No camera or light nodes exist today. Add get/set active camera (by actor), get/set field of view, get/set orthographic size, enable/disable light, set light color, set light intensity. Graphs emit commands or mutate component variables; they do not import Babylon.

**Out of v1:** area lights, multiple shadow casters, volumetric lights, required skybox mesh (the guid is enough), Cinemachine-style follow or virtual cameras, extra viewports, render-target cameras, camera stacking, post-process as lighting.
...
- *Scene Outliner*: actor hierarchy with **Search** and **+** on the same row (not a PanelFrame toolbar), hold-to-reorder (~250ms arm; move before that scrolls), visibility and lock icon toggles, long-press (~500ms) context menu. **Double-tap** a row frames that actor in the viewport. Dockview tab is the title — no second PanelFrame heading. **+** opens a **Place Actors** `CatalogDialog` (Shapes, Lights, Camera, Empty, Project assets). There is no Scene Assets / Mini Asset Browser dock; saved layouts close `mini-asset-browser` on restore.
- *Details*: scene settings when nothing is selected (environment colour and optional environment texture / IBL, fog colour plus linear start/end, gravity, fixed timestep, post-process stack, **main camera** actor with a CameraComponent, GameInstance override — section 2.5); otherwise the selection, with transform as one nowrap XYZ row each for Position / Rotation (Euler degrees in the UI, quaternion in `transform.rotation`) / Scale, a components list supporting add, remove and reorder via the same catalog chrome, per-component properties (humanized camelCase labels, icon reset; light kind and camera projection as enums), exposed script variables, and per-property reset-to-default.
...
- Choosing a hit **opens the asset that contains it**: Scene/Class tabs via the existing open-or-focus path; actors select in the outliner; graph nodes reuse Compiler Results focus (`setFocusDiagnostic`); Texture/Material/Model/Audio/Animation/Enum/Structure/ScriptInterface open Settings tabs.
...
- **Lighting and cameras (`p-lighting-camera`)**: full contract in section 2.5 — rotation-driven lights, Play color/intensity/range/cone, one `ShadowGenerator` from `shadowquality`, detached `UniversalCamera` game cameras selected by `mainCameraActorId`, fog/IBL/clear color, editor dir/spot gizmos, small camera/light node catalog. Named slice, not a new P-number; may run beside P11. *Accepts when* a directional light aims from actor rotation, Play matches editor color and intensity, one shadow map follows `shadowquality`, Play uses `mainCameraActorId` as a detached UniversalCamera, and 2D ortho still looks +Z.
