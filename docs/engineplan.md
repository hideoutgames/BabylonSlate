...
| Unbuilt components in catalogs | `BehaviourTreeComponent` and `NavAgentComponent` are addable. `NavMeshComponent` is Place Actors only. `WidgetComponent` is hidden until `CreateForMesh` exists. `AudioComponent` is not in Search/Add; graphs use `audio.play` → `playSound` command | P11 / later |
