...
**Keep and build on:**
- The two-tier tab model: a **two-row editor chrome** in [apps/editor/src/components/editor-chrome-bar.tsx](apps/editor/src/components/editor-chrome-bar.tsx) — a title/tab bar (truncated project name with a trailing `.babproject` stripped for display, plus global document tabs with a pinned Content Browser and **no tab-bar Add**) and a **global toolbar** below it (Undo, Redo, **Compile** when the active document is a graph, Save All with a dirty dot when unsaved, Play, Focus, Search, Settings) — plus a per-document Dockview workspace in [apps/editor/src/shell/dockview-shell.tsx](apps/editor/src/shell/dockview-shell.tsx). This is exactly the Unreal-style shell the engine needs.
...
- [ ] **p7-play-scene-load** — P7 follow-up (P6/P7): Play loads the active `SerializedScene` actors into the worker. Today Play posts a dummy `play-scene` guid and seeds demo actors plus script spawn only, so authored RigidBody/Collider (and any other scene component) do not simulate until this lands. Broader than physics — needed for any authored component to run in Play.
