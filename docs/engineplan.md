...
**Keep and build on:**
- The two-tier tab model: a **two-row editor chrome** in [apps/editor/src/components/editor-chrome-bar.tsx](apps/editor/src/components/editor-chrome-bar.tsx) — a title/tab bar (truncated project name with a trailing `.babproject` stripped for display, plus global document tabs with a pinned Content Browser and **no tab-bar Add**) and a **global toolbar** below it (Undo, Redo, **Compile** when the active document is a graph, a **Compilation Error** status after Compile when blocking diagnostics exist, Save All with a dirty dot when unsaved, Play, **Windows** immediately left of Focus, Focus, Search, Settings) — plus a per-document Dockview workspace in [apps/editor/src/shell/dockview-shell.tsx](apps/editor/src/shell/dockview-shell.tsx). This is exactly the Unreal-style shell the engine needs.
...
- CI shape: `verify.yml` (typecheck, lint, Vitest, Playwright, VitePress docs build) plus `preview.yml` (GitHub Pages: editor at `/` and docs site at `/docs/` for on-device iPad testing) and the `VITE_TEST_MODE` hatch in [packages/storage/src/test-mode.ts](packages/storage/src/test-mode.ts). A deployed URL is the only practical iPad loop without a Mac, so this stays central.
- Compact chrome CSS in [apps/editor/src/shell/editor-chrome.css](apps/editor/src/shell/editor-chrome.css) (`--chrome-row` 28px) with graph pin rows still at `--touch-target` (44px), and the rules in [.cursor/rules/touch-editor.md](.cursor/rules/touch-editor.md).
