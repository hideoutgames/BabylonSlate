...
- *Graph*: event graph and per-function graphs. **Double-tap empty pane** opens the unfiltered Add Node catalog (search is **not** autofocused). There is no floating Add node button. **Tap-to-connect** and **drag-to-connect** both persist edges (`onConnect` / `isValidConnection`). Dropping a wire on empty canvas (not near the source pin) opens Add Node **filtered to compatible opposite pins**, places the node, and auto-wires. Tap empty pane clears selection; hold empty pane then move marquees. Overlay toolbar: Copy, Paste, Delete, and **Format** (visible disabled stub — layout selected nodes, or if exactly one node is selected walk the exec/data chain to the right and tidy that chain; algorithm is a P5 follow-up).
