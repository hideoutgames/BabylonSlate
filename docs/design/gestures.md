...
- Desktop document history binds **Mod+Z** (undo) and **Mod+Shift+Z** / **Mod+Y** (redo) through the editor keybind dispatcher ([keybinds.md](../architecture/keybinds.md)). It no-ops inside native-editing targets and while three or more pointers are down, so a three-finger viewport pan cannot fire document undo.
...
| Native | WKWebView `scrollView.bounces = false`, applied by `SceneDelegate` on scene connect and activation | Capacitor iOS app only |
...
- **Pinch** (two fingers, spread change) and **wheel** zoom along the **3D camera's view center**, keeping its target and orientation fixed, regardless of Pivot Around Center. In **2D**, pinch stays anchored at the initial finger midpoint and wheel at the cursor. Two-finger translation does not orbit or pan. 2D pinch and wheel are continuous, including when pixel-perfect integer zoom steps is on. Ending a pinch with one finger still down starts a new stroke (`moved = false`, `skipLook`) so leftover look does not jump. **Game Camera** preview ignores editor zoom.
- **Three fingers** move the camera. In **3D** this is the same look-relative **fly** as WASD (strafe on horizontal drag, dolly along look on vertical drag; fixed world-units-per-pixel scale). It is not a view-plane pan (camera right + up). **Pivot Around Center** uses the same fly — the orbit center travels with the camera. In **2D** this is the same 1:1 frustum / CSS-pixel mapping as one-finger pan.
...
- **Editor camera joystick** (`settings.editorJoystickEnabled`) is an on-screen stick (default **on**; missing keys normalize to true; user off persists `false`) that drives the same fly/pan path, or **orbits** in 3D when **Pivot Around Center** is on. Scene and Prefab **Viewport Settings** menus expose **Viewport Mode** (PBR / Unlit / Wireframe), Snap, Show Grid, **Show Navmesh**, **Show Collisions** (session MeshComponent dashes, default off), Joystick, **Pivot Around Center**, and Game Camera; Scene persists Snap / Show Grid / Joystick, Prefab uses live context (joystick also defaults on). Viewport Mode, Pivot Around Center, Game Camera, and Show Collisions are session-only (not scene JSON). Not the P9 game `TouchJoystick`.
...
Scale about the midpoint, then apply two-finger translation. Wheel zooms about the cursor. Canvases use `touch-none`. **Move** is the default tool in both editors (`applyPointerPan`). Two-finger pinch/pan and wheel zoom stay available in every tool. A second finger drops any in-progress one-finger paint/select stroke so pinch does not paint.
...
- Gesture-opened node palettes are anchored popups clamped inside the safe-area viewport (search **not** autofocused on touch); the toolbar **+** opens the same palette as a large modal. Both close instantly. Long-press / secondary click opens node context menus when enabled.
...
**Outliner Delete:** with multiple actors selected, an actor's **⋯ → Delete** preserves the selection and asks **Delete Selected (N)**, **Delete This Object**, or **Cancel**, even when the clicked actor is outside the selection. Each delete includes attached children and applies in one undoable scene edit; surviving selected actors stay selected. Single-actor deletion and the folder-specific delete choices retain their existing behavior.
