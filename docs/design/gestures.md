...
- **Editor camera joystick** (`settings.editorJoystickEnabled`) is an optional on-screen stick that drives the same fly/pan path, or **orbits** in 3D when **Pivot Around Center** is on. Scene and Prefab **Viewport Settings** menus expose Snap, Show Grid, Joystick, **Pivot Around Center**, and Game Camera; Scene persists Snap / Show Grid / Joystick, Prefab uses live context. Pivot Around Center and Game Camera are session-only (not scene JSON). Not the P9 game `TouchJoystick`.
- **Double-tap** an Outliner actor or Prefab Components row (including Prefab Root) retargets the editor camera center to that origin. 3D keeps the current eye (`setTarget` + rebuild alpha/beta/radius); 2D pans the target. **Frame Selection** uses the same path.
