...
- Add Node catalog **body** is window-virtualised (`p17-add-node-virtualize`); category sidebar stays unwindowed. Distinct from canvas `p17-graph-virtualize`.
- Output Log and Compiler Results window-virtualise to viewport plus overscan (`p19-log-virtualize`; TreeView arithmetic). Ring buffer cap 500 stays. Same helper may window `SearchDialog` (AssetPicker / ClassPicker). Place Actors catalogs stay unwindowed. Global Search **result** body is not virtualised.
...
- Construct textures only through `ResourceCache` (stable blob URL + canonical sampling flags). **One cache per Engine lifetime** (`p19-shared-resource-cache`): Play / Prefab / Material / UI reuse the viewport cache even when `sharedEngine` is set; eviction stays unreferenced-LRU only.
- Editor idle `freezeActiveMeshes()` / static `freezeWorldMatrix()` / `material.freeze()` / unique-id maps / scene-load `forceCompilationAsync` are **P19** (`p19-editor-scene-freeze`). Visible editor stays at `viewportFrameCap` — do not dirty-skip an on-screen scene.
- Global Search rebuilds when the dialog is initiated (`p19-search-on-demand`), not on project open. Async/chunked; include open-document JSON. No on-disk search cache.
