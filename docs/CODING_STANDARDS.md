...
- `core`, `edit`, `assets`, `vfs`, `object-model`, `scripting`, `scripting-nodes`, `bridge`, `runtime`, `debugger`, `input`, `behaviour-tree` — no React or Babylon imports (Capacitor banned too except `vfs` adapters).
