...
export type MaterialParameterCatalog = Readonly<Record<string, {
...
export function normalizeMaterialParameterCatalog(value: unknown): MaterialParameterCatalog {
  if (!value || typeof value !== "object" || Array.isArray(value)) return {};
  const entries: Array<[string, MaterialParameterCatalog[string]]> = [];
  for (const [guid, raw] of Object.entries(value)) {
    if (!guid || !raw || typeof raw !== "object" || Array.isArray(raw)) continue;
    const entry = raw as Record<string, unknown>;
    if ((entry.domain !== "surface" && entry.domain !== "postProcess" && entry.domain !== "particle") ||
        typeof entry.planHash !== "string" || !entry.planHash) continue;
    entries.push([guid, { domain: entry.domain, planHash: entry.planHash,
