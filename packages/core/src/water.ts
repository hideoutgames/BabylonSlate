...
export const waterWaveComponents = [[0, 1, 0.65, 0], [1.1, 1.73, 0.25, 1.2], [-0.7, 2.41, 0.1, 2.7]] as const;
...
export function waterFootprint(body: WaterBodyProperties, x: number, z: number) {
  ...
  let best = Infinity, height = 0, flowX = 0, flowZ = 1, slopeX = 0, slopeZ = 0, flowY = 0, edgeX = 1, edgeZ = 0;
  for (let i = 1; i < body.points.length; i++) {
    ...
    if (distance < best) {
      ...
      edgeX = distance > 1e-6 ? (x - a[0] - t * vx) / distance : -flowZ;
      edgeZ = distance > 1e-6 ? (z - a[2] - t * vz) / distance : flowX;
      flowY = (b[1] - a[1]) / length;
      const slope = t > 0 && t < 1 ? flowY : 0;
      slopeX = slope * flowX; slopeZ = slope * flowZ;
    }
  }
  return { inside: best <= body.width / 2 + 1e-7, height, edge: body.width / 2 - best, edgeX, edgeZ, flowX, flowY, flowZ, slopeX, slopeZ };
}
