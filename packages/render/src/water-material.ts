  ...
  override getCustomCode(shaderType: string, language = ShaderLanguage.GLSL): Record<string, string> | null {
    ...
    const code = [
      ...
      ...waterWaveComponents.flatMap(([turn, frequency, amplitude, phase], i) => [
        ...
        decl(f, `swFilter${i}`, `1.0 - smoothstep(0.5, 2.0, fwidth(swP${i}))`),
        `swHeight += ${w}.x * ${amplitude.toFixed(4)} * sin(swP${i}) * swFilter${i};`,
        `swGradient += swD${i} * (${w}.x * ${amplitude.toFixed(4)} * swK${i} * cos(swP${i}) * swFilter${i});`,
      ]),
