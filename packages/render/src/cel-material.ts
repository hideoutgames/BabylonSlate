import {
  Color3, ImageProcessingConfiguration, PBRMaterial, ShaderStore, StandardMaterial,
...
for (const wgsl of [false, true]) {
  ...
  store.slateCelVertexShader = (wgsl ? defaultVertexShaderWGSL : defaultVertexShader).shader;
  const uniforms = CEL_UNIFORMS.map((name) => wgsl ? `uniform ${name}: vec4f;` : `uniform vec4 ${name};`).join("\n");
  store.slateCelPixelShader = (wgsl ? defaultPixelShaderWGSL : defaultPixelShader).shader
    .replace("#include<lightsFragmentFunctions>", `${uniforms}\n#include<slateCelLightsFragmentFunctions>`)
    .replaceAll("#include<lightFragment>", "#include<slateCelLightFragment>")
    .replace("#ifdef EMISSIVEASILLUMINATION", "diffuseBase=slateCelSurfaceLight(diffuseBase);\n#ifdef EMISSIVEASILLUMINATION");
}
...
export class CelMaterial extends StandardMaterial {
  ...
  constructor(readonly source: PBRMaterial | StandardMaterial, scene: Scene) {
    ...
    this.customShaderNameResolve = (_shader, uniforms) => { uniforms.push(...CEL_UNIFORMS); return "slateCel"; };
  ...
  override getClassName(): string { return "CelMaterial"; }

  syncSource(): void {
    const source = this.source;
    const common = ["alpha", "alphaMode", "transparencyMode", "backFaceCulling", "sideOrientation", "twoSidedLighting", "zOffset", "zOffsetUnits", "fogEnabled", "disableDepthWrite", "needDepthPrePass", "pointSize"] as const;
    for (const key of common) if (this[key] !== source[key]) this[key] = source[key] as never;
    ...
    if (this.sourceFillMode !== source.fillMode) { this.fillMode = source.fillMode; this.sourceFillMode = source.fillMode; }
    const unlit = source.disableLighting || (source instanceof PBRMaterial && source.unlit);
    if (this.sourceLighting !== unlit) { this.disableLighting = unlit; this.sourceLighting = unlit; }
    if (source instanceof PBRMaterial) {
      // glTF color factors are linear; textures retain their authored sRGB pixels.
      source.albedoColor.toGammaSpaceToRef(this.diffuseColor, this.getScene().getEngine().useExactSrgbConversions);
      source.emissiveColor.toGammaSpaceToRef(this.emissiveColor, this.getScene().getEngine().useExactSrgbConversions);
...
export function canUseCelMaterial(material: Material): material is PBRMaterial | StandardMaterial {
  if (material instanceof CelMaterial) return false;
  if (material instanceof PBRMaterial) return !material.unlit && !material.disableLighting;
