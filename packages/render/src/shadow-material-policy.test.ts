...
async function compiledMaterial(scene: Scene, document: MaterialDocument, functions: Record<string, MaterialFunctionDocument> = {}) {
...
describe("shadow material cache policy", () => {
  it.each(["pbr", "standard", "cel"] as const)("admits an opaque native %s caster but stops caching alpha coverage", (kind) => {
    const { scene, mesh } = fixture();
    const material = kind === "pbr" ? new PBRMaterial("opaque", scene)
      : kind === "standard" ? new StandardMaterial("opaque", scene)
  ...
  it("rejects untrusted materials, plugins, bind hooks and shadow wrappers", () => {
    ...
    expect(canCacheShadowMaterial(new CustomPBR("custom", scene), mesh)).toBe(false);
    ...
    new MaterialPluginBase(plugged, "unknown-vertex-plugin", 200, {}, true, true);
  ...
  it.each(["skeleton", "morph", "baked", "instance", "render-hook"] as const)("keeps %s deformation on live shadow refresh", (kind) => {
    const { scene, mesh } = fixture();
    const material = new PBRMaterial("native", scene);
    if (kind === "skeleton") mesh.skeleton = new Skeleton("rig", "rig", scene);
    if (kind === "morph") mesh.morphTargetManager = new MorphTargetManager(scene);
    if (kind === "baked") mesh.bakedVertexAnimationManager = new BakedVertexAnimationManager(scene);
  ...
  it("admits only a trusted opaque compiler result and revokes an out-of-band rebuild", async () => {
    const { scene, mesh } = fixture();
    const material = await compiledMaterial(scene, createDefaultMaterialDocument());
    mesh.material = material;
    expect(canCacheShadowMaterial(material, mesh)).toBe(true);
    const rebuilt = new Promise<void>((resolve) => material.onBuildObservable.addOnce(() => resolve()));
    material.build();
    await rebuilt;
    expect(canCacheShadowMaterial(material, mesh)).toBe(false);
    const masked = createDefaultMaterialDocument();
    masked.blendMode = "masked";
    expect(canCacheShadowMaterial(await compiledMaterial(scene, masked), mesh)).toBe(false);
  });
  ...
  it("rejects nested-function vertex displacement even with zero bounds padding", async () => {
    ...
    document.nodes.push({ id: "offset", type: "function.call", position: { x: 0, y: 0 }, properties: { functionGuid: "offset-function" } });
    document.edges.push(
      { id: "to-offset", sourceNodeId: "baseColor", sourcePinId: "out", targetNodeId: "offset", targetPinId: "in_value" },
      { id: "offset-to-vertex", sourceNodeId: "offset", sourcePinId: "out_value", targetNodeId: "output", targetPinId: "worldPositionOffset" },
    );
    const material = await compiledMaterial(scene, document, { "offset-function": fn });
  ...
  it("does not certify custom GLSL even in an opaque fragment graph", async () => {
    const { scene, mesh } = fixture();
    const document = createDefaultMaterialDocument();
    document.nodes.push({ id: "glsl", type: "custom.glsl", position: { x: 0, y: 0 }, properties: { body: "a + b" } });
    document.edges = document.edges.filter((edge) => edge.id !== "e-color-output");
    document.edges.push(
      { id: "color-to-glsl", sourceNodeId: "baseColor", sourcePinId: "out", targetNodeId: "glsl", targetPinId: "a" },
      { id: "glsl-to-output", sourceNodeId: "glsl", sourcePinId: "out", targetNodeId: "output", targetPinId: "baseColor" },
    );
    expect(canCacheShadowMaterial(await compiledMaterial(scene, document), mesh)).toBe(false);
  });
});
