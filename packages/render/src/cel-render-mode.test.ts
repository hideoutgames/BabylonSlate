...
import { Color3, Material, MeshBuilder, MultiMaterial, NullEngine, PBRMaterial, PointLight, RawTexture, Scene, StandardMaterial, Vector3 } from "@babylonjs/core";
...
function host() { const engine = new NullEngine(); engines.push(engine); return new Scene(engine); }
...
describe("native CEL render mode", () => {
  it("adapts imported slots and late meshes, preserving textures, alpha and PBR restoration", () => {
    ...
    pbr.albedoTexture = RawTexture.CreateRGBATexture(new Uint8Array([30, 90, 120, 200]), 1, 1, scene);
    ...
    const unlit = new StandardMaterial("sprite", scene); unlit.disableLighting = true;
    slots.subMaterials = [pbr, unlit, null];
    const mesh = MeshBuilder.CreateBox("model", {}, scene); mesh.material = slots;
    ...
    const late = MeshBuilder.CreateBox("late", {}, scene); late.material = pbr;
    scene.onBeforeRenderObservable.notifyObservers(scene);
    expect(late.material).toBe(cel);
    setSceneRenderSettings(scene, { mode: "pbr", cel: normalizeCelShadingSettings({ shadowBands: 8 }) });
  ...
  it("switches authored graphs in place and preserves frozen materials and live parameters", async () => {
    ...
    const compiled = compileMaterialPlan(lowered.plan, { scene, name: "authored" });
    if (!compiled.ok) throw new Error(JSON.stringify(compiled.diagnostics));
    expect(await compiled.ready).toEqual([]);
    const material = compiled.material;
    const mesh = MeshBuilder.CreateBox("actor", {}, scene); mesh.material = material;
    for (let i = 0; i < 6; i++) new PointLight(`light-${i}`, new Vector3(i, 3, 0), scene);
    setSceneRenderSettings(scene, { mode: "cel" });
    ...
    await vi.waitFor(() => expect(material.compiledShaders).toContain("pbrBlockAlbedoOpacity"));
    expect(mesh.material).toBe(material);
    expect(material.isFrozen).toBe(true);
    setSceneRenderSettings(scene, { mode: "cel" });
    await vi.waitFor(() => expect(material.compiledShaders).toContain("slateCelSurfaceLight"));
    expect(mesh.material).toBe(material);
    expect(material.isFrozen).toBe(true);
    compiled.dispose();
  ...
  it("resolves scene overrides without leaking them between scenes or into PBR", () => {
    ...
    const project = { mode: "cel" as const, cel: normalizeCelShadingSettings({ shadowBands: 4, specularStrength: 0.3 }) };
    ...
    setSceneRenderSettings(a, { ...project, cel: { ...project.cel, specularStrength: 0.6 } });
    expect(sceneRenderingSettings(a).cel).toMatchObject({ shadowBands: 7, specularStrength: 0.6 });
