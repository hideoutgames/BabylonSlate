...
type UploadedTexture = Pick<InternalTexture,
  "isReady" | "width" | "height" | "depth" | "isCube" | "is3D" |
  "is2DArray" | "format" | "type" | "generateMipMaps"
>;
...
function blockLayout(format: number): readonly [number, number, number] | null {
  const astc = ASTC_BLOCKS[format - Constants.TEXTUREFORMAT_COMPRESSED_RGBA_ASTC_4x4] ??
    ASTC_BLOCKS[format - Constants.TEXTUREFORMAT_COMPRESSED_SRGB8_ALPHA8_ASTC_4x4_KHR];
...
function bytesPerPixel(format: number, type: number): number {
  ...
  switch (format) {
    ...
    case Constants.TEXTUREFORMAT_RED_INTEGER: channels = 1; break;
    case Constants.TEXTUREFORMAT_LUMINANCE_ALPHA:
    case Constants.TEXTUREFORMAT_RG:
    case Constants.TEXTUREFORMAT_RG_INTEGER: channels = 2; break;
    case Constants.TEXTUREFORMAT_RGB:
    case Constants.TEXTUREFORMAT_RGB_INTEGER: channels = 3; break;
    case Constants.TEXTUREFORMAT_RGBA:
    case Constants.TEXTUREFORMAT_RGBA_INTEGER: channels = 4; break;
    // Unknown upload formats are conservatively accounted as RGBA float.
    default: return 16;
  }
  switch (type) {
    case Constants.TEXTURETYPE_UNSIGNED_BYTE:
    case Constants.TEXTURETYPE_BYTE: return channels;
    case Constants.TEXTURETYPE_HALF_FLOAT:
    case Constants.TEXTURETYPE_SHORT:
    case Constants.TEXTURETYPE_UNSIGNED_SHORT: return channels * 2;
    case Constants.TEXTURETYPE_UNSIGNED_SHORT_4_4_4_4:
    case Constants.TEXTURETYPE_UNSIGNED_SHORT_5_5_5_1:
    case Constants.TEXTURETYPE_UNSIGNED_SHORT_5_6_5: return 2;
    case Constants.TEXTURETYPE_UNSIGNED_INT_2_10_10_10_REV:
    case Constants.TEXTURETYPE_UNSIGNED_INT_10F_11F_11F_REV:
    case Constants.TEXTURETYPE_UNSIGNED_INT_5_9_9_9_REV: return 4;
    default: return channels * 4;
  }
}
...
export function uploadedTextureBytes(texture: UploadedTexture | null, mipLevels?: number): number | null {
  if (!texture?.isReady || !Number.isFinite(texture.width) || !Number.isFinite(texture.height) || texture.width <= 0 || texture.height <= 0) return null;
  let width = Math.floor(texture.width);
  let height = Math.floor(texture.height);
  let depth = texture.is3D || texture.is2DArray ? Math.max(1, texture.depth) : 1;
  if (!Number.isFinite(depth)) return null;
  depth = Math.floor(depth);
  const fullChain = 1 + Math.floor(Math.log2(Math.max(width, height, texture.is3D ? depth : 1)));
  const levels = mipLevels !== undefined && Number.isFinite(mipLevels)
    ? Math.max(1, Math.min(fullChain, Math.floor(mipLevels)))
    : texture.generateMipMaps ? fullChain : 1;
