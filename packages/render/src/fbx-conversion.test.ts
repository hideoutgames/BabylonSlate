...
beforeAll(async () => {
  ...
  importer = await assimp({ wasmBinary: readFileSync(require.resolve("assimpjs/dist/assimpjs.wasm")) });
});
const fixture = (name: string) => ({ name, bytes: new Uint8Array(readFileSync(new URL(`./__fixtures__/fbx/${name}`, import.meta.url))) });
...
describe("FBX canonical conversion", () => {
  it("converts a static FBX into a loadable GLB model", async () => {
    ...
    try {
      const container = await loadModelContainer(handle.scene, bytes, "box.glb");
      expect(container.meshes.some((mesh) => mesh.getTotalVertices() > 0)).toBe(true);
  ...
  it("preserves a skeleton, joint weights and a playable animation", async () => {
    const bytes = convertFbxWithAssimp(importer, fixture("animation_with_skeleton.fbx"));
    const json = splitGlbJsonBin(bytes)!.json;
    expect((json.skins as unknown[]).length).toBeGreaterThan(0);
    expect(parseGlbForBrowse(bytes)!.animations.some((clip) => (clip.durationMs ?? 0) > 0)).toBe(true);
    const handle = createTestEngine();
    try {
      const container = await loadModelContainer(handle.scene, bytes, "rig.glb");
      expect(container.meshes.some((mesh) => mesh.skeleton && mesh.isVerticesDataPresent("matricesWeights"))).toBe(true);
      expect(handle.scene.animatables).toHaveLength(0);
  ...
  it("rejects malformed FBX instead of importing a different file from the batch", () => {
    expect(() => convertFbxWithAssimp(importer, { name: "bad.fbx", bytes: new Uint8Array([1, 2, 3]) }, [fixture("box.fbx")])).toThrow(/FBX conversion failed/);
  });
});
