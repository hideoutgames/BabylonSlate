...
export function withShadowDistanceFade(source: string, wgsl: boolean): string {
  ...
  return checkedShader(source, wgsl ? "shadow distance WGSL" : "shadow distance GLSL").replace(
...
for (const wgsl of [false, true]) {
  ShaderStore.GetIncludesShadersStore(wgsl ? 1 : 0).lightFragment = withShadowDistanceFade((wgsl ? lightFragmentWGSL : lightFragment).shader, wgsl);
}
