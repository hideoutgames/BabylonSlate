...
function ledger(engine: AbstractEngine): Ledger {
  ...
  if (!value) {
    value = {
      limit: MANAGED_LIGHTING_BYTE_LIMIT,
      shadowBytes: 0,
      resourceBytes: 0,
      pendingBytes: 0,
      shadows: new Map(),
...
export function managedLightingReservations(engine: AbstractEngine) {
  ...
  return {
    ...
    clusterBytes: value.resourceBytes,
...
export function limitManagedLightingBytes(
  ...
  if (
    ...
    throw new Error(
      "Managed lighting limit must decrease without revoking live reservations.",
    );
  value.limit = bytes;
}
...
export function reserveManagedShadowBytes(
  ...
  if (
    !Number.isSafeInteger(bytes) ||
    bytes < 0 ||
    bytes - previous > availableManagedLightingBytes(engine)
  )
    throw new Error(
      "Shadow allocation exceeds the shared managed lighting reservation.",
    );
...
export function beginManagedLightingAllocation(
  ...
  if (!Number.isSafeInteger(bytes) || bytes <= 0)
    throw new Error(
      "Managed lighting allocation requires a positive byte count.",
    );
  ...
  return {
    commit(resources) {
      if (released || !pending)
        throw new Error("Managed lighting allocation is no longer pending.");
      ...
      if (actual > bytes || actual === 0)
        throw new Error(
          "Actual managed lighting allocation exceeds its reserved peak.",
        );
      for (const [handle, size] of unique) {
        const current = value.resources.get(handle);
        if (current) current.references++;
        else {
          value.resources.set(handle, { bytes: size, references: 1 });
          value.resourceBytes += size;
        }
      }
      owned = unique;
      pending = false;
