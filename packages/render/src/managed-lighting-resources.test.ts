...
import { beginManagedLightingAllocation, limitManagedLightingBytes,
  managedLightingReservations, reserveManagedShadowBytes } from "./managed-lighting-resources";
...
function fixture(bytes: number) { const engine = new NullEngine(); engines.push(engine);
  limitManagedLightingBytes(engine, bytes); return engine; }
...
describe("managed lighting allocation transactions", () => {
  it("holds old and new generations before allocation, rolls back, and deduplicates borrowed handles", () => {
    ...
    const texture = {};
    first.commit([{ handle: texture, bytes: 30 }, { handle: texture, bytes: 30 }]);
  ...
  it("keeps an underestimated candidate reserved until caller confirms cleanup", () => {
    ...
    expect(() => candidate.commit([{ handle: {}, bytes: 41 }])).toThrow(/reserved peak/);
  ...
  it("derives float formats, rectangular mip chains and multisample resolve storage from actual textures", () => {
    const engine = fixture(10000); const scene = new Scene(engine);
    const texture = new RawTexture(new Float32Array(32), 4, 2,
      Constants.TEXTUREFORMAT_RGBA, scene, true, false, 1, Constants.TEXTURETYPE_FLOAT);
    const internal = texture.getInternalTexture()!;
    // 4x2 +2x1 +1x1 =11 RGBA32F texels, plus a 4-sample 4x2 attachment.
    internal.samples = 4;
    expect(managedTextureResource(texture)).toEqual({ handle: internal, bytes: 688 });
