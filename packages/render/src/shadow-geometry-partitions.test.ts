...
it("partitions a large static ground without changing its geometry, UVs or face order", () => {
  ...
  try {
    ...
    const ground = MeshBuilder.CreateGround(
      "terrain",
      { width: 1000, height: 1000, subdivisions: 64 },
      scene,
    );
    ...
    expect(
      ground.subMeshes.reduce((sum, part) => sum + part.indexCount, 0),
    ).toBe(indices.length);
    expect(
      ground.subMeshes.every(
        (part) => part.indexCount <= 3072 && part.materialIndex === 0,
      ),
    ).toBe(true);
