...
export class ResourceCache {
  ...
  private readonly onEvict?: (assetGuid: string) => void;
  ...
  evictToCeiling(): void {
    ...
    for (const entry of candidates) {
      ...
      this.evictEntry(entry.assetGuid);
    }
  }
  ...
  private evictEntry(assetGuid: string): void {
    const entry = this.entries.get(assetGuid);
    if (!entry) return;
    this.totalBytes -= entry.bytes;
    ...
    this.onEvict?.(assetGuid);
  }

  dispose(): void {
    for (const guid of [...this.entries.keys()]) {
      this.evictEntry(guid);
    }
  }
}
