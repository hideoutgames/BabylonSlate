...
type QueryShape =
  Exclude<ColliderShape, { kind: "convex" | "mesh" }> | TriangleShape;
...
type ColliderPose = { position: Vector3; rotation: Quaternion; scale: Vector3 };

function composePose(
  local: SerializedTransform,
  parent?: ColliderPose,
): ColliderPose {
  const position = new Vector3(...local.position);
  const rotation = new Quaternion(...local.rotation);
  const scale = new Vector3(...local.scale);
  if (!parent) return { position, rotation, scale };
  position.multiplyInPlace(parent.scale);
  position.rotateByQuaternionToRef(parent.rotation, position);
  position.addInPlace(parent.position);
  return {
    position,
    rotation: parent.rotation.multiply(rotation),
    scale: scale.multiply(parent.scale),
  };
}

/** Build once per Drop request; optional collision debug meshes are irrelevant. */
export function collisionSurfaces(
  sceneData: SerializedScene,
  assets?: MeshAssetContext,
): DropSurface[] {
  const surfaces: DropSurface[] = [];
  const worldKind = sceneData.settings.physicsWorld;
  const complexMeshes = new Map<
    string,
    ReturnType<typeof extractGltfCollisionMesh>
  >();
  const actors = new Map(sceneData.actors.map((actor) => [actor.id, actor]));
  const actorPoses = new Map<string, ColliderPose>();
  const resolvingActors = new Set<string>();
  const actorPoseFor = (id: string): ColliderPose => {
    const known = actorPoses.get(id);
    if (known) return known;
    const actor = actors.get(id);
    if (!actor || resolvingActors.has(id))
      return composePose(identitySerializedTransform());
    resolvingActors.add(id);
    const pose = composePose(
      actor.transform,
      actor.parentId ? actorPoseFor(actor.parentId) : undefined,
    );
    resolvingActors.delete(id);
    actorPoses.set(id, pose);
    return pose;
  };
  const add = (
    actorId: string,
    shape: ColliderShape,
    { scale, rotation, position }: ColliderPose,
  ) => {
    if (
    ...
    const prepared: QueryShape = triangles
      ? {
          kind: "mesh",
          vertices: triangles.vertices.map((v) => new Vector3(v.x, v.y, v.z)),
          indices: triangles.indices,
          convex: baked.kind === "convex",
        }
      : (baked as QueryShape);
    surfaces.push({
      actorId,
      shape: prepared,
      world: pose,
      inverse: Matrix.Invert(pose),
    });
  };
  for (const actor of sceneData.actors) {
    const actorWorld = actorPoseFor(actor.id);
    ...
    const sprite =
      typeof spriteGuid === "string"
        ? assets?.spritePayloads?.get(spriteGuid)
        : undefined;
    const spriteFrame = sprite
      ? spriteClipFrameAt(
          sprite,
          typeof spriteComponent?.properties.clipName === "string"
            ? spriteComponent.properties.clipName
            : "",
          0,
        )
      : null;
    const spriteCollision =
      worldKind === "2d" && sprite && spriteFrame
        ? spriteCollisionToBox2d({
            collision: spriteFrame.collision ?? DEFAULT_SPRITE_COLLISION,
            pivot: spriteFrame.pivot,
            pixelWidth: spriteFrame.width ?? 100,
            pixelHeight: spriteFrame.height ?? 100,
            pixelsPerUnit: sprite.pixelsPerUnit || assets?.pixelsPerUnit || 100,
          })
        : null;
    const components = new Map(
      actor.components.map((component) => [component.id, component]),
    );
    const componentWorlds = new Map<string, ColliderPose>();
    const visiting = new Set<string>();
    const componentWorldFor = (id: string): ColliderPose => {
    ...
    for (const component of actor.components) {
      ...
      if (component.classId === "ColliderComponent") {
        ...
        if (collider.shape.kind === "box2d" && spriteCollision) {
          ...
          const shifted = composePose(
            {
              ...local,
              position: [
                local.position[0] + spriteCollision.translation.x,
                local.position[1] + spriteCollision.translation.y,
                local.position[2],
              ],
            },
            parent,
          ...
          add(
            actor.id,
            { kind: "box2d", halfExtents: spriteCollision.halfExtents },
            shifted,
          );
        ...
        worldKind === "3d" &&
        properties.meshKind !== "pivot"
      ) {
        const guid =
          typeof properties.assetGuid === "string"
            ? properties.assetGuid.trim()
            : "";
        const modelPayload = assets?.modelPayloads?.get(guid);
        if (
          properties.collisionMode === "complex" &&
          guid &&
          !complexMeshes.has(guid)
        ) {
          const bytes = assets?.modelBytes?.get(guid);
          complexMeshes.set(
            guid,
            bytes
              ? extractGltfCollisionMesh(bytes, modelPayload?.importScale ?? 1)
              : null,
          );
        }
        for (const collision of resolveMeshCollisions(properties, {
          modelPayload,
        ...
        const guid =
          typeof properties.assetGuid === "string" ? properties.assetGuid : "";
        const map = assets?.tilemaps?.get(guid);
        if (map && assets?.tilesets) {
          const ppu = assets.pixelsPerUnit || 100;
          for (const chain of tilemapCollisionChains(
            map,
            assets.tilesets,
            map.tileWidth / ppu,
            map.tileHeight / ppu,
            ...
            add(
              actor.id,
              { kind: "chain", points: chain.points, loop: chain.loop },
              world,
            );
          }
        }
...
export function collisionBounds(
  ...
  for (const surface of surfaces) {
    ...
    if (shape.kind === "mesh") points = shape.vertices;
    else if (shape.kind === "polygon" || shape.kind === "chain")
      points = shape.points.map((p) => new Vector3(p.x, p.y, 0));
    else {
      let x: number, y: number, z: number;
      if (shape.kind === "box" || shape.kind === "box2d") {
        x = shape.halfExtents.x;
        y = shape.halfExtents.y;
        z = shape.kind === "box" ? shape.halfExtents.z : 0;
      } else {
        x = shape.radius;
        y =
          shape.kind === "capsule" || shape.kind === "capsule2d"
      ...
      points = [-x, x].flatMap((px) =>
        [-y, y].flatMap((py) => [-z, z].map((pz) => new Vector3(px, py, pz))),
      );
    }
    for (const point of points) {
      const transformed = Vector3.TransformCoordinates(point, surface.world);
      if (!bounds)
        bounds = { min: transformed.clone(), max: transformed.clone() };
...
function cylinderHits(
  ...
  if (
    origin.x ** 2 + origin.z ** 2 <= radius ** 2 &&
    Math.abs(origin.y) <= halfHeight
  )
    return [0];
  const values = quadratic(
    direction.x ** 2 + direction.z ** 2,
    2 * (origin.x * direction.x + origin.z * direction.z),
    origin.x ** 2 + origin.z ** 2 - radius ** 2,
  ...
  if (Math.abs(direction.y) > EPS) {
    for (const y of [-halfHeight, halfHeight]) {
      ...
      if (
        (origin.x + direction.x * t) ** 2 + (origin.z + direction.z * t) ** 2 <=
        radius ** 2 + EPS
      )
        values.push(t);
    }
...
function shapeHit(
  ...
  switch (shape.kind) {
    case "box":
    case "box2d":
      return boxHit(origin, direction, shape.halfExtents);
    case "sphere":
    case "circle":
      return nearest(
        sphereHits(origin, direction, shape.radius, 0, shape.kind === "circle"),
      );
    case "cylinder":
      return nearest(
        cylinderHits(origin, direction, shape.radius, shape.height / 2),
      );
    case "capsule":
      return nearest([
        ...cylinderHits(origin, direction, shape.radius, shape.halfHeight),
        ...sphereHits(origin, direction, shape.radius, shape.halfHeight),
        ...sphereHits(origin, direction, shape.radius, -shape.halfHeight),
      ]);
    case "capsule2d": {
      const center = boxHit(origin, direction, {
        x: shape.radius,
        y: shape.halfHeight,
      });
      return nearest([
    ...
    case "mesh": {
      if (shape.convex && shape.indices.length > 0) {
        ...
        for (let index = 0; index + 2 < shape.indices.length; index += 3) {
          const a = shape.vertices[shape.indices[index]!]!,
            b = shape.vertices[shape.indices[index + 1]!]!,
            c = shape.vertices[shape.indices[index + 2]!]!;
          if (
            Vector3.Dot(
      ...
      for (let index = 0; index + 2 < shape.indices.length; index += 3) {
        ...
        if (
          hit &&
          hit.distance >= -EPS &&
          (closest === null || hit.distance < closest)
        )
          closest = Math.max(0, hit.distance);
      }
      return closest;
    }
    case "polygon":
      ...
      if (shape.kind === "polygon") {
        ...
        for (
          ...
          const a = shape.points[index]!,
            b = shape.points[previous]!;
          if (
            a.y > origin.y !== b.y > origin.y &&
            origin.x < ((b.x - a.x) * (origin.y - a.y)) / (b.y - a.y) + a.x
          )
            inside = !inside;
        }
        if (inside) return 0;
