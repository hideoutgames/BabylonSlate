import { DirectionalLight, FreeCamera, NullEngine, PointLight, Scene, SpotLight, TransformNode, Vector3 } from "@babylonjs/core";
...
it("bounds contributors, includes borrowed cluster children and follows parent transforms", () => {
  ...
  try {
    ...
    const child = new SpotLight("borrowed", new Vector3(2, 0, 0), Vector3.Down(), 1, 1, scene);
    ...
    const unregister = registerClusteredLightPolicy(scene, {
      sync() {}, limits: () => [], target: () => undefined, ownsContainer: () => false,
      allowsLocal: () => true, clusteredCount: () => 1, authoredLights: () => [child],
    });
    const settings = { ...DEFAULT_RENDER_EFFECTS.volumetricLighting, enabled: true, maxLights: 3, maxDistance: 10 };
    expect(selectVolumetricLights(scene, camera, settings)).toEqual([sun, near, child]);
    expect(selectVolumetricLights(scene, camera, { ...settings, maxLights: 2 })).toEqual([sun, near]);
    parent.setEnabled(false);
    expect(selectVolumetricLights(scene, camera, settings)).toEqual([sun, near]);
    parent.setEnabled(true); parent.position.x = 100; child.range = 2;
    expect(selectVolumetricLights(scene, camera, settings)).toEqual([sun, near]);
