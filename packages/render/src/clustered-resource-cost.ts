...
export function clusteredTextureAllocationBytes(batchSize: number, batches: number): number {
  return batches * (64 * 64 * 4 + 5 * batchSize * 16);
}

/** Actual uncompressed storage, including complete mip chains and MSAA resolve storage. */
export function managedTextureResource(texture: BaseTexture): ManagedLightingResource {
  const internal = texture.getInternalTexture();
  if (!internal) throw new Error("Managed lighting texture has no allocation.");
  const channels = internal.format === Constants.TEXTUREFORMAT_R ? 1
    : internal.format === Constants.TEXTUREFORMAT_RG ? 2
    : internal.format === Constants.TEXTUREFORMAT_RGB ? 3
    : internal.format === Constants.TEXTUREFORMAT_RGBA ? 4 : undefined;
  const componentBytes = internal.type === Constants.TEXTURETYPE_FLOAT ? 4
    : internal.type === Constants.TEXTURETYPE_HALF_FLOAT ? 2
    : internal.type === Constants.TEXTURETYPE_UNSIGNED_BYTE ? 1 : undefined;
  if (!channels || !componentBytes || internal.isCube || internal.is3D || internal.is2DArray)
  ...
  if (![width, height, samples].every((value) => Number.isSafeInteger(value) && value > 0))
...
export function clusteredTextureResources(container: ClusteredLightContainer): ManagedLightingResource[] {
  const fields = container as unknown as {
    _lightDataTexture?: BaseTexture;
    _tileMaskTexture?: BaseTexture;
  };
  if (!fields._lightDataTexture || !fields._tileMaskTexture ||
      container.horizontalTiles !== 64 || container.verticalTiles !== 64)
