import { MaterialPluginBase, RegisterMaterialPlugin, PBRBaseMaterial, StandardMaterial, TextureBlock, NodeMaterialBlockTargets, NodeMaterialBlockConnectionPointTypes, type AbstractEngine, type Effect, type Material, type NodeMaterial, type Scene, type ShaderLanguage, type SubMesh, type UniformBuffer } from "@babylonjs/core";
...
export function textureLodSamplePattern(samplers: string, wgsl: boolean): string {
  return wgsl
...
export class QualityTextureBlock extends TextureBlock {
  ...
  protected override _buildBlock(state: NodeMaterialBuildState): this | undefined {
    ...
    state._emitUniformFromString("slateTextureLodBias", NodeMaterialBlockConnectionPointTypes.Float);
    const wgsl = state.shaderLanguage === 1;
    const generated = state.compilationString.slice(start).replace(
      new RegExp(textureLodSamplePattern(this.samplerName, wgsl), "g"),
      wgsl ? "textureSampleBias($1,$2,$3,uniforms.slateTextureLodBias)" : "texture2D($1,$2,slateTextureLodBias)",
    );
...
const MATERIAL_SAMPLERS = "(?:albedo|diffuse|ambient|opacity|emissive|specular|reflectivity|metallicReflectance|reflectance|microSurface|bump|lightmap)Sampler";
class TextureQualityPlugin extends MaterialPluginBase {
  ...
  override isCompatible(_language: ShaderLanguage): boolean { return true; }
  override hardBindForSubMesh(buffer: UniformBuffer, scene: Scene, _engine: AbstractEngine, _subMesh: SubMesh): void {
    buffer.updateFloat("slateTextureLodBias", sceneRenderingSettings(scene).textureLodBias);
  }
  override getUniforms() { return { ubo: [{ name: "slateTextureLodBias", size: 1, type: "float" }] }; }
  override getCustomCode(type: string, language?: ShaderLanguage) {
    if (type !== "fragment") return null;
    const wgsl = language === 1;
    return { ["!" + textureLodSamplePattern(MATERIAL_SAMPLERS, wgsl)]: wgsl
...
RegisterMaterialPlugin("SlateTextureQuality", (material) =>
  material instanceof PBRBaseMaterial || material instanceof StandardMaterial
    ? new TextureQualityPlugin(material) : null);
