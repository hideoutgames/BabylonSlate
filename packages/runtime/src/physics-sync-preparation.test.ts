...
function fixture(count: number) {
  ...
  const actors = Array.from({ length: count }, (_, index) => {
    const actor = world.createActor({ classId: "Actor", guid: `actor-${index}`, transform: identityTransform() });
    actor.transform.position.x = index * 4;
    actor.attachComponent(world.createComponent({
      classId: "RigidBodyComponent", guid: `body-${index}`,
      variables: { motionType: "dynamic", mass: 1, gravityScale: 0, linearDamping: 0, angularDamping: 0 },
    }));
    actor.attachComponent(world.createComponent({
      classId: "MeshComponent", guid: `mesh-${index}`,
...
function install(sync: PhysicsWorldSync, geometry: ReturnType<typeof triangles>) {
  sync.setModelContent({ models: { mesh: {
    materialSlots: [], clipNames: [], skeletonGuid: null, importScale: 1,
  } }, complexMeshes: { mesh: geometry } });
}

describe("physics preparation work", () => {
  it.each([4, 2048])("does no content-sized work for moving bodies with %i fixed triangles", (triangleCount) => {
  ...
  it.each([128, 512, 2048])("indexes the ordered actor list once for %i body readbacks", (bodyCount) => {
    ...
    try {
      ...
      reads.mockClear(); bodyReads.mockClear(); scans.mockClear();
      sync.step(1 / 60, world);
      console.info("physics actor lookup operations", { bodyCount, actorListReads: reads.mock.calls.length, actorScans: scans.mock.calls.length, bodyReads: bodyReads.mock.calls.length });
      expect(actors.at(-1)!.transform.position.x).toBeGreaterThan((bodyCount - 1) * 4);
