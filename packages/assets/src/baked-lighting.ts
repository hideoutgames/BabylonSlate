...
const atlasSchema = z
  .object({
    ...
    sha256: hash,
    encoding: z.literal("rgba32float-le"),
    colorSpace: z.literal("linear"),
    quantity: z.literal("diffuseIrradiance"),
    convention: z.literal("physical-E"),
    alpha: z.literal("coverage"),
    rowOrder: z.literal("bottomFirst"),
    uvSet: z.literal(1),
    mipLevels: z.literal(1),
    gutterTexels: integer,
  })
  .strict();
        ...
        z.discriminatedUnion("kind", [
          z
            .object({
              ...
              actorId: id,
              componentId: id,
              mobility: z.enum(["static", "stationary"]),
              inputHash: hash,
            })
            .strict(),
          z
...
function unique(values: readonly string[], label: string): void {
  if (new Set(values).size !== values.length)
    throw new Error(`Duplicate ${label} in baked lighting.`);
}

/** Stable even when names or runtime object allocation order change. */
export function bakedReceiverKey(
  identity: BakedLightingManifest["receivers"][number]["identity"],
): string {
...
export function parseBakedLightingManifest(
  ...
  unique(
    manifest.sources.map((source) =>
      source.kind === "light"
        ...
        : stableStringify([source.kind, source.assetGuid]),
    ),
    "source identity",
  );
  unique(
    manifest.atlases.map((atlas) => atlas.guid),
    "atlas GUID",
  );
  unique(
    manifest.atlases.map((atlas) => atlas.chunkId),
    "atlas chunk",
  );
  unique(
    manifest.receivers.map((receiver) => bakedReceiverKey(receiver.identity)),
    "receiver identity",
  );
  ...
  for (const atlas of manifest.atlases) {
    if (
      atlas.chunkId === BAKED_LIGHTING_MANIFEST_CHUNK ||
      atlas.chunkId.startsWith("nested:")
    )
      throw new Error("Atlas chunk uses a reserved asset chunk ID.");
    bytes += atlas.width * atlas.height * 16;
  }
  if (bytes > BAKED_LIGHTING_MAX_BYTES)
    throw new Error(
  ...
  for (const receiver of manifest.receivers) {
    ...
    for (const axis of [0, 1] as const) {
      ...
      if (
        receiver.offset[axis] < gutter ||
        receiver.offset[axis] + receiver.scale[axis] > 1 - gutter + 1e-12
      )
        throw new Error(
          "Baked receiver atlas rectangle or gutter is outside the atlas.",
        );
    }
    unique(
      receiver.contributions.map((contribution) => contribution.sourceId),
      "receiver contribution",
    );
    for (const contribution of receiver.contributions) {
      const source = sources.get(contribution.sourceId);
      if (!source)
        throw new Error("Baked receiver references a missing source.");
      if (
        source.kind === "environment"
          ? contribution.term !== "environmentDiffuse"
          : contribution.term === "environmentDiffuse" ||
            (source.mobility === "stationary" &&
              contribution.term !== "indirectOnly")
      )
        throw new Error(
          "Baked contribution is incompatible with its source or mobility.",
        );
    }
    if (
      receiver.identity.primitive.kind === "model" &&
      !manifest.dependencies.includes(receiver.identity.primitive.assetGuid)
    )
  ...
  for (const source of manifest.sources)
    if (
      source.kind === "environment" &&
      !manifest.dependencies.includes(source.assetGuid)
    )
      throw new Error(
        "Baked environment input is missing from asset dependencies.",
      );
  if (used.size !== atlases.size)
    throw new Error("Baked lighting contains an unreferenced atlas.");
  return manifest;
}

/** Each supplied value is canonical JSON; geometry/UV binary data may use SHA-256 digests as values. */
export async function fingerprintBakeInputs(
  inputs: Record<keyof BakeInputHashes, unknown>,
): Promise<BakeInputHashes> {
  const keys = Object.keys(hashFields) as Array<keyof BakeInputHashes>;
  if (
    Object.keys(inputs).length !== keys.length ||
    keys.some((key) => !(key in inputs))
  )
  ...
  const entries = await Promise.all(
    keys.map(async (key) => {
      ...
      return [
        key,
        await sha256Hex(new TextEncoder().encode(stableStringify(value))),
      ];
    }),
  );
...
export function bakedLightingValidity(
  ...
  if (!manifest)
    return {
      status: "missing",
      reason: "No readable baked lighting output is assigned.",
    };
...
async function validateChunks(
  ...
  const document = chunks.find(
    (chunk) => chunk.id === BAKED_LIGHTING_MANIFEST_CHUNK,
  );
  if (
    !document ||
    document.kind !== "document" ||
    document.mime !== "application/json" ||
    document.data.byteLength > MAX_MANIFEST_BYTES
  )
    throw new Error(
      "Baked lighting manifest is missing or exceeds its size limit.",
    );
  const manifest = parseBakedLightingManifest(
    JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(document.data)),
  );
  if (
    stableStringify([...header.dependencies].sort()) !==
    stableStringify([...manifest.dependencies].sort())
  )
    throw new Error(
      "Baked lighting header dependencies do not match the manifest.",
    );
  if (chunks.length !== manifest.atlases.length + 1)
  ...
  for (const atlas of manifest.atlases) {
    ...
    if (
      !chunk ||
      chunk.kind !== "diffuseIrradiance" ||
      chunk.mime !== "application/octet-stream" ||
  ...
  for (const atlas of manifest.atlases) {
    ...
    if ((await sha256Hex(data)) !== atlas.sha256)
      throw new Error(`Irradiance atlas ${atlas.guid} hash mismatch.`);
    ...
    for (let offset = 0; offset < view.byteLength; offset += 4) {
      ...
      if (
        ...
        value < 0 ||
        (offset % 16 === 12 && value > 1)
      )
        throw new Error(
          `Irradiance atlas ${atlas.guid} contains invalid radiance or coverage.`,
        );
    }
  }
...
function ownedChunks(decoded: DecodedBakedLighting): ChunkInput[] {
  return [
    {
      id: BAKED_LIGHTING_MANIFEST_CHUNK,
      kind: "document",
      mime: "application/json",
      data: new TextEncoder().encode(stableStringify(decoded.manifest)),
    },
    ...decoded.manifest.atlases.map((atlas) => ({
      id: atlas.chunkId,
      kind: "diffuseIrradiance",
      mime: "application/octet-stream",
      data: decoded.atlases.get(atlas.guid)!,
    })),
  ];
}
...
export async function bakedLightingImportResult(options: {
  ...
  const result: ImportResult = {
    type: BAKED_LIGHTING_ASSET_TYPE,
    version: 1,
    guid: id.parse(options.guid),
    name: id.parse(options.name),
...
export async function encodeBakedLightingAsset(
  result: ImportResult,
): Promise<Uint8Array> {
  ...
  return encodeBabasset({
    header: {
      ...
      type: BAKED_LIGHTING_ASSET_TYPE,
      version: 1,
      dependencies: decoded.manifest.dependencies,
      payload: {},
      mode: "bundled",
      engineVersion: "0.0.0",
    },
    chunks: ownedChunks(decoded),
  });
}
...
export async function decodeBakedLightingAsset(
  ...
  if (bytes.byteLength < 12)
    throw new Error("Truncated baked lighting container.");
  ...
  if (headerLength > MAX_MANIFEST_BYTES || payloadStart > bytes.byteLength)
    throw new Error(
      "Baked lighting header exceeds its size limit or is truncated.",
    );
...
export async function readBakedLightingAssetChunks(
  ...
  if (header.chunks.length < 2 || header.chunks.length > 17)
    throw new Error("Unexpected baked lighting asset chunks.");
  unique(
    header.chunks.map((entry) => entry.id),
    "asset chunk",
  );
  const document = header.chunks.find(
    (entry) => entry.id === BAKED_LIGHTING_MANIFEST_CHUNK,
  );
  if (
    !document ||
    document.kind !== "document" ||
  ...
  for (const entry of header.chunks) {
    ...
    if ("inline" in entry.locator) {
      ...
      if (
        ...
        length < 0 ||
        length >
          (entry === document ? MAX_MANIFEST_BYTES : BAKED_LIGHTING_MAX_BYTES)
      )
        throw new Error(
          "Baked lighting chunk exceeds its size limit or has an invalid locator.",
        );
    } else if (entry.locator.blob !== entry.sha256)
      throw new Error(
        "Baked lighting blob identity does not match its chunk hash.",
      );
  }
  const readOwned = async (
    ...
    if (
      ...
      (exact !== undefined && data.byteLength !== exact)
    )
      throw new Error(
        `Baked lighting chunk ${entry.id} has an invalid byte length.`,
      );
    return data.slice();
  };
  const documentBytes = await readOwned(document, MAX_MANIFEST_BYTES);
  if ((await sha256Hex(documentBytes)) !== document.sha256)
    throw new Error("Baked lighting manifest hash mismatch.");
  const manifest = parseBakedLightingManifest(
    JSON.parse(new TextDecoder("utf-8", { fatal: true }).decode(documentBytes)),
  );
  if (
    header.chunks.length !== manifest.atlases.length + 1 ||
    stableStringify([...header.dependencies].sort()) !==
      stableStringify([...manifest.dependencies].sort())
  )
    throw new Error(
      "Baked lighting chunks or dependencies do not match the manifest.",
    );
  const declared = manifest.atlases.map((atlas) => {
    ...
    if (
      !entry ||
      entry.sha256 !== atlas.sha256 ||
      ("inline" in entry.locator && entry.locator.inline.length !== length)
    )
      throw new Error(
        `Irradiance atlas ${atlas.guid} metadata does not match the manifest.`,
      );
    return { entry, length };
  });
  const chunks: ChunkInput[] = [{ ...document, data: documentBytes }];
  for (const { entry, length } of declared)
    chunks.push({ ...entry, data: await readOwned(entry, length, length) });
