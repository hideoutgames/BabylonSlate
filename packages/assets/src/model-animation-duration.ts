...
export function resolveModelAnimationDurations<T extends {
  guid: string;
  type: string;
  modelGuid?: string;
  clipName?: string;
  ...
  retargetLoads: ReadonlyMap<string, readonly RetargetAnimationLoad[]> = new Map(),
): T[] {
  const sources = new Map([...retargetLoads.values()].flat().map((load) => [load.animationGuid, load.sourceModelGuid]));
  ...
  return catalog.map((entry) => {
    ...
    if (!clips) {
      ...
      if (bytes) {
        try {
          json = splitGlbJsonBin(bytes)?.json ?? JSON.parse(new TextDecoder().decode(bytes));
        } catch { /* Unavailable or unsupported source: retain persisted metadata. */ }
      }
      clips = new Map(json ? gltfAnimationClips(json).map((clip) => [clip.name, clip.durationMs]) : []);
