...
import { loadBakedLightingReference, publishBakedLighting } from "./baked-lighting-store";
...
async function setup() {
  ...
  for (const [guid, type] of [["model", "Model"], ["cube", "Texture"]]) {
    await registry.createAsset("project", `${guid}.babasset`, { guid: guid!, type: type!,
      name: guid!, version: 1, dependencies: [], payload: {}, chunks: [] });
  }
  let owner = { sceneGuid: "scene", generation: 1, inputs: fixture.manifest.inputs };
  const scene = createDefaultScene();
  const commit = vi.fn((guid: string) => { scene.settings.bakedLightingAssetGuid = guid; return true; });
  const options = { registry, rootId: "project", name: "Bake", ...fixture, current: () => owner, commit };
  const published = await publishBakedLighting(options);
  if (published.status !== "published") throw new Error("Fixture publication was refused");
  return { ...fixture, storage, registry, scene, options, published, commit,
    replaceOwner: (next: typeof owner) => { owner = next; } };
}

it("reopens a persisted Scene and its external atlas blobs, retaining valid and stale output separately", async () => {
  const { storage, registry, scene, manifest, bytes, published } = await setup();
  const asset = registry.getByGuid(published.guid)!;
  expect(asset.header.chunks.find((chunk) => chunk.id === "atlas:atlas")?.locator).toHaveProperty("blob");
  const encoded = await encodeAssetDocument({ type: "Scene", guid: "scene", name: "Scene", version: 4,
    payload: scene as unknown as Record<string, unknown> }, { dependencies: [published.guid] });
  ...
  const document = await decodeAssetDocument(await storage.readBinary("assets/Scene.babasset"));
  const restored = normalizeScene(document.payload);
  expect(restored.settings.bakedLightingAssetGuid).toBe(published.guid);
  const options = { registry: reopened, guid: restored.settings.bakedLightingAssetGuid, sceneGuid: "scene", inputs: manifest.inputs };
  ...
  const stale = await loadBakedLightingReference({ ...options, inputs: { ...manifest.inputs, uv: "f".repeat(64) } });
...
it.each(["generation", "hash", "abort", "refuse", "write failure"] as const)(
  "preserves the last usable Scene reference when a replacement encounters %s during storage", async (failure) => {
    const { storage, registry, scene, options, published, commit, replaceOwner } = await setup();
    let release!: () => void;
    let writing!: () => void;
    const held = new Promise<void>((resolve) => { release = resolve; });
    const started = new Promise<void>((resolve) => { writing = resolve; });
    const original = storage.writeBinary.bind(storage);
    const write = vi.spyOn(storage, "writeBinary").mockImplementation(async (path, data) => {
    ...
    await started;
    if (failure === "generation") replaceOwner({ ...options.current(), generation: 2 });
    if (failure === "hash") replaceOwner({ ...options.current(), inputs: { ...options.current().inputs, lights: "f".repeat(64) } });
    ...
    if (failure === "write failure") {
      expect(result).toHaveProperty("error", expect.objectContaining({ message: "Disk full" }));
    } else {
      expect(result).toHaveProperty("value", expect.objectContaining({ status: "refused",
        reason: failure === "refuse" ? "not-accepted" : failure === "abort" ? "aborted" : "stale" }));
...
it("does not write pre-cancelled or already stale candidates, and reports missing input dependencies on reopen", async () => {
  ...
  expect(await publishBakedLighting({ ...options, signal: abort.signal })).toEqual({ status: "refused", reason: "aborted", candidateGuid: null });
  expect(await publishBakedLighting({ ...options, current: () => ({ ...options.current(), sceneGuid: "copy" }) })).toEqual({ status: "refused", reason: "stale", candidateGuid: null });
  expect(registry.list({ type: "BakedLighting" })).toHaveLength(1);
  await registry.deleteAsset("cube");
  const loaded = await loadBakedLightingReference({ registry, guid: published.guid, sceneGuid: "scene", inputs: options.manifest.inputs });
  expect(loaded.validity).toEqual({ status: "missing", reason: "Baked lighting input asset cube is unavailable." });
