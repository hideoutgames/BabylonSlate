...
import {
  ...
  fingerprintBakeInputs,
  parseBakedLightingManifest,
  validateBakedLightingChunks,
} from "./baked-lighting";
...
it("roundtrips complete physical irradiance and mixed source membership without a renderer or provider", async () => {
  ...
  const decoded = await decodeBakedLightingAsset(
    await encodeBakedLightingAsset(result),
  );
  const data = decoded.atlases.get("atlas")!;
  const view = new DataView(data.buffer, data.byteOffset, data.byteLength);
  expect(
    Array.from({ length: 8 }, (_, index) => view.getFloat32(index * 4, true)),
  ).toEqual([2, 1, 0.5, 1, 0, 3, 0, 0]);
  ...
  expect(
    bakedLightingValidity("scene", manifest.inputs, decoded.manifest),
  ).toEqual({ status: "valid" });
});

it("distinguishes missing, duplicated Scene ownership, and every changed bake input", async () => {
  const { manifest } = await fixture();
  expect(bakedLightingValidity("scene", manifest.inputs, null).status).toBe(
    "missing",
  );
  expect(
    bakedLightingValidity("duplicated-scene", manifest.inputs, manifest),
  ).toEqual({ status: "stale", reasons: ["Scene identity changed"] });
  for (const category of [
    ...
    const current: BakeInputHashes = {
      ...manifest.inputs,
      [category]: "f".repeat(64),
    };
    expect(bakedLightingValidity("scene", current, manifest)).toEqual({
      status: "stale",
      reasons: [`${category} changed`],
    });
  }
  const first = manifest.receivers[0]!.identity;
  expect(bakedReceiverKey(first)).not.toBe(
    bakedReceiverKey({ ...first, actorId: "duplicated-instance" }),
  );
  expect(bakedReceiverKey(first)).not.toBe(
    bakedReceiverKey({
      ...first,
...
it("hashes canonical inputs independently of object key order but requires every category and finite JSON", async () => {
  ...
  expect(
    await fingerprintBakeInputs({ ...input, materials: { b: 2, a: 1 } }),
  ).toEqual(await fingerprintBakeInputs(input));
  expect(
    (await fingerprintBakeInputs({ ...input, materials: { a: 1, b: 3 } }))
      .materials,
  ).not.toBe((await fingerprintBakeInputs(input)).materials);
  await expect(
...
it("rejects unsupported data, ambiguous receiver bindings and duplicate lighting terms", async () => {
  ...
  expect(() =>
    parseBakedLightingManifest({ ...manifest, version: 2 }),
  ).toThrow();
  expect(() =>
  ...
  expect(() =>
    parseBakedLightingManifest({
      ...manifest,
      receivers: [{ ...manifest.receivers[0], mobility: "dynamic" }],
    }),
  ).toThrow();
  bad((copy) => {
    copy.receivers.push(structuredClone(copy.receivers[0]!));
  });
  bad((copy) => {
    copy.receivers[0]!.contributions[1]!.term = "directAndIndirect";
  });
  bad((copy) => {
    copy.receivers[0]!.contributions.push({
      sourceId: "sun",
      term: "directAndIndirect",
    });
  });
  bad((copy) => {
    copy.receivers[0]!.contributions[0]!.sourceId = "missing";
  });
  bad((copy) => {
    copy.receivers[0]!.offset[0] = 0.5;
  });
  bad((copy) => {
    copy.dependencies = [];
  });
  bad((copy) => {
    copy.atlases[0]!.width = 4096;
    copy.atlases[0]!.height = 4096;
  });
  expect(() =>
    parseBakedLightingManifest({
      ...manifest,
      atlases: [{ ...manifest.atlases[0], colorSpace: "srgb" }],
    }),
  ).toThrow();
});
...
it("rejects missing, corrupt and nonfinite atlas contents before they can become a usable output", async () => {
  ...
  await expect(
    validateBakedLightingChunks(result, result.chunks.slice(0, 1)),
  ).rejects.toThrow();
  const corrupt = result.chunks.map((chunk) => ({
    ...chunk,
    data: chunk.data.slice(),
  }));
  corrupt[1]!.data[0] = 1;
  await expect(validateBakedLightingChunks(result, corrupt)).rejects.toThrow(
    "hash mismatch",
  );
  ...
  await expect(
    bakedLightingImportResult({
      ...
      name: "Bad",
      manifest,
      atlases: new Map([["atlas", bytes]]),
    }),
  ).rejects.toThrow("invalid radiance or coverage");
});
...
it("owns atlas and header inputs across asynchronous validation and later caller mutation", async () => {
  ...
  const preparing = bakedLightingImportResult({
    guid: "next",
    name: "Next",
    manifest,
    atlases: new Map([["atlas", bytes]]),
  ...
  const decoded = await decodeBakedLightingAsset(
    await encodeBakedLightingAsset(prepared),
  );
...
it("validates imports before admission and refuses source GUID rebinding while allowing a new bake asset GUID", async () => {
  ...
  const [imported] = await importBabasset(bytes, {
    fileName: "Bake.babasset",
    existingGuids: new Set(["bake"]),
  });
  expect(imported!.guid).not.toBe("bake");
  bytes.fill(0);
  expect(
    (await validateBakedLightingChunks(imported!, imported!.chunks)).manifest
      .sceneGuid,
  ).toBe("scene");
  const corrupt = result.chunks.map((chunk) => ({
    ...chunk,
  ...
  await expect(
    importBabasset(
      await encodeBabasset({
        header: { ...result, engineVersion: "0", mode: "bundled" },
        chunks: corrupt,
      }),
      { fileName: "Bad.babasset", existingGuids: new Set() },
    ),
  ).rejects.toThrow("hash mismatch");
  const source = await encodeBabasset({
    header: {
      guid: "model",
      name: "Model",
      type: "Model",
  ...
  await expect(
    importBabasset(bundled, {
      fileName: "Bundle.babasset",
      existingGuids: new Set(["model"]),
    }),
  ).rejects.toThrow("source GUID remapping requires a new bake");
});
...
it("rejects invalid thin manifest sizes and unexpected chunks before fetching any atlas blob", async () => {
  ...
  const writeBlob = async (sha: string, data: Uint8Array) => {
    blobs.set(sha, data);
  };
  const bytes = await encodeBabasset({
    header: { ...result, engineVersion: "0", mode: "thin" },
    chunks,
    blobThreshold: 1,
    writeBlob,
  });
  const readBlob = vi.fn(async (sha: string) => blobs.get(sha)!);
  await expect(decodeBakedLightingAsset(bytes, readBlob)).rejects.toThrow(
    "64 MiB",
  );
  expect(readBlob.mock.calls).toEqual([[await sha256Hex(chunks[0]!.data)]]);
  readBlob.mockClear();
  const unexpected = await encodeBabasset({
    header: { ...result, engineVersion: "0", mode: "thin" },
    chunks: [
      ...result.chunks,
      {
  ...
  await expect(decodeBakedLightingAsset(unexpected, readBlob)).rejects.toThrow(
    "Unexpected baked lighting asset chunks",
  );
