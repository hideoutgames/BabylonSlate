  ...
  document.nodes.push(
    {
      ...
      type: "param.color",
      position: { x: 0, y: 0 },
      properties: { name: "Tint", value: [0.1, 0.2, 0.3, 0.4] },
    },
    {
      id: "unused",
      type: "param.float",
      position: { x: 0, y: 0 },
      properties: { name: "Unused", value: [9] },
    },
    {
      id: "call",
      type: "function.call",
      position: { x: 0, y: 0 },
      properties: { functionGuid: "fn" },
    },
    {
      id: "multiply",
      type: "math.multiply",
...
describe("worker Material parameter catalog", () => {
  it("requires full function lowering and exposes only the actual root parameter bindings", () => {
    ...
    const catalog = buildMaterialParameterCatalog(
      documents,
      new Map([["fn", fn]]),
    );
    expect(catalog.post).toMatchObject({
      domain: "postProcess",
    ...
    color.value[0] = 0.9;
    expect(catalog.post!.parameters.Tint).toEqual({
      kind: "color",
      value: [0.1, 0.2, 0.3, 0.4],
    });
    expect(
      document.nodes.find((node) => node.id === "tint")!.properties.value,
  ...
  it("preserves typed Float/Texture defaults and rejects malformed transport domains and values", () => {
    ...
    document.nodes.push(
      {
        ...
        type: "param.float",
        position: { x: 0, y: 0 },
        properties: { name: "Roughness", value: [0.35] },
      },
      {
        id: "image",
        type: "param.texture",
        position: { x: 0, y: 0 },
        properties: { name: "Image", textureGuid: "image" },
      },
      {
        id: "sample",
        type: "texture.sample",
    ...
    document.edges = document.edges.filter(
      (edge) => !["roughness", "baseColor"].includes(edge.targetPinId),
    );
    document.edges.push(
      {
        id: "rough-out",
    ...
    const catalog = buildMaterialParameterCatalog(
      new Map([["surface", document]]),
    );
    expect(catalog.surface!.parameters).toEqual({
      Roughness: { kind: "float", value: 0.35 },
      Image: { kind: "texture", textureAssetGuid: "image" },
    });
    expect(
