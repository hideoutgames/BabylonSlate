...
Local shared mode admits up to three one-worker phases across agents. Tooling and docs use smaller dedicated memory reservations; heavy application builds keep two slots and their larger reservation. Keep shared mode when agents overlap, and reserve `BL_TEST_PROFILE=fast` for a single active agent.
