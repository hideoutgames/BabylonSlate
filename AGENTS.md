...
Never generate artwork or use larger GitHub runners. Agents run only targeted local tests and relevant scoped static checks before opening or updating a PR. Select explicit test files/cases for changed behavior and directly affected consumers; do not automatically run full suites, coverage, all browser tests, workspace-wide typechecks, or the cumulative `pnpm verify:local` preflight. A broader local run requires an explicit user request, including after public API, lockfile, infrastructure, or CI-repair changes. Instruction/prose-only edits need diff/link checks rather than application tests. See the workflow rule for scope selection and reuse of unaffected results. Required GitHub CI and merge gates remain in place. Mark ready once when fewer than two counted non-draft PRs target `main` (#271 is excluded); otherwise keep the draft and wait. Public PR descriptions contain only a short change summary, without attribution, session links, secrets, or logs.

Local execution defaults to the per-user low-memory policy: at least 2 GiB host headroom, one active root workload, one test worker, and no lightweight bypass. Keep `BL_TEST_PROFILE=shared`; neither `fast`, `CI=true`, nor a config override may relax the host policy. Use `pnpm test:resources` for a read-only snapshot. Build artifacts retain source, environment, toolchain, and integrity checks. Do not change reservations or lower reserves to escape a queue. See [local execution settings](docs/architecture/testing.md#local-test-execution), including remaining Windows supervision limitations.
