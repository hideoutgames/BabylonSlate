...
PRs stay draft until local `pnpm verify` passes; mark ready once **only if fewer than 3 non-draft PRs target `main`**. If all three slots are taken, leave the draft and stop — a human marks ready. Details in [.cursor/rules/github-actions-pr-cadence.mdc](.cursor/rules/github-actions-pr-cadence.mdc).
