...
Never generate artwork or use larger GitHub runners. Pass focused regressions and `pnpm verify:local` before opening any PR, including drafts. The preflight checks changed package owners and related tests; prose/instruction metadata is diff-only, while root build configuration and unknown paths retain conservative static checks. Leave full consumer typechecking, coverage, and browser suites to GitHub CI; run the full workspace typecheck locally as an extra check when intentionally changing a public cross-package API. Mark ready once when fewer than two counted non-draft PRs target `main` (#271 is excluded); if both slots are occupied, keep the draft and wait for a slot. Monitor CI, fix failures with focused regression checks and local preflight before each repair push, and merge automatically once the current head passes all gates. Public PR descriptions contain only a short change summary, without attribution, session links, secrets, or logs.
