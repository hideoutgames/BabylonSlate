...
Local shared mode uses the per-user resource configuration across worktrees. On constrained machines, use its low-memory profile: 3 GiB host headroom, one heavy phase, and bounded bypass for smaller checks. Build artifacts with matching source, environment, and toolchain can share the user cache. Keep `BL_TEST_PROFILE=shared`; do not change reservations per worktree or increase workers to escape a queue. See [local execution settings](docs/architecture/testing.md#local-test-execution).
