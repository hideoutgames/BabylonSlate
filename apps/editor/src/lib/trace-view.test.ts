...
describe("trace view helpers", () => {
  ...
  it("retains log source metadata and filters severity, category and print keys", () => {
    ...
    expect(lines[0]).toMatchObject({ frameIndex: 0, tickIndex: 1, kind: "log", severity: "log", category: "game" });
    expect(filterTraceLogs(lines, "game", "log").map((line) => line.text)).toEqual(["first", "second"]);
    expect(filterTraceLogs(lines, "a", "print").map((line) => line.key)).toEqual(["a"]);
  });

  it("keeps each timing bucket's true spike and recorded-event marker", () => {
    const frames = [1, 50, 2, 4, 3].map((scriptMs, tickIndex) => ({ tickIndex, scriptMs, physicsMs: 1, logs: [], prints: tickIndex === 0 ? [{ key: "a", message: "event" }] : [] }));
    const buckets = traceGraphBuckets({ ...payload, frames }, 0, 4, 2);
    expect(buckets).toEqual([
      { start: 0, end: 2, peak: 1, scriptMs: 50, physicsMs: 1, overBudget: true, hasEvents: true },
      { start: 3, end: 4, peak: 3, scriptMs: 4, physicsMs: 1, overBudget: false, hasEvents: false },
    ]);
  });

  it("rejects malformed frame content instead of crashing a dock", () => {
    expect(asTracePayload({ ...payload, frames: [null] })).toBeNull();
    expect(asTracePayload({ ...payload, frames: [{ ...payload.frames[0], logs: [null] }] })).toBeNull();
  });
});
