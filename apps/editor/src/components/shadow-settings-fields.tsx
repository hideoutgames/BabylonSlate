import { normalizeShadowSettings, resolveShadowSettings, SHADOW_LIMITS, SHADOW_PROFILES, type ShadowOverrides, type ShadowSettings } from "@babylonslate/core";
...
const options: Partial<Record<keyof ShadowSettings, Record<string, string>>> = {
  profile: { economy: "Economy", a16: "iPad A16", high: "High", ultra: "Ultra" },
...
const fields: { key: keyof ShadowSettings; label: string; description: string }[] = [
...
export function ShadowSettingsFields({ project, overrides, onChange }: {
  project?: ShadowSettings; overrides?: ShadowOverrides; onChange: (value: ShadowOverrides) => void;
}) {
  ...
  const patch = (key: keyof ShadowSettings, value: string | number | boolean) => {
    const preset = key === "profile" ? SHADOW_PROFILES[value as keyof typeof SHADOW_PROFILES] : {};
    onChange({ ...(scene ? overrides : defaults), ...preset, [key]: value });
  };
  return <FieldSet data-testid={scene ? "scene-shadow-settings" : "project-shadow-settings"}>
    <FieldLegend>Shadows</FieldLegend>
    <FieldGroup className="gap-2">{fields.filter(({ key }) => !scene || key !== "profile").map(({ key, label, description }) => {
      const id = `${scene ? "scene" : "project"}-shadow-${key}`;
      const overridden = scene && Object.hasOwn(overrides, key);
      const choices = options[key];
      const limits = key in SHADOW_LIMITS ? SHADOW_LIMITS[key as keyof typeof SHADOW_LIMITS] : [0, 2048];
      return <Field key={key} className="settings-field">
        <div className="flex items-center justify-between gap-2">
          <FieldLabel htmlFor={id}>{label}</FieldLabel>
          {scene ? <Button type="button" variant="ghost" size="sm" aria-label={`${overridden ? "Reset" : "Override"} ${label}`} onClick={() => {
        ...
        {key === "enabled" ? <Switch id={id} checked={effective.enabled} disabled={scene && !overridden} onCheckedChange={(value) => patch(key, value)} />
          ...
          </Select> : <NumberField id={id} value={Number(effective[key])} min={limits[0]} max={limits[1]} step={key === "distance" || key === "cascades" || key === "maxLocalLights" ? 1 : 0.0001} disabled={scene && !overridden} onChange={(value) => patch(key, value)} />}
        <FieldDescription>{scene ? `${overridden ? "Scene Override · Project" : "Project Setting"}: ${String(defaults[key])}. ` : ""}{description}</FieldDescription>
      </Field>;
    })}</FieldGroup>
  </FieldSet>;
}
