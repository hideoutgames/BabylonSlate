...
async function fixture() {
  ...
  manifest.receivers.push({
    ...structuredClone(manifest.receivers[0]!),
    identity: {
      actorId: "other",
      componentId: "mesh",
      primitive: { kind: "mesh" },
  ...
  const sources = ["model", "cube"].map((guid): IndexedAsset => ({
    ...bake,
...
it("retains Scene references, all atlas chunks and source dependencies through packed export and reopen", async () => {
  ...
  expect(
    assetReferencesIncludingOpenDocuments("bake", assets, [
      { ref: { path: "assets/Scene.babasset" }, content: payload },
    ]).inbound,
  ).toEqual(["scene"]);
  ...
  const result = await collectAndExportGame({
    ...
    assets: assetsFromIndexed(assets),
    plugins: [],
  ...
  const sceneEntry = result.value.manifest.assets.find(
    (asset) => asset.guid === "scene",
  )!;
  const scenePack = decodeBabpack(result.value.files.get(sceneEntry.pack)!);
  expect(
    normalizeScene(
      JSON.parse(new TextDecoder().decode(scenePack.read("scene"))),
    ).settings.bakedLightingAssetGuid,
  ).toBe("bake");
...
it("fails export explicitly if any assigned bake atlas is missing instead of packing a partial output", async () => {
  ...
  const loaded = await loadExportDocuments({
    assets,
    loadDocument: async () => scene,
    readAssetChunk: (path, id) =>
      id === "atlas:second" ? Promise.resolve(null) : readAssetChunk(path, id),
  });
  expect(() => loaded.bytesByGuid("bake")).toThrow(
    "Missing baked lighting chunk atlas:second",
  );
  scene.settings.bakedLightingAssetGuid = null;
  assets[0]!.header.dependencies = [];
  const unrelated = await collectAndExportGame({
    startupSceneGuid: "scene",
    assets: assetsFromIndexed(assets),
    plugins: [],
    projectPluginOverrides: {},
  ...
  if (unrelated.ok)
    expect(unrelated.value.manifest.assets.map((asset) => asset.guid)).toEqual([
      "scene",
    ]);
});
