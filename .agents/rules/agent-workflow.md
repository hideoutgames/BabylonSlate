...
- Before a PR is ready, push after every commit with `git push -u origin <branch>`. For fixes to a ready PR, commit, pass local `pnpm verify`, then push the verified batch so each push starts one CI run.
...
- `pnpm verify` passes locally for the current PR head.
