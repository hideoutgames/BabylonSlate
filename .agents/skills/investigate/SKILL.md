---
name: investigate
description: Investigate, trace, map, or audit a large area of the repository in read-only mode, using delegated slices when available or sequential passes. Use for deep codebase explanations and investigation supporting an explicitly requested plan.
---
...
3. When delegation is available and authorized, dispatch slices in parallel within the host's concurrency limit. Otherwise investigate the same slices sequentially and keep a findings map. Scale agent count with breadth:
   - One file or one function → no sub-agent; read it yourself
   - One package or one feature → 1–2 read-only code-search agents
...
**Delegation capabilities:** choose an available read-only code-search agent; include git or documentation access only when the slice needs it. Do not require particular tool names or agent types. Inherit the parent model by default; explicit selections follow [the model policy](../../rules/agent-workflow.md#subagent-models). If delegation is unavailable, use sequential passes.
...
Do not recommend a patch list or implementation plan unless asked. An explicit planning request permits a read-only implementation plan; it does not authorize edits.
