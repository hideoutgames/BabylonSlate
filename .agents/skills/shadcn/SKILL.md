...
Run `pnpm dlx shadcn@latest info --json` from `apps/editor` using the available shell tool, and inspect its JSON output before choosing components. This is an explicit command, not host-specific template expansion. Tool permissions come from the host.

The output contains the project config and installed components. Use `pnpm dlx shadcn@latest docs <component>` to get documentation and example URLs for any component.
