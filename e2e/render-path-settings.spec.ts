...
test("pipeline preferences preserve the effective viewport and Scene path inheritance through history and reopen", async ({ page }, testInfo) => {
  ...
  await expect(page.getByTestId("setting-render-mode")).toHaveText("PBR");
  ...
  await expect(path).toHaveText("Clustered Forward");
  ...
  await expect(path).toHaveText("Forward");
  ...
  await expect(page.getByTestId("scene-render-path")).toHaveText("Clustered Forward");
  await page.screenshot({ path: testInfo.outputPath("scene-pipeline-inherited.png") });
  await openRendering(page);
  await expect(page.getByTestId("project-render-path")).toHaveText("Clustered Forward");
  await expect(page.getByTestId("project-gpu-backend")).toHaveText("WebGPU");
  await expect(page.getByTestId("setting-render-mode")).toHaveText("PBR");
  ...
  await page.getByRole("button", { name: "Done", exact: true }).click();
  await waitForSceneViewportReady(page);
  await expect(page.getByTestId("scene-render-path")).toHaveText("Auto");
