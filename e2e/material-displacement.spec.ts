...
for (const mode of ["pbr", "cel"]) {
  test(`animated normal displacement keeps the ${mode} Material preview rendering ${IPAD_TEST_TAG}`, async ({
    page,
  }) => {
    const errors: string[] = [];
    ...
    await expect(
      page.getByTestId("material-graph-editor").locator(".react-flow__edge"),
    ).toHaveCount(doc.edges.length + 1);
    await compileMaterialPreview(page);
    const first = await canvas.evaluate((node: HTMLCanvasElement) =>
      node.toDataURL(),
    );
    ...
    await expect
      ...
      .not.toBe(first);
    const second = await canvas.evaluate((node: HTMLCanvasElement) =>
      node.toDataURL(),
    );
    await expect
      .poll(
