...
for (const backend of ["webgl2", "webgpu"] as const) {
  test(`shared production outlines preserve consumers, occlusion and bounded work on ${backend}`, async ({ page }, testInfo) => {
    ...
    expect(defaultMaterial.lanes[1]!.green).toBeGreaterThan(0);
    expect(defaultMaterial.lanes[0]).toEqual(original.lanes[0]);
    expect(defaultMaterial.lanes[2]).toEqual(original.lanes[2]);
    ...
    expect(unchanged.renderers).toEqual(original.renderers);
    expect(original.renderers.length).toBeGreaterThan(0);
    ...
    expect(edited.renderers).toEqual(original.renderers);
    ...
    expect(strict.coveredBoundaryRed).toBe(0);
    ...
    const close = at("close-partial-occluder");
    expect(close.lanes[1]!.red).toBeGreaterThan(0);
    expect(close.coveredBoundaryRed).toBe(0);
    expect(close.coveredPartialRed).toBe(0);
    expect(at("close-partial-occluder-removed").lanes).toEqual(at("occluders-removed").lanes);
    ...
    expect(steady.renderers).toEqual(retired.renderers);
    ...
    expect(report.unpresentedRetirement.preparedPath).toBe("frameGraph");
    expect(report.unpresentedRetirement.allocated.reservedBytes).toBeGreaterThan(report.unpresentedRetirement.before.reservedBytes);
    expect(report.unpresentedRetirement.after).toEqual(report.unpresentedRetirement.before);
    expect(report.unpresentedRetirement.retainedRenderers).toBe(0);
  });
}
