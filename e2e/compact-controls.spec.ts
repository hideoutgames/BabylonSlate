...
test("snap settings open on hold and right-click without toggling", { tag: IPAD_TEST_TAG }, async ({ page }) => {
  await openMinimalTestProject(page);
  await openMainScene(page);
  ...
  await expect(dialog).toBeVisible();
  await expect(dialog.getByLabel("Rotation Snap (Degrees)")).toHaveValue("45");
