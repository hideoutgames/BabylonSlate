...
export function validateJobRunner(job, context) {
  ...
  const fail = () => { throw new Error(`${context}: expected literal standard GitHub-hosted runner labels`); };
  ...
  if (!matrix || typeof matrix !== "object" || Array.isArray(matrix)) return fail();
  const axis = matrix[matrixKey];
  const includes = matrix.include ?? [];
  if (!Array.isArray(includes) || (axis !== undefined && !Array.isArray(axis))) return fail();
  if (includes.some(row => !row || typeof row !== "object" || Array.isArray(row))) return fail();
  const labels = [...(axis ?? []), ...includes.filter(row => matrixKey in row).map(row => row[matrixKey])];
  if (!labels.length || labels.some(value => !standardRunners.has(value))) return fail();
...
export async function validateWorkflowRunners(root, path, visited = new Set()) {
  ...
  if (!workflow?.jobs || typeof workflow.jobs !== "object") throw new Error(`${path}: missing jobs`);
  for (const [name, job] of Object.entries(workflow.jobs)) {
    if (job.uses !== undefined) {
      if (typeof job.uses !== "string" || !job.uses.startsWith("./.github/workflows/"))
        throw new Error(`${path}/${name}: reusable workflow must have locally auditable runners`);
