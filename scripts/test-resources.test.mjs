...
test("unknown constraints and failed Windows probes remain explicit", async () => {
  const observation = await observeHostMemory({ platform: "win32", totalmem: () => 16, availableMemory: () => 5, constrainedMemory: () => 0, windowsProbe: () => { throw new Error("denied"); } });
  ...
  const constrained = await observeHostMemory({ platform: "linux", totalmem: () => 16, availableMemory: () => 5, constrainedMemory: () => 8 });
...
test("diagnostics expose owned reservations without secrets or unrelated environment", async (t) => {
  ...
  await writeFile(join(directory, "queue", "owned.json"), JSON.stringify({ pid: 123, active: true, token: "private-ticket-token", command: "private-command", request: { workers: 1, memoryGiB: 2, browsers: 0 } }));
  const report = await resourceReport({ directory, env: { BL_LOCAL_RESOURCE_CONFIG: join(directory, "missing.json"), API_TOKEN: "private-api-token" }, observeMemory: () => ({ availableBytes: null, errors: ["unavailable"] }) });
