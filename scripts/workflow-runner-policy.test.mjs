...
test("runner selection rejects premium, self-hosted, grouped, and uncontrolled expressions", () => {
  for (const label of [
    ...
    assert.throws(
      () => validateJobRunner({ "runs-on": label }, "fixture"),
      /standard/,
    );
  }
  for (const matrix of [
    "${{ fromJSON(inputs.matrix) }}",
    { os: ["ubuntu-latest", "ubuntu-paid"] },
    { os: ["ubuntu-latest"], include: [{ os: "self-hosted" }] },
    { include: [{ other: "ubuntu-latest" }] },
  ]) {
    assert.throws(
      () =>
        validateJobRunner(
...
test("a local reusable workflow cannot hide an unsafe runner", async (t) => {
  ...
  await writeFile(
    join(root, ".github/workflows/parent.yml"),
  ...
  await assert.rejects(
    validateWorkflowRunners(root, ".github/workflows/parent.yml"),
    /child.yml\/check/,
  );
